...
procedure TRecBolForm.DBGrid1DrawColumnCell(Sender: TObject; const Rect: TRect; DataCol: Integer; Column: TColumn; State: TGridDrawState);
begin
  if Assigned(Baixa2Form) then
  begin
    if (BancodeDados.RecBolSITUACAO_BOLETO.Value = 'A RECEBER') and (BancodeDados.RecBolVENCIMENTO.Value < Baixa2Form.Dt_Pagar.DateTime) then
      DBGrid1.Canvas.Font.Color := clRed;
  end
  else
  begin
    if (BancodeDados.RecBolSITUACAO_BOLETO.Value = 'A RECEBER') and (BancodeDados.RecBolVENCIMENTO.Value < Date) then
      DBGrid1.Canvas.Font.Color := clRed;
  end;
...
procedure TRecBolForm.DBGrid1KeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  if Key = 13 then
    Key := 9;
end;
...
begin
  if Assigned(Baixa2Form) then
  begin
    PrincipalForm.CalcularValorBaixaIndividual(Baixa2Form.Dt_Pagar.DateTime, False);
  end
  else
  begin
...
begin
  ...
  begin
    ...
    begin
      ...
      BancodeDados.RecBolVALOR_APAGAR.Value := BancodeDados.RecBolVALOR_INTEGRAL.Value + BancodeDados.RecBolACRESCIMO.Value + BancodeDados.RecBolMULTA.Value +
        BancodeDados.RecBolJUROS.Value;
      if (RetirarDesc = true) then
      begin
        if not(BancodeDados.RecBolDIAS.Value > 0) then
          BancodeDados.RecBolVALOR_APAGAR.Value := BancodeDados.RecBolVALOR_APAGAR.Value - BancodeDados.RecBolDESCONTO.Value;
      end
      else
      begin
        BancodeDados.RecBolVALOR_APAGAR.Value := BancodeDados.RecBolVALOR_APAGAR.Value - BancodeDados.RecBolDESCONTO.Value;
