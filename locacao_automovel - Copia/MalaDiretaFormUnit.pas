    ...
    begin
      ...
      begin
        if strL_emails[i] <> EmptyStr then
          EditDestinatarios.Text := EditDestinatarios.Text +
            strL_emails[i] + ';';
      end;
    end;
...
begin
  ...
  if list_arquivos.Items.Count > 0 then
    resposta := BancodeDados.EnviarEmail(EditDestinatarios.Text,
      EditAssunto.Text, WebBrowser1.OleObject.Document.DocumentElement.
      InnerHtml, list_arquivos.Items.Strings[0])
  else
    resposta := BancodeDados.EnviarEmail(EditDestinatarios.Text,
      EditAssunto.Text, WebBrowser1.OleObject.Document.DocumentElement.
      InnerHtml, EmptyStr);
...
begin
  ...
  fWBW.LoadFromString(trim(StringReplace(Memo1.Text, '%texto_livre%',
