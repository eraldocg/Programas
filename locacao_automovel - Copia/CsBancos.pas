...
procedure TCsBancosForm.EditNomeKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
        ...
        begin
          ...
          begin
            ...
            if nmcampo <> 'BANCO_ID' then
              try
                BancodeDados.Bancos.FieldByName(nmcampo).Value :=
                  qryBancos.FieldByName(nmcampo).Value;
              except
              end;
          end;

          // BancodeDados.BancosUTIL_FREQ.Value:=1;
          BancodeDados.BancosBANCO_NOME.Value := 'Cópia ' +
          ...
          BancodeDados.BancosN_SEQ_REM_DEB_AUTO.Value :=
            NumeroSeqDebAut + 10000;
...
begin
  ...
  begin
    ...
    begin
      if BancodeDados.BancosCONT_ENCERRADA.Value = 1 then
      begin
      ...
      begin
        ...
        if (Trim(BancodeDados.BancosCODIGO_CEDENTE.Value) <> '') and
          (BancodeDados.BancosCODIGO_CEDENTE.Value <> '0') and
          (BancodeDados.BancosNOSSO_NUMERO.Value > 0) then
        begin
    ...
    Try
      ...
      begin
        ...
        if cbVisualizar.ItemIndex > 0 then
          SQL.Add(' and coalesce(cont_encerrada,0)=' +
            IntToStr(cbVisualizar.ItemIndex - 1));
