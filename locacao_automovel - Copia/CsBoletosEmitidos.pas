...
begin
    ...
    begin
      ...
      try
        ...
        Mensagem('Ocorrência não encontrada! Processe o arquivo retorno correspondente ao período do registro.', mtInformation, [mbOk], mrOk, 0);
        end;
     ...
     begin
      PodeIR := true; //PrincipalForm.LiberaAcesso(72, 'Processar arquivos de retorno');
       ...
       begin
          ...
          Mensagem('Processo concluído com sucesso!'#13#13+'Obs.: Apenas boleto com vencimento acima da data atual será enviado na remessa!', mtInformation, [mbOk], mrOk, 0);
        ...
        begin
          ...
          begin
            if (BancodeDados.RetornoNUMERO_ARQUIVO.Value > 0) then
              Mensagem('Operação não autorizada! Só é possível estornar boletos que tenham sido baixados manualmente.', mtWarning, [mbOk], mrOk, 0)
            else
              podePassar := true;
          end;
              ...
              try
                ...
                begin
                  ...
                  begin
                    ...
                    begin
                      Mensagem('Não é possível efetuar estorno em uma data já encerrada.', mtWarning, [mbOk], mrOk, 0);
                      HabilitarBotoes(Self, true);
                      Screen.Cursor := crDefault;
                      exit;
                    end;
                  ...
                  if Mensagem('Esta operação irá estornar todos os títulos do recebimento múltiplo abaixo relacionados:' + #13#13 + linha + #13#13 + 'Deseja realmente prosseguir?', mtWarning, [mbyes, mbNo], mrNo, 0, 'Courier New') = idyes then
                  begin
                    BancodeDados.Adiciona.SQL.Text := 'update livro_caixa set cancelado = 1, can_motivo = ' + QuotedStr('Estorno do recebimento múltiplo ' + idsReceb + ' pelo usuário ' + usrLogin) + ' where receb_id in (' + idsReceb + ')';
                    BancodeDados.Adiciona.ExecSQL;
...
//        end;
        ...
        begin
          if CbRemessa.ItemIndex=1 then
             BancodeDados.V_BOLETOS.SQL.Add(' and coalesce(remessa,0)>0 and coalesce(cod_mov,0)=0') else // AGUARDANDO CONFIRMAÇÃO >0 =0
          ...
          if CbRemessa.ItemIndex=3 then
             BancodeDados.V_BOLETOS.SQL.Add(' and coalesce(remessa,0)=0 and coalesce(cod_mov,0)=0'); // NÃO ENVIADA =0 =0
        end;
        ...
        begin
          if Mensagem('A consulta completa poderá levar vários minutos.'#13 + 'Deseja executá-la?', mtConfirmation, [mbyes, mbno], mryes, 0) = idyes then
          begin
...
begin
  ...
  if not BancodeDados.V_BOLETOS.IsEmpty then begin
    ...
    try
      ...
      relBoletosEmitidosForm.lbPeriodo_1.Caption := 'Período: ' + cbCampo.Text + ' - ' + (Dt_Inicial.DateTimeStr) + ' até ' + (Dt_Final.DateTimeStr);
      if ChkBolValidos.Checked then
        relBoletosEmitidosForm.lbfiltro_1.Caption := 'Boletos Válidos'
      else
        relBoletosEmitidosForm.lbfiltro_1.Caption := 'Boletos Válidos/Inválidos';
    ...
    Mensagem('Não foi encontrada nenhuma informação!', mtInformation, [mbok], mrok, 0);
...
begin
    ...
    if Mensagem('Deseja realizar uma reimpressão de boleto?', mtConfirmation, [mbyes, mbno], mrNo, 0) = idyes then  begin
    HabilitarBotoes(self,false);
...
begin
  ...
  if (IDEmissao <> '') then
  begin
    if Mensagem('Tem certeza que deseja cancelar este boleto?'+#13#13+'Atenção. O sistema não cancela boletos já enviados para o banco.', mtConfirmation, [mbyes, mbno], mrNo, 0) = idyes then
    begin
...
begin
  ...
  if not(listaV_Boletos.Count > 0) then
    Mensagem('Selecione alguma emissão para ser re-impressa.', mtInformation, [mbok], mrok, 0)
  else
  begin
    Screen.Cursor := crSQLWait;
    ...
    begin
      try
        ...
        //try
          if MensagemChk('Deseja realizar uma reimpressão de boleto?', mtConfirmation, [mbYes, mbNo], mrYes, 0, true, 'Papel cortado (1 boleto na vertical)?',
    ...
    begin
      ...
      begin
        ...
        try
          ...
          try
            ...
            // MM.DataField  := 'OBS';
            MM.Lines.Text := 'Obs.: ' + #13 + BancodeDados.V_BoletosOBS.AsString + #13 + #13 + #13 + 'Descrição: ' + #13 + BancodeDados.V_BoletosOBS.AsString;
            MM.ReadOnly := true;
            Frm.showmodal;
          finally
            MM.Free;
      ...
      begin
        ...
        begin
          ...
          Mensagem('Não foi encontrado nenhum título referente a está emissão.'+#13+'Título(s) de origem: ' + IntToStr(BancodeDados.V_BoletosEMISSAO_ID.Value), mtInformation, [mbok], mrok, 0);
      end;
    end;
end;
  ...
  if BancodeDados.V_BoletosEMISSAO_ID.Value > 0 then
    ...
    begin
      ...
      begin
        ...
        Mensagem('Não foi encontrado nenhum retorno referente a está emissão.', mtInformation, [mbok], mrok, 0);
    end;
end;
            ...
            begin
              ...
              begin //Alteração do dias para baixa/devolucao no banco
                 ...
                 begin //caso tenha gerado a remessa, deve gerar um outro arq. remessa com o codigo de movi
                  ...
                  case BancodeDados.V_BoletosCOD_BANCO.Value of  //pegando o código de ocorrencia para alterar boleto
                     104 : TipoOcorrencia :=31; //8; //Caixa Econômica Federal cnab400
                     else
                           TipoOcorrencia :=31;//alguns bancos utilizam deste mesmo código para alteração de outros dados...
                  end;
...
begin
  ...
  if not (BancodeDados.V_BoletosCANCELADO.Value = 1) then begin
            ...
            begin
              ...
              begin
                ...
                if (recebidoTmp = true) then
                  Mensagem('Um ou mais títulos desta emissão já foram recebidos.', mtInformation, [mbok], mrok, 0)
                else
                begin
                ...
                Mensagem('Não foi encontrado nenhum título referente a está emissão.', mtInformation, [mbok], mrok, 0);
            end
            else
              Mensagem('Este boleto já foi recebido.', mtInformation, [mbok], mrok, 0);
          end; // True
    ...
    Screen.Cursor := crDefault;
    HabilitarBotoes(self, true);
  end else Mensagem('Marque algum boleto válido!', mtInformation, [mbOk], mrOk, 0);//if not (BancodeDados.V_BoletosCANCELADO.Value = 1)
end;
