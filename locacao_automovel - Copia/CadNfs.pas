...
type
  TCadNfsForm = class(TForm)
    ...
    procedure SedDBGrid1KeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        try
          ...
          begin
            ...
            if (Trim(BancodeDados.NfsServDESCRICAO.Value) <> '') and (BancodeDados.NfsServQUANT.Value > 0) and (BancodeDados.NfsServVL_UNIT.Value > 0) then
      ...
      begin
        Mensagem('Informe algum item antes de gravar.', mtInformation, [mbOK], mrOk, 0);
      end;
    end
    else
    begin
  ...
  begin
    Mensagem('Informe a data de emissão do RPS!', mtInformation, [mbOK], mrOk, 0);
...
begin
  ...
  begin
    ...
    begin
      ...
      CadNfsForm.Caption := CadNfsForm.Caption + ' - Boleto(s): ' + Trim(BancodeDados.NfsIDS_BOLETOS.Value);
    end;
  end
  else
  begin
  ...
  CadNfsForm.Caption := CadNfsForm.Caption + ' - Código: ' + FormatFloat('000000', BancodeDados.NfsNFS_ID.Value) + ' - NFSe Nº: ' +
...
begin
  ...
  begin
    ...
    begin
      Memo1.Lines.Add(BancodeDados.NfsServDESCRICAO.Value + ' - ' + FormatFloat('R$ ,0.00', BancodeDados.NfsServVL_TOTAL.Value));
      BancodeDados.NfsServ.Next;
    end;

    BancodeDados.Boletos.Close;
    BancodeDados.Boletos.SQL.Text := PrincipalForm.SQLrepositorio(0) + ' and b.nfs_id = ' + IntToStr(BancodeDados.NfsNFS_ID.Value) + ' order by b.recebimento';
    ...
    begin
      Memo1.Lines.Add('COD.: ' + FormatFloat('000000', BancodeDados.BoletosBOLETO_ID.Value) + ' - ' + 'Vencimento: ' + FormatDateTime('dd/mm/yyyy',
    ...
    try
      ...
      if CsClientesForm.ShowModal = mrOk then
        ...
        begin
          ...
          BancodeDados.NfsEMAIL_CONTATO2.Value := BancodeDados.ClientesEMAIL_CONTATO2.Value;
        end;
      if EditCli.CanFocus then
        EditCli.SetFocus;
    finally
...
procedure TCadNfsForm.SedDBGrid1KeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  if Key = 9 then
  begin
    Key := 0;
...
begin
  if (BancodeDados.NfsSIT.Value = 0) and (BancodeDados.NfsServNFS_IT_ID.Value > 0) then
  begin
    if Mensagem('Tem certeza que deseja excluir este serviço?', mtConfirmation, [mbyes, mbNo], mryes, 0) = idYes then
    begin
      if not BancodeDados.FDConnection1.InTransaction then
        BancodeDados.FDConnection1.StartTransaction;
