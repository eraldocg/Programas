...
begin
 //if PrincipalForm.LiberaAcesso(183, 'protestar o(s) título(s)') then
  if Mensagem('Tem certeza que deseja retirar este título do protesto?', mtConfirmation, [mbYes, mbNo], mrNo, 0) = idYes then
...
begin
 //if PrincipalForm.LiberaAcesso(183, 'cancelar protesto(s) de título(s)') then
  if Mensagem('Tem certeza que deseja incluir este título no protesto?', mtConfirmation, [mbYes, mbNo], mrNo, 0) = idYes then
...
if listaBoletos.Count >0 then begin
SQlAntigo  :='';
IDsBoletos := '';
modeloEtiqueta:=0;

  try
  if not Assigned(RelEtiquetasBolForm) then  RelEtiquetasBolForm := TRelEtiquetasBolForm.Create(Application);
  HabilitarBotoes(self,false);
  SQlAntigo  :=BancodeDados.Boletos.SQL.Text;
  IDsBoletos := BancodeDados.PegarAtivo(listaBoletos);

    BancodeDados.Boletos.Close;
    BancodeDados.Boletos.SQL.Text:=PrincipalForm.SQLrepositorio(0) + ' and b.boleto_id in (' + (IDsBoletos) +') order by b.emissao_id desc';
  ...
  while not BancodeDados.boletos.Eof  do begin

    BancodeDados.Config.Open;

    BancodeDados.TempEtiqueta.Append;
    BancodeDados.TempEtiquetaRESPONSAVEL.AsString     := BancodeDados.BoletosCLIENTE.AsString;
    BancodeDados.TempEtiquetaTURMA_NOME.AsString      := BancodeDados.BoletosFantasia.AsString;

    BancodeDados.Clientes.Close;
    BancodeDados.Clientes.SQL.Text:=PrincipalForm.SQLrepositorio(1) +' and c.cli_id = '+IntToStr(BancodeDados.BoletosCLI_ID.Value);
    BancodeDados.Clientes.Open;

    BancodeDados.TempEtiquetaLOGRADOURO.AsString  := BancodeDados.ClientesLOGRADOURO.Value;
    BancodeDados.TempEtiquetaNUMERO.AsString      := BancodeDados.ClientesNUMERO.Value;
    BancodeDados.TempEtiquetaBAIRRO.AsString      := BancodeDados.ClientesBAIRRO.Value;
    BancodeDados.TempEtiquetaCIDADE.AsString      := BancodeDados.ClientesCIDADE.Value;
    BancodeDados.TempEtiquetaUF.AsString          := BancodeDados.ClientesESTADO.Value;
    BancodeDados.TempEtiquetaCEP.AsString         := BancodeDados.ClientesCEP.Value;
    BancodeDados.TempEtiquetaCOMPLEMENTO.AsString := BancodeDados.ClientesOBS.Value;
  ...
  if modeloEtiqueta = 0 then  RelEtiquetasBolForm.RLReport5.Preview  else
  if modeloEtiqueta = 1 then   RelEtiquetasBolForm.RLReport4.Preview;
...
procedure TCsBaixaBancoForm.EditNomeKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  ...
  try
    ...
    begin
      if not BancodeDados.Bancos.Active then begin
        ...
        BancodeDados.Bancos.SQL.Text:='select * from bancos where CODIGO_BANCO>0 and BANCO_ID = '+IntToStr(BancoID);
        BancodeDados.Bancos.Open;
      end;
      Nome_Banco  :=BancodeDados.BancosBANCO_NOME.Value;

      RelArquivoRetornoForm.lbPeriodo1.Caption      := 'Data do Arquivo: ' + FormatDateTime('dd/mm/yyyy', Dt_Arquivo);
      RelArquivoRetornoForm.lbSituacao1.Caption     := 'Situação: RECEBIDO';
      RelArquivoRetornoForm.lbTipoQuitacao1.Caption :=TipoQuitacao;

      RelArquivoRetornoForm.lbPeriodo10.Caption     := 'Data do Arquivo: ' + FormatDateTime('dd/mm/yyyy', Dt_Arquivo);
      RelArquivoRetornoForm.lbSituacao10.Caption    := 'Situação: RECEBIDO';
      RelArquivoRetornoForm.lbTipoQuitacao10.Caption:=TipoQuitacao;

      if not BancodeDados.Bancos.IsEmpty then begin
        RelArquivoRetornoForm.lbArquivo1.Caption     := 'Banco: ' + Nome_Banco + ' - Arquivo Ret.: '+ QuotedStr(NomeArquivo) +' N.º '+IntToStr(COD_NArquivo);
        RelArquivoRetornoForm.lbArquivo10.Caption    := 'Banco: ' + Nome_Banco + ' - Arquivo Ret.: '+ QuotedStr(NomeArquivo) +' N.º '+IntToStr(COD_NArquivo);
      end else begin
        RelArquivoRetornoForm.lbArquivo1.Caption     := 'Arquivo Ret.: ' + QuotedStr(NomeArquivo) + ' -  N.º ' + IntToStr(COD_NArquivo);
        RelArquivoRetornoForm.lbArquivo10.Caption    := 'Arquivo Ret.: ' + QuotedStr(NomeArquivo) + ' -  N.º ' + IntToStr(COD_NArquivo);
      end;


    end
    else if AtivandoEve = 2 then //relatorio impresso via consulsta de baixa bancaria
    begin
      RelArquivoRetornoForm.lbPeriodo1.Caption := 'Período : ' + (Dt_Inicial.DateTimeStr) + ' até ' + (Dt_Final.DateTimeStr);
      RelArquivoRetornoForm.lbSituacao1.Caption := 'Situação: RECEBIDO';
      RelArquivoRetornoForm.lbArquivo1.Caption := 'Banco: ' + cbTipBanco.Text;

      RelArquivoRetornoForm.lbPeriodo10.Caption := 'Período : ' + (Dt_Inicial.DateTimeStr) + ' até ' + (Dt_Final.DateTimeStr);
      RelArquivoRetornoForm.lbSituacao10.Caption := 'Situação: RECEBIDO';
      RelArquivoRetornoForm.lbArquivo10.Caption := 'Banco: ' + cbTipBanco.Text;
    end; //

      if Assigned(RetornoForm) then begin // na janela Arquivo de Retorno, marcar: Na Consulta de Recebimento Via Banco >> Anexar os relatórios de “Recebimento Via Banco” e “Arquivo Retorno”
        if RetornoForm.cbAnexarRel.Checked then
         RelArquivoRetornoForm.RLReport10.NextReport:=RelArquivoRetornoForm.RLReport1;
      end;


      //BancodeDados.PrepararRel(RelArquivoRetornoForm.RLReport10);;
      BancodeDados.PrepararRel(RelArquivoRetornoForm.RLReport10, false, nil, chkMala.Checked, Gauge1);
  finally
...
begin
HlinkNavigateString(nil, 'http://www.sedsoft.com.br/manuais/doc_janela_recebido_via_banco.pdf');
end;
  ...
  Try
    ...
    begin
      ...
      begin
        Close;
        SQL.Clear;
        SQL.Add( PrincipalForm.SQLrepositorio(0) + ' and b.banco_id = ' + IntToStr(BancodeDados.RetornoBANCO_ID.Value) +
          ' and b.retorno_id =(select retorno_id from retorno where retorno_id=b.retorno_id and b.numero_arquivo =' + IntToStr(COD_NArquivo) + ')');
      end
      else if AtivandoEve = 2 then
      begin
        Close;
        SQL.Text := PrincipalForm.SQLrepositorio(0) + ' and b.retorno_id =(select retorno_id from retorno where retorno_id=b.retorno_id and (' +
          tipoData + ' between ' + QuotedStr(FormatDateTime('yyyy/mm/dd',CsBaixaBancoForm.Dt_Inicial.DateTime)) + ' and ' + QuotedStr(FormatDateTime('yyyy/mm/dd',CsBaixaBancoForm.Dt_Final.DateTime)) + ') ' + TxtArquivo + ' )';
      end;
...
procedure TCsBaixaBancoForm.EditConsultaKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
...
procedure TCsBaixaBancoForm.DBGrid1DrawColumnCell(Sender: TObject; const Rect: TRect; DataCol: Integer; Column: TColumn; State: TGridDrawState);
begin
  if not BancodeDados.Boletos.IsEmpty then
  begin
    ...
    begin
      ...
      if (BancodeDados.BoletosRECEBIMENTO.Value > 0) and (BancodeDados.BoletosSITUACAO_BOLETO.Value = 'RECEBIDO') then
      ...
      if (BancodeDados.BoletosSITUACAO_BOLETO.Value = 'RENEGOCIADO') or (BancodeDados.BoletosSITUACAO_BOLETO.Value = 'CRED RENEGOCIADO') then
        DBGrid1.Canvas.Font.Color := $009F9F9F;
...
begin
    if BancodeDados.TempEtiqueta.Active then BancodeDados.TempEtiqueta.Close;
    listaBoletos.Clear;

    if FileExists(DiretorioTemp + 'tempEti.xml') then DeleteFile(DiretorioTemp + 'tempEti.xml'); 
end;
...
begin
  ...
  BancodeDados.Prt_Boleto.SQL.Text:='select * from prt_boleto where sit=1 order by protestar';
  ...
  qryBancos.Close;
  qryBancos.SQL.Text := 'select banco_id, banco_nome from bancos where codigo_banco>0 and coalesce(cont_encerrada,0)=0 and codigo_cedente <> ' + QuotedStr('')+' and agencia <> ' + QuotedStr('')+' order by banco_nome';
  qryBancos.Open;     //(CODIGO_CEDENTE <> '' and CODIGO_CEDENTE<>'0') order by BANCO_NOME

  qryBancos.First;
  cbTipBanco.Items.Add('TODOS');
  while not qryBancos.Eof do
  begin
    cbTipBanco.Items.Add(FormatFloat('000000', qryBancosBANCO_ID.AsInteger) + ' - ' + (qryBancosBANCO_NOME.Value));
    qryBancos.Next;
  end; // while
