...
begin
  if (BancodeDados.BoletosEMISSAO_ID.Value > 0) then
    if Mensagem
      ('Tem certeza que deseja cancelar a emissão do boleto bancário deste título?',
      mtConfirmation, [mbyes, mbNo], mrNo, 0) = idYes then
    begin
      BancodeDados.Emissoes.close;
      BancodeDados.Emissoes.SQL.Text :=
        'select * from boletos_emissao where emissao_id = ' +
      ...
      begin
        if (BancodeDados.EmissoesRETORNO_ID.Value > 0) then
          Mensagem('Esta emissão já foi quitada através do arquivo de retorno.',
            mtInformation, [mbOk], mrOk, 0)
        else
        begin
          podePassar := True;
          if (BancodeDados.EmissoesREMESSA.Value > 0) then
            if Mensagem('Atenção! Este boleto já foi enviado ao banco.' + #13#13
              + 'Tem certeza que deseja cancelá-lo?', mtConfirmation,
              [mbyes, mbNo], mrNo, 0) = idNo then
              podePassar := False;
          ...
          try
            BancodeDados.Adiciona.SQL.Text :=
              'update boletos set emissao_id = 0 where emissao_id = ' +
              IntToStr(BancodeDados.BoletosEMISSAO_ID.Value);
...
begin
  ...
  begin
    if (date) > (BancodeDados.BoletosVENCIMENTO.Value) then
      EditDias.Text :=
        IntToStr((round(date - BancodeDados.BoletosVENCIMENTO.Value)));
...
procedure TCadBoletosForm.FormCloseQuery(Sender: TObject;
...
procedure TCadBoletosForm.DBNavigator1Click(Sender: TObject;
  Button: TNavigateBtn);
begin
  if BancodeDados.BoletosSITUACAO_BOLETO.Value = 'RECEBIDO' then
    Panel2.Enabled := False
