...
type
  ...
  public
    ...
    procedure EscreveDataHora(Nome: String; Valor: TDateTime);
    ...
    function LerDataHora(Nome: String; Padrao: TDateTime): TDateTime;
...
begin
  try
    FAberto := False;
    FSomenteLeitura := True;
    if Self.OpenKey(Chave, True) then
    begin
    ...
    begin
      if Self.OpenKeyReadOnly(Chave) then
      begin
        FAberto := True;
        FSomenteLeitura := True;
      end;
...
begin
  if (FAberto = True) and not(FSomenteLeitura = True) then
  begin
    try
...
procedure TSEDRegistro.EscreveDataHora(Nome: String; Valor: TDateTime);
begin
  if (FAberto = True) and not(FSomenteLeitura = True) then
  begin
    try
      Self.WriteDateTime(Nome, Valor);
    except
    end;
  end;
end;
...
function TSEDRegistro.LerDataHora(Nome: String; Padrao: TDateTime): TDateTime;
begin
  Result := Padrao;
  if (FAberto = True) then
  begin
    try
      Result := Self.ReadDateTime(Nome);
    except
    end;
  end;
end;
