...
procedure TCsOcorrenciasRetForm.DBGrid1DrawColumnCell(Sender: TObject; const Rect: TRect; DataCol: Integer; Column: TColumn; State: TGridDrawState);
...
begin
  ...
  begin
    if not(gdSelected in State) then
      ...
      if  BancodeDados.Retorno_OcorCOD_OCORRENCIA.Value = 2 then
        DBGrid1.Canvas.Font.Color := clNavy else DBGrid1.Canvas.Font.Color := clRed;
...
begin
  ...
  begin
    ...
    lbContadorN.Caption := 'Total de registros: ' + FormatFloat('000000',  BancodeDados.Retorno_Ocor.RecNo);
  ...
  begin
    ...
    Try
      ...
      begin
        ...
        begin
          Last;
          lbContadorN.Caption := 'Total de registros: ' + FormatFloat('000000', RecNo);
        end;
...
begin
  ...
  begin
    ...
    BancodeDados.qryBanco.Params[0].Value:=BancodeDados.Retorno_OcorBANCO_ID.Value;
    BancodeDados.qryBanco.Open;

     //(IntToStr(BancodeDados.qryBancoBAIX_BOL_PARAM.Value));

    BancodeDados.Emissoes.Close;
    BancodeDados.Emissoes.SQL.Text := 'select * from boletos_emissao where codigo_banco = '+IntToStr(BancodeDados.Retorno_OcorCODIGO_BANCO.Value);

    if (BancodeDados.qryBancoBAIX_BOL_PARAM.Value>0) then begin //OK

      if (BancodeDados.Retorno_OcorTIPO.Value <> 2) then begin //2
        BancodeDados.Emissoes.SQL.Add(' AND nosso_numero = ' + QuotedStr(BancodeDados.Retorno_OcorNOSSO_NUMERO.Value));

        if trim(BancodeDados.Retorno_OcorSEU_NUMERO.Value)<>'' then
        BancodeDados.Emissoes.SQL.Add(' AND emissao_id = ' + QuotedStr(BancodeDados.Retorno_OcorSEU_NUMERO.Value))
        else begin
          FormatSettings.DecimalSeparator:='.';
          if BancodeDados.Retorno_OcorDATA_VENCIMENTO.Value>0 then
          BancodeDados.Emissoes.SQL.Add(' AND vencimento ='+QuotedStr(FormatDateTime('yyyy/mm/dd',BancodeDados.Retorno_OcorDATA_VENCIMENTO.Value)));

          if BancodeDados.Retorno_OcorVALOR_DOCUMENTO.Value>0 then
          BancodeDados.Emissoes.SQL.Add(' AND valor_doc ='+CurrToStr(BancodeDados.Retorno_OcorVALOR_DOCUMENTO.Value));
         FormatSettings.DecimalSeparator:=',';
        end;
      end else //2
      BancodeDados.Emissoes.SQL.Add(' AND emissao_id = ' + QuotedStr(BancodeDados.Retorno_OcorSEU_NUMERO.Value));
      ...
      BancodeDados.Emissoes.SQL.Add(' AND banco_id = ' + IntToStr(BancodeDados.Retorno_OcorBANCO_ID.Value));

      if ( BancodeDados.Retorno_OcorTIPO.Value = 2) then // deb. auto
        BancodeDados.Emissoes.SQL.Add(' AND emissao_id = ' + Trim( BancodeDados.Retorno_OcorSEU_NUMERO.Value))
      else
        BancodeDados.Emissoes.SQL.Add(' AND nosso_numero = ' + QuotedStr( BancodeDados.Retorno_OcorNOSSO_NUMERO.AsString));
    ...
    begin
      Screen.Cursor := crDefault;
      if trim(BancodeDados.Retorno_OcorSEU_NUMERO.Value)<>'' then
      Mensagem('Não foi encontrada nenhuma emissão referente a este retorno.'#13+'Emissão: '+(BancodeDados.Retorno_OcorSEU_NUMERO.Value), mtInformation, [mbOk], mrOk, 0) else Mensagem('Não foi encontrada nenhuma emissão referente a este retorno.', mtInformation, [mbOk], mrOk, 0)
    ...
    begin
      ...
      try
        ...
        begin
          ...
          BancodeDados.Boletos.Open;
          VisualizaTitulosForm.Caption := '** ATENÇÃO: Lista dos prováveis títulos referentes ao retorno';
        end;
...
begin
if BancodeDados.Retorno_Ocor.Active then BancodeDados.Retorno_Ocor.Close;
end;
...
begin
  ...
  qryBancos.SQL.Add(' and BANCO_ID = '+IntToStr(BancodeDados.V_BoletosBANCO_ID.Value))
  else
  qryBancos.SQL.Add(' and codigo_banco>0 and coalesce(cont_encerrada,0)=0 and codigo_cedente <> ' + QuotedStr('')+' and agencia <> ' + QuotedStr('')+' order by banco_nome');
...
procedure TCsOcorrenciasRetForm.EditConsultaKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
...
begin
  if not BancodeDados.Retorno_Ocor.IsEmpty then begin
    ...
    lbPeriodo6.Caption    := 'Data do Arquivo: '+(Dt_Inicial.DateTimeStr)+' até '+(Dt_Final.DateTimeStr);
...
begin
  ...
  begin
    ...
    BancodeDados.V_BOLETOS.SQL.Text := 'select * from v_boletos where cod_banco = ' + IntToStr(BancodeDados.Retorno_OcorCODIGO_BANCO.Value);

    if (BancodeDados.qryBancoBAIX_BOL_PARAM.Value>0) then begin //OK

      if (BancodeDados.Retorno_OcorTIPO.Value <> 2) then begin //2
          BancodeDados.V_BOLETOS.SQL.Add(' AND nosso_numero = ' + QuotedStr(BancodeDados.Retorno_OcorNOSSO_NUMERO.Value));

        if trim(BancodeDados.Retorno_OcorSEU_NUMERO.Value)<>'' then
        BancodeDados.V_BOLETOS.SQL.Add(' AND emissao_id = ' + QuotedStr(BancodeDados.Retorno_OcorSEU_NUMERO.Value))
        else begin
          FormatSettings.DecimalSeparator:='.';
          if BancodeDados.Retorno_OcorDATA_VENCIMENTO.Value>0 then
          BancodeDados.V_BOLETOS.SQL.Add(' AND vencimento ='+QuotedStr(FormatDateTime('yyyy/mm/dd',BancodeDados.Retorno_OcorDATA_VENCIMENTO.Value)));

          if BancodeDados.Retorno_OcorVALOR_DOCUMENTO.Value>0 then
          BancodeDados.V_BOLETOS.SQL.Add(' AND valor_doc ='+CurrToStr(BancodeDados.Retorno_OcorVALOR_DOCUMENTO.Value));
         FormatSettings.DecimalSeparator:=',';
        end;
      end else //2
      BancodeDados.V_BOLETOS.SQL.Add(' AND emissao_id = ' + QuotedStr(BancodeDados.Retorno_OcorSEU_NUMERO.Value));
      ...
      BancodeDados.V_BOLETOS.SQL.Add(' AND  banco_id = ' + IntToStr(BancodeDados.Retorno_OcorBANCO_ID.Value));
