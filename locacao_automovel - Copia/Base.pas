...
uses
  ...
  IdTCPClient, IdExplicitTLSClientServerBase, IdMessageClient, IdSMTPBase, IdSMTP,
  IdMessage, IdText, IdAttachmentFile, IdGlobalProtocols, Winapi.ActiveX, RLRichFilter,
...
type
  ...
  //  function PegarID3(Generator: String): Integer;
   ...
   // function PegaID2(Generator: String): Integer;
    ...
    function PegaSeq_Nfse(configID: Integer): Integer;  //add por Denilson 08/09/2020

    function MontaMsgBoleto(Msg: string; GradeID: Integer; vl: currency; Desc_Exp_Dia: Integer): String;
    ...
    procedure PrepararRel(Relatorio: TRLReport; FMatricial: Boolean = false; RLDraftFilterTemp: TRLDraftFilter = nil; FMalaDireta: Boolean = false;
      Gauge: TGauge = nil);
    procedure FecharNota;
    procedure PegaNFItens;
    procedure CalculaTotalNfs;
    procedure MarcarAtivo(ListaTmp: TStringList; IDTmp: Integer; var Campo: TBooleanField);
    procedure CalcularAtivo(ListaTmp: TStringList; IDTmp: Integer; var Campo: TBooleanField);
   ...
   // function PegaDescricaoRetornoDebAuto(cod: integer): String;
    ...
    procedure conRecover( ASender, AInitiator: TObject; AException: Exception; var AAction: TFDPhysConnectionRecoverAction);
  end;
...
procedure TBancodeDados.conRecover( ASender, AInitiator: TObject; AException: Exception;
  var AAction: TFDPhysConnectionRecoverAction);
var
  res: Integer;
begin
  res := Mensagem('Sua conexão com o banco de dados foi perdida. Deseja conectar?', mtConfirmation, [mbYes, mbNo, mbCancel], mrYes, 0) ;
...
begin
  ...
  try
    ...
    on E : Exception do
      begin
        MessageDlg( 'Erro ao carregar o código: ' + #13 + #13 + (IntToStr(Proc1PegarID.Params[2].Value))+ #13 +
...
begin
 try
  ...
  begin
    ...
    begin
    if  TFDQuery(Components[i]).Active then TFDQuery(Components[i]).Close;
    end;
  end;
  ...
  begin
    ...
    begin
      if TFDStoredProc(Components[P]).Active then TFDStoredProc(Components[P]).Close;
    end;
  end;
  ...
  begin
    ...
    begin
      if TFDConnection(Components[F]).Connected then TFDConnection(Components[F]).Close;
    end;
  end;
...
begin
case cod of
00: Result := 'Débito efetuado';
01: Result := 'Débito não efetuado - Insuficiência de fundos';
02: Result := 'Débito não efetuado - Conta não cadastrada';
04: Result := 'Débito não efetuado - Outras restrições';
05: Result := 'Débito não efetuado – valor do débito excede valor limite aprovado.';
10: Result := 'Débito não efetuado - Agência em regime de encerramento';
12: Result := 'Débito não efetuado - Valor inválido';
13: Result := 'Débito não efetuado - Data de lançamento inválida';
14: Result := 'Débito não efetuado - Agência inválida';
15: Result := 'Débito não efetuado - conta inválida';
18: Result := 'Débito não efetuado - Data do débito anterior à do processamento';
30: Result := 'Débito não efetuado - Sem contrato de débito automático';
31: Result := 'Débito efetuado em data diferente da data informada – feriado na praça de débito';
96: Result := 'Manutenção do Cadastro';
...
begin
  ...
  begin
    if (arq <> '') then
    begin
      ShellExecute(Application.Handle, 'open', pchar('scan.exe'), PChar(AnsiQuotedStr(arq, '"') + ' -n'), nil, SW_HIDE);
    end
    else
    begin
      ...
      if dlg.Execute then
      begin
        ShellExecute(Application.Handle, 'open', pchar('scan.exe'), PChar(AnsiQuotedStr(dlg.FileName, '"') + ' -n'), nil, SW_HIDE);
      end;
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    ShellExecute(Application.Handle, nil, pchar(DiretorioTemp + SDocNOME_ARQ.AsString), nil, nil, SW_SHOWMAXIMIZED);
  end;
end;


procedure TBancodeDados.PegaDocScan(iTipo, idPessoa, origID : integer; origNome : string);
begin
  ///BancodeDados.FDConnection3.StartTransaction;
  SDoc.Close;
  SDoc.SQL.Text := 'select * from s_doc where tipo = ' + IntToStr(iTipo) + ' and ID = ' + IntToStr(idPessoa);
...
begin
  ...
  try
    ...
    QryID.Open;
    Result := QryID.FieldByName('GEN_ID').AsInteger; //QryID.Fields[0].AsInteger;
  finally
    QryID.Close;
  end;
end;
...
begin
  ...
  if RetornaNumeros(ClientesFONE1.Value) <> '' then
    ClientesCelula_Novo_1.AsString := ClientesFONE1.Value + ' ' + BancodeDados.qryOper_CelularDESCRICAO.value;
  ...
  if RetornaNumeros(ClientesFONE2.Value) <> '' then
    ClientesCelula_Novo_2.AsString := ClientesFONE2.Value + ' ' + BancodeDados.qryOper_CelularDESCRICAO.Value;
  BancodeDados.qryOper_Celular.Close;


if (ClientesCONFIAVEL.Value=1) and (ClientesSIT.Value = 1) then ClientesConfiavelNome.Value:='SIM' else ClientesConfiavelNome.Value:='NÃO';
  ...
  begin
    ListaTmp := '';
    qryServ.Close;
    qryServ.SQL.Text := 'select veiculo_id, cli_id from cont_serv_clien where cli_id = ' + IntToStr(ClientesCLI_ID.Value);
...
procedure TBancodeDados.IdSMTP1Work(ASender: TObject; AWorkMode: TWorkMode; AWorkCount: Int64);
begin
  if Assigned(ProgressoEmailForm) then
  begin
    ProgressoEmailForm.Show;
    ProgressoEmailForm.lbTamanho.Caption := FormatFloat(',0', AWorkCount / 1024) + ' / ' + FormatFloat(',0', emailTamMax / 1024) + ' KB.';
    ProgressoEmailForm.Gauge1.Progress := AWorkCount;
  end;
  Application.ProcessMessages;
end;

procedure TBancodeDados.IdSMTP1WorkBegin(ASender: TObject; AWorkMode: TWorkMode; AWorkCountMax: Int64);
begin
  if Assigned(ProgressoEmailForm) then
  begin
...
begin
  ...
  begin
    ...
    begin
      ...
      try
        vlMulta := StrToCurr(FormatFloat('0.00', (BoletosVALOR_INTEGRAL.Value * BoletosGradeMulta.Value) / 100));
      except
        vlMulta := StrToCurr(FormatFloat('0.00', (BoletosVALOR_INTEGRAL.Value * BoletosGradeMulta.Value) / 100));
      end;

      try
        vlJuros := StrToCurr(FormatFloat('0.00', (BoletosVALOR_INTEGRAL.AsCurrency * BoletosGradeJuros.AsCurrency) / 100));
      except
        vlJuros := StrToCurr(FormatFloat('0.00', (BoletosVALOR_INTEGRAL.AsCurrency * BoletosGradeJuros.AsCurrency) / 100));
      end;
  ...
  if BoletosSITUACAO_BOLETO.Value = 'CANCELADO' then
    ...
    BoletosvlFaltando.Value := BoletosvlAPagarCalc.Value - BoletosVALOR_PAGO.Value;
end;
...
begin
  ...
  LigacoesCLI_ID.Value      := ClientesCLI_ID.Value else LigacoesCLI_ID.Value  := 0
end;

procedure TBancodeDados.LigacoesBeforePost(DataSet: TDataSet);
begin
if (BancodeDados.LigacoesCLI_ID.Value = 0) or (BancodeDados.LigacoesCLI_ID.IsNull)  then
  abort;
...
if LigacoesCLI_ID.Value> 0 then begin
  qryClientes.Close;
...
if LigacoesSIT.Value=1 then LigacoesSituacao_Nome.Value:='PENDENTE';
if LigacoesSIT.Value=2 then LigacoesSituacao_Nome.Value:='RESOLVIDO';
if LigacoesSIT.Value=3 then LigacoesSituacao_Nome.Value:='PARCIAL';
if LigacoesSIT.Value=4 then LigacoesSituacao_Nome.Value:='NÃO ATENDIDO';

if LigacoesTIPO.Value=1 then LigacoesTipo_Nome.Value:='SUPORTE LIGOU' else
if LigacoesTIPO.Value=2 then LigacoesTipo_Nome.Value:='CLIENTE LIGOU' else
if LigacoesTIPO.Value=3 then LigacoesTipo_Nome.Value:='WHATSAPP' else
if LigacoesTIPO.Value=4 then LigacoesTipo_Nome.Value:='SMS' else
if LigacoesTIPO.Value=5 then LigacoesTipo_Nome.Value:='VISITA' else
if LigacoesTIPO.Value=6 then LigacoesTipo_Nome.Value:='COBRANÇA' else
if LigacoesTIPO.Value=7 then LigacoesTipo_Nome.Value:='OUTRO';
...
begin
  ...
  Livro_CaixaDESCRICAO_PAGO.Value := 'Dinheiro';
...
begin
  if Livro_CaixaORIGEM_ID.Value > 0 then
    Abort;
  Adiciona.SQL.Text := 'delete from livro_caixa where origem_id = ' + IntToStr(Livro_CaixaCAIXA_ID.Value);
  Adiciona.ExecSQL;
end;
...
begin
  if not(BaixandoAutom = true) then
    if (Livro_CaixaBOLETO_ID.Value > 0) { or (Livro_CaixaPAGAR_ID.Value>0) } or (Livro_CaixaCHEQUE_ID.Value > 0) or (Livro_CaixaNUMERO_ARQUIVO.Value > 0) or
      (Livro_CaixaORIGEM_ID.Value > 0) then
      Abort;
end;
...
begin
  if Trim(LoginSENHA.Value) <> '' then
    if DecifrarSed(LoginSENHA.AsString, False, LoginLOGIN_ID.Value * 2233) = '' then
    begin
      LoginSENHA.AsString := CifrarSed(LoginSENHA.AsString, LoginLOGIN_ID.Value * 2233);
    end;
end;
...
if not BancodeDados.Login.Active then BancodeDados.Login.Open();
  if BancodeDados.LoginGERA_NFSE.Value>0 then BancodeDados.LoginNfseGerar.AsString:='SIM' else BancodeDados.LoginNfseGerar.AsString:='NÃO';
...
Emissa2ID := BoletosEMISSAO_ID.Value;
  ...
  BoletosVALOR_APAGAR.Value := (BoletosVALOR_MENSAL.Value + BoletosVALOR_LIC.Value + BoletosACRESCIMO.Value) - BoletosDESCONTO.Value;
  if (BoletosSITUACAO_BOLETO.Value = 'A RECEBER') or (BoletosSITUACAO_BOLETO.Value = 'CANCELADO') then
  begin
    Adiciona.SQL.Text := 'delete from livro_caixa where boleto_id=' + IntToStr(BoletosBOLETO_ID.Value);
    Adiciona.ExecSQL;
  end;
  ...
  begin  //boletos emitidos: atualizando o campo 'data de recebimento' e a 'situacao do boleto', tirando os campos calculados
    if (BoletosSITUACAO_BOLETO.NewValue = 'RECEBIDO') then
      BancodeDados.Adiciona.SQL.Text := 'update boletos_emissao set data_recebimento='+QuotedStr(FormatDateTime('yyyy/mm/dd',BoletosRECEBIMENTO.NewValue))+', situacao_boleto ='+QuotedStr(BoletosSITUACAO_BOLETO.Value)+' where emissao_id in ('+ IntToStr(Emissa2ID) +')'
    else
    if (BoletosSITUACAO_BOLETO.OldValue = 'RECEBIDO')  then
      BancodeDados.Adiciona.SQL.Text := 'update boletos_emissao set data_recebimento=null, situacao_boleto ='+QuotedStr(BoletosSITUACAO_BOLETO.Value)+' where emissao_id in ('+ IntToStr(Emissa2ID) +')';
...
begin
  ...
  BancodeDados.qryRetorn1.Params[0].Value := BancodeDados.V_BoletosBANCO_ID.Value;
  BancodeDados.qryRetorn1.Params[1].Value := BancodeDados.V_BoletosNOSSO_NUMERO.Value;
  BancodeDados.qryRetorn1.Open;
  BancodeDados.V_BoletosValor_Rec_Boleto.Value := BancodeDados.qryRetorn1VALOR_PAGO.Value;


    if V_BoletosREMESSA.Value > 0 then begin
       V_BoletosRemessaGerada.Value := 'SIM';
         if (V_BoletosCOD_MOV.Value=2) then
           V_BoletosRemessaSit.Value:='Entrada Confirmada' else
           V_BoletosRemessaSit.Value:='Aguardando Confirmação'

    end else begin
      V_BoletosRemessaSit.Value    :='Não enviada';
      V_BoletosRemessaGerada.Value :='NÃO';
    end;
  ...
  if (V_BoletosCOD_BANCO.Value = 104) then
  begin
  ...
  try
    if (V_BoletosDIG_AGENCIA.Value <> '') then
      agTmp := FormatFloat('0000', StrToInt64(V_BoletosAGENCIA.Value)) + '-' + V_BoletosDIG_AGENCIA.Value
    else
  ...
  try
    if (V_BoletosDIG_COD_CEDENTE.Value <> '') then
      codCedTmp := (V_BoletosCOD_CEDENTE.Value) + '-' + (V_BoletosDIG_COD_CEDENTE.Value)  //codCedTmp := FormatFloat('00000000', StrToInt64(V_BoletosCOD_CEDENTE.Value)) + '-' + V_BoletosDIG_COD_CEDENTE.Value
    else
  ...
  V_BoletosCodIdentBanco.Value := FormatFloat('000', V_BoletosCOD_BANCO.Value) + '-' + V_BoletosDIG_BANCO.AsString;
  V_BoletosSacado.Value := FormatFloat('000000', V_BoletosCLI_ID.Value) + ' / ' + V_BoletosCLI_NOME.Value + ' - CPF/CNPJ: ' + V_BoletosCLI_CNPJ_CPF.Value;
  V_BoletosSacado2.Value := FormatFloat('000000', V_BoletosCLI_ID.Value) + ' / ' + V_BoletosCLI_ABREV.Value + ' - CPF/CNPJ: ' + V_BoletosCLI_CNPJ_CPF.Value;
  V_BoletosEndereco.Value := V_BoletosLOGRADOURO.Value + ' - ' + V_BoletosBAIRRO.Value + ' - ' + V_BoletosCIDADE.Value + ' - ' + V_BoletosESTADO.Value;
end;

function TBancodeDados.MontaMsgBoleto(Msg: string; GradeID: Integer; vl: currency; Desc_Exp_Dia: Integer): String;
var
  vlMulta, vlJuros: currency;
  DtVenc, DTJurosMulta, DtDesconto : TDate;
begin
  if not GradeFin.Active then GradeFin.Open;
  ...
  begin
    Result := Result + #13 + 'Sr(a). caixa, receber o valor de ' + FormatFloat('R$ ,0.00', EmissoesVALOR_DOC.Value - EmissoesDESCONTO.Value);

    DtDesconto:= MontaData(  DtVenc, Desc_Exp_Dia);
    if DtDesconto > DtVenc then DtDesconto:=DtVenc;

    if (Desc_Exp_Dia > 0) then
      Result := Result + ' se o pagamento for efetuado até o dia ' + FormatDateTime('dd/mm/yyyy', DtDesconto ) + '.'

    else
      Result := Result + ' se o pagamento for efetuado até '+ FormatDateTime('dd/mm/yyyy', DtVenc);
  end;  //infDesc = true  //18


  Result := Result + #13 + #13 + 'MULTA DE ' + FormatFloat('R$ ,0.00', vlMulta) + ' APÓS: ' + FormatDateTime('dd/mm/yyyy', DTJurosMulta) + '  -  JUROS DE ' +
    FormatFloat('R$ ,0.00', vlJuros) + ' AO DIA.';



  if EmissoesDIAS_TOLERANCIA.Value > 0 then
    Result := Result + #13 + #13 + 'COBRAR JUROS E MULTA APÓS: ' + FormatDateTime('dd/mm/yyyy', DTJurosMulta)
  else
    Result := Result + #13 + #13 + 'COBRAR JUROS E MULTA APÓS O VENCIMENTO.';
...
procedure TBancodeDados.MudaOrdem(Tabela: TFDQuery; Ordem: TField; Ordem2: String = '');
var
...
begin
  Adiciona.SQL.Text := 'delete from livro_caixa where pagar_id = ' + IntToStr(PagarPAGAR_ID.Value);
  Adiciona.ExecSQL;
end;
...
begin
  ...
  begin //Pedido de Baixa de boletos registrados
   ...
   TipoOcorrencia :=2;
     ...
     if (QtdDias < 0) then QtdDias := 0;

    QrySql.close;
    QrySql.SQL.Text:='select remessa, cod_mov from boletos_emissao where emissao_id = ' + IntToStr(BoletosEMISSAO_ID.Value);
    QrySql.Open;

      if (BancodeDados.QrySql.FieldByName('REMESSA').AsInteger>0) and (QtdDias <= 29) and (BancodeDados.QrySql.FieldByName('COD_MOV').AsInteger=2) then begin //caso tenha gerado a remessa, deve gerar um outro arq. remessa com o codigo de movi

            Remessa_Ocorrencia.Close;
            Remessa_Ocorrencia.SQL.Text:='select * from remessa_ocorrencia where emissao_id = '+ IntToStr(BoletosEMISSAO_ID.Value)+' and cod_mov_rem = '+IntToStr(TipoOcorrencia); //+' and TIPO_OCORRENCIA = '+QuotedStr('Pedido de baixa');
            Remessa_Ocorrencia.Open;

            if Remessa_Ocorrencia.IsEmpty then Remessa_Ocorrencia.Append else Remessa_Ocorrencia.Edit;
          ...
          BancodeDados.Adiciona.SQL.Text := 'update boletos_emissao set remessa = null, dt_proces='+QuotedStr(FormatDateTime('yyyy/mm/dd',PrincipalForm.DataServidor))+' where EMISSAO_ID = ' + IntToStr(BoletosEMISSAO_ID.Value);
          BancodeDados.Adiciona.ExecSQL;

          BancodeDados.Adiciona.SQL.Text := 'update boletos set emissao_id = null where emissao_id = ' + IntToStr(BoletosEMISSAO_ID.Value);
          BancodeDados.Adiciona.ExecSQL;

      end else
      begin
        BancodeDados.Adiciona.SQL.Text := 'update boletos_emissao set cancelado=1 where emissao_id = ' + IntToStr(BoletosEMISSAO_ID.Value);
        BancodeDados.Adiciona.ExecSQL;
  ...
  Adiciona.SQL.Text := 'update livro_caixa set cancelado = 1 where boleto_id = ' + IntToStr(BoletosBOLETO_ID.Value);
  Adiciona.ExecSQL;
...
try
  QrySql.Close;
  QrySql.SQL.Text:='select dias_tolerancia from boletos_emissao where emissao_id = '+QuotedStr(RetornoSEU_NUMERO.Value);
  QrySql.Open();
  ...
  RetornoDescAbat.Value := RetornoVALOR_DESCONTO.Value + RetornoVALOR_ABATIMENTO.Value;
  ...
  begin
    RetornoRetornoDebAuto.Value := PegaDescricaoRetornoDebAuto(RetornoCOB_DEB.Value);
    RetornoTipoQuitacao.Value   := 'Débito Automático';
  end else RetornoTipoQuitacao.Value := 'Retorno Cobrança';
...
begin
  ...
  qryPrt_Boleto.Close;
  qryPrt_Boleto.Params[0].Value := (BancodeDados.Cont_Prt_BoletoPRT_BLT_ID.Value);
  qryPrt_Boleto.Open;
  BancodeDados.Cont_Prt_BoletoTipo_Protesto.Value := qryPrt_BoletoPROTESTAR.Value;
...
begin
  ...
  case Cont_Serv_ClienSIT.Value of
    1 ://1: locado
    begin
      Adiciona.SQL.Text:='update veiculos set c_s_c_id='+IntToStr(Cont_Serv_ClienC_S_C_ID.Value)+' where veiculo_id = '+IntToStr(Cont_Serv_ClienVEICULO_ID.Value);
      Adiciona.ExecSQL;
    end
    else
    begin //2: Cancelado, 3: devolvido
      Adiciona.SQL.Text:='update veiculos set c_s_c_id=0 where veiculo_id = '+IntToStr(Cont_Serv_ClienVEICULO_ID.Value);
      Adiciona.ExecSQL;
    end;
...
begin
  ...
  begin
    Mensagem('Informe um nome válido para o documento.', mtInformation, [mbOk], mrOk, 0);
    Abort;
  end;
end;
...
begin
  ...
  begin
      try
        ...
        ServidorWeb.Params.Add('server=db3044.mysql51.laniway.com.br'); //IP do servidor do banco de dados
...
//if ID>0 then begin
  ...
  BancodeDados.qryEstados2.Params[0].Value := Trim(BancodeDados.ConfigPAIS.Value);
...
begin
  ...
  BancodeDados.Municipios3.Params[0].Value := BancodeDados.ConfigUF_NATURAL.Value;
    ...
    if Boletos.Active then
      ...
      begin
        ...
        try
          ...
          begin
            ...
            begin
              ...
              if (BoletosSITUACAO_BOLETO.Value = 'A RECEBER') or (BoletosSITUACAO_BOLETO.Value = 'CRED A RECEBER') then
              begin
                { if not (BoletosTIPO_CONTAS_ID.Value>0) then begin
                  Result:=False;
                  RecBol.Close;
                  Mensagem('Há um ou mais títulos sem tipo de conta definido.', mtWarning, [mbOk], mrOk, 0);
                  break;
                  end;
                  ...
                  Result:=False;
                  RecBol.Close;
                  Mensagem('Só é possível utilizar títulos referentes ao mesmo ano.', mtWarning, [mbOk], mrOk, 0);
                  break;
                  end;
                  end; }
        ...
        begin
          Result := False;
          RecBol.Close;
          Mensagem('Não foi encontrato nenhum título a receber.', mtInformation, [mbOk], mrOk, 0);
        end;
...
begin
  ...
  begin
    Abort;
    Mensagem('Cliente não informado. Operação cancelada.', mtInformation, [mbOk], mrOk, 0);
  end;

  if Trim(SerialSERIAL.AsString) = '' then
  begin
    Abort;
    Mensagem('Chave serial inválida. Operação cancelada.', mtInformation, [mbOk], mrOk, 0);
  end;
...
var
  iRes: Integer;
begin
  iRes := MessageDlg('A conexão está perdida. Off-line: sim, tente novamente: ok, falhe: cancele.',
...
procedure TBancodeDados.PrepararRel(Relatorio: TRLReport; FMatricial: Boolean = false; RLDraftFilterTemp: TRLDraftFilter = nil; FMalaDireta: Boolean = false;
  Gauge: TGauge = nil);
var
  i { , TamPapel } : Integer;
...
if not config.active then config.open;
  ...
  begin
    try
      ...
      begin
        ...
        if (Pos(UpperCase('LbNomeInstituicao'), UpperCase(ComponenteTmp.Name))) > 0 then
        begin
        ...
        else if (Pos(UpperCase('lbCNPJ'), UpperCase(ComponenteTmp.Name))) > 0 then
        begin
        ...
        else if (Pos(UpperCase('LbEndereco'), UpperCase(ComponenteTmp.Name))) > 0 then
        begin
          (ComponenteTmp as TRLLabel).Caption := 'End.: ' + BancodeDados.configLOGRADOURO.Value + ', Telefone: ' + BancodeDados.configFONE1.Value +
            ' - ' + BancodeDados.configFONE2.Value + ' ' + BancodeDados.configBAIRRO.Value + ' - CEP: ' + BancodeDados.configCEP.Value + ', ' +
        ...
        else if (Pos(UpperCase('LbSite'), UpperCase(ComponenteTmp.Name))) > 0 then
        begin
          (ComponenteTmp as TRLLabel).Caption := '';
          if trim(BancodeDados.configEMAIL_COMERC.Value) <> '' then
            (ComponenteTmp as TRLLabel).Caption := 'E-mail: ' + BancodeDados.configEMAIL_COMERC.Value;
          if trim(BancodeDados.configSITE.Value) <> '' then
            (ComponenteTmp as TRLLabel).Caption := (ComponenteTmp as TRLLabel).Caption + ' - Site: ' + BancodeDados.configSITE.Value;
          if (FMatricial = true) then
            (ComponenteTmp as TRLLabel).Font.Color := clBlack
          else
            (ComponenteTmp as TRLLabel).Font.Color := CorPadraoTXT; // RLDraw
        end
        else if (Pos(UpperCase('lbCorPadrao'), UpperCase(ComponenteTmp.Name))) > 0 then
        begin
          if (FMatricial = true) then
        ...
        else if (Pos(UpperCase('lbPeriodo'), UpperCase(ComponenteTmp.Name))) > 0 then
        begin
        end
        else if (Pos(UpperCase('lbAnoLetivo'), UpperCase(ComponenteTmp.Name))) > 0 then
        begin
        end
        else if (Pos(UpperCase('lbTitulo'), UpperCase(ComponenteTmp.Name))) > 0 then
        begin
          if not(FMatricial = true) then
            (ComponenteTmp as TRLLabel).Font.Color := CorPadraoTXT;
        end
        else if (Pos(UpperCase('lbEmpresaNome'), UpperCase(ComponenteTmp.Name))) > 0 then
        begin
          (ComponenteTmp as TRLLabel).Caption := EmpresaNome;
        end
        else if (Pos(UpperCase('lbEmpresaSite'), UpperCase(ComponenteTmp.Name))) > 0 then
        begin
          (ComponenteTmp as TRLLabel).Caption := EmpresaSite;
        end
        else if (Pos(UpperCase('lbSistemaNome'), UpperCase(ComponenteTmp.Name))) > 0 then
        begin
          (ComponenteTmp as TRLLabel).Caption := SistemaNome;
        end
        else if (Pos(UpperCase('linhamatricial'), UpperCase(ComponenteTmp.Name))) > 0 then
        begin
          if (FMatricial = true) then
          ...
          { if (Pos(UpperCase('LbResol'), UpperCase(ComponenteTmp.Name))) > 0 then
            begin
            (ComponenteTmp as TRLLabel).Caption := 'Resolução Nº : ' + BancodeDados.InstituicaoRESOLUCAO.Value;
            end
            else } if (Pos(UpperCase('LbResolCNPJ'), UpperCase(ComponenteTmp.Name))) > 0 then
          begin
            (ComponenteTmp as TRLLabel).Caption := 'CNPJ: ' + BancodeDados.configCNPJ.Value;
          end
          else if (Pos(UpperCase('lblogradouro'), UpperCase(ComponenteTmp.Name))) > 0 then
          begin
            (ComponenteTmp as TRLLabel).Caption := 'Endereço: ' + BancodeDados.configLOGRADOURO.Value + ' - ' + BancodeDados.configBAIRRO.Value + ', '
              + BancodeDados.configCIDADE.Value + ', ' + BancodeDados.configUF.Value + '.';
          end
          else if (Pos(UpperCase('LbCidadeData'), UpperCase(ComponenteTmp.Name))) > 0 then
          begin
            (ComponenteTmp as TRLLabel).Caption := BancodeDados.ConfigCIDADE_NOME.Value + ', ' + PegaUF(BancodeDados.configUF.Value) + ' - ' +
              FormatDateTime('dd/mm/yyyy', Date);
          end
          else if (Pos(UpperCase('LbTelefEmail'), UpperCase(ComponenteTmp.Name))) > 0 then
          begin
            if (trim(BancodeDados.configEMAIL_COMERC.Value) <> '') then
              (ComponenteTmp as TRLLabel).Caption := 'Telefone: ' + BancodeDados.configFONE1.Value + ' - E-mail: ' + BancodeDados.configEMAIL_COMERC.Value
            else
              (ComponenteTmp as TRLLabel).Caption := 'Telefone: ' + BancodeDados.configFONE1.Value;
          end
          else if (Pos(UpperCase('lbNomeSite'), UpperCase(ComponenteTmp.Name))) > 0 then
          begin
            if (trim(BancodeDados.configSITE.Value) <> '') then
              (ComponenteTmp as TRLLabel).Caption := 'Site: ' + BancodeDados.configSITE.Value
            else
              (ComponenteTmp as TRLLabel).Caption := '';
          end;
      ...
      begin
        if (Pos(UpperCase('lbCorPadrao'), UpperCase(ComponenteTmp.Name))) > 0 then
        begin
          if (FMatricial = true) then
  ...
  begin
    PrincipalForm.GeraMalaDireta(TFDQuery(Relatorio.DataSource.DataSet), true, Gauge);
  end
...
begin
  ...
  NfsSIT.Value := 0;
  NfsTIPO.Value := 1; // 0 - Talão; 1 - Eletronica
  NfsSERIE.Value := '1';
...
begin
  ...
  begin
    NfsServ.Close;
    Adiciona.SQL.Text := 'DELETE FROM NFS_SERV WHERE NFS_ID = ' + IntToStr(NfsNFS_ID.Value);
    Adiciona.ExecSQL;
  end;
end;
...
begin
  ...
  begin
    Mensagem('Informe o cliente desta nota fiscal.', mtInformation, [mbOk], mrOk, 0);
    Abort;
  end;
  ...
  begin
    ...
    try
      ...
      try
        ...
        NfsVL_ISS.Value := StrToCurr(FormatFloat('0.00', (vlTotal * NfsPERC_ISS.Value) / 100));
        ...
        On E: Exception do
        begin
          Mensagem('Erro ao calcular o total da nota.' + #13 + E.Message, mtWarning, [mbOk], mrOk, 0);
        end;
      end;
    finally
...
begin
if not qryOperadora.active then qryOperadora.Open
end;
...
begin
  ...
  begin
    Mensagem('Informe a quantidade do serviço.', mtInformation, [mbOk], mrOk, 0);
    Abort;
  end;
  if not(NfsServVL_UNIT.Value > 0) then
  begin
    Mensagem('Informe o valor unitário do serviço.', mtInformation, [mbOk], mrOk, 0);
    Abort;
  end;

  NfsServVL_TOTAL.Value := StrToCurr(FormatFloat('0.00', NfsServQUANT.Value * NfsServVL_UNIT.Value));
...
begin
  ...
  try
    ...
    begin
      ...
      if (nfse.Certificado.SelecionaCertificadoNroSerie(Trim(ConfigNFSE_CERT_DIG_NUM_SERIE.Value)+';'+RetornaNumeros(ConfigCIDADE.AsString)+';'))  then
      begin
       // Sleep(4000);
         Application.ProcessMessages;
         if not Config.Active then Config.Open;
...
begin
  ...
  begin
    ...
    if oExc.Kind = ekRecordLocked then
      oExc.Message := 'Por favor, tente a operação mais tarde. No momento, o registro está ocupado'
    else if (oExc.Kind = ekUKViolated) and SameText(oExc[0].ObjName, 'UniqueKey_Orders') then
      oExc.Message := 'Forneça as informações solicitadas. Parece que o registro já foi feito';
  end;
...
begin
  // if (BancodeDados.NfsSIT.Value = 1) then Mensagem('Esta nota já está fechada.', mtInformation, [mbOk], mrOk, 0) else
  // if (BancodeDados.NfsSIT.Value = 2) then Mensagem('Esta nota está cancelada.', mtInformation, [mbOk], mrOk, 0) else
  if (BancodeDados.NfsSIT.Value <> 0) then
    Mensagem('Esta nota não está aberta.', mtInformation, [mbOk], mrOk, 0)
  else
  begin
    if (Trim(BancodeDados.NfsCNPJ.Value) = '') then
    begin
      Mensagem('O cliente não possui CNPJ.', mtWarning, [mbOk], mrOk, 0);
      exit;
    end;
...
begin
  ...
  begin
    ...
    begin
      PrincipalForm.Memo1.Lines.Add(qryNFSServDESCRICAO.Value + ' - ' + FormatFloat('R$ ,0.00', qryNFSServVL_TOTAL.Value));
      qryNFSServ.Next;
    end;

    Boletos.Close;
    Boletos.SQL.Text := PrincipalForm.SQLrepositorio(0) + ' and b.nfs_id = ' + IntToStr(idNota) + ' order by b.recebimento';
    Boletos.Open;

    PrincipalForm.Memo1.Lines.Add('');
    PrincipalForm.Memo1.Lines.Add('OBS.: NF REFERENTE AO(S) SEGUINTE(S) TITULO(S)');

    Boletos.First;
    while not Boletos.Eof do
    begin
      PrincipalForm.Memo1.Lines.Add('COD.: ' + FormatFloat('000000', BoletosBOLETO_ID.Value) + ' - ' + 'Vencimento: ' + FormatDateTime('dd/mm/yyyy',
        BoletosVENCIMENTO_ORIG.Value));
      Boletos.Next;
    end;
...
begin
  ...
  try
    if (NfsNFS_ID.Value > 0) then
    begin
      NfsServ.SQL.Text := 'select * from nfs_serv where nfs_id = ' + IntToStr(NfsNFS_ID.Value) + ' order by nfs_it_id';
      NfsServ.Open;
    end;
  except
    On E: Exception do
...
procedure TBancodeDados.MarcarAtivo(ListaTmp: TStringList; IDTmp: Integer; var Campo: TBooleanField);
var
  idxTmp: Integer;
begin
...
procedure TBancodeDados.CalcularAtivo(ListaTmp: TStringList; IDTmp: Integer; var Campo: TBooleanField);
begin
  if Assigned(ListaTmp) then
  begin
...
const
  ...
  SemAcentuacao: array [1 .. 58] of string = ('&agrave;', '&aacute;', '&acirc;', '&atilde;', '&auml;', '&egrave;', '&eacute;', '&ecirc;', '&euml;', '&igrave;',
    '&iacute;', '&icirc;', '&iuml;', '&ograve;', '&oacute;', '&ocirc;', '&otilde;', '&ouml;', '&ugrave;', '&uacute;', '&ucirc;', '&uuml;', '&ccedil;',
    '&Agrave;', '&Aacute;', '&Acirc;', '&Atilde;', '&Auml;', '&Egrave;', '&Eacute;', '&Ecirc;', '&Euml;', '&Igrave;', '&Iacute;', '&Icirc;', '&Iuml;',
    '&Ograve;', '&Oacute;', '&Ocirc;', '&Otilde;', '&Ouml;', '&Ugrave;', '&Uacute;', '&Ucirc;', '&Uuml;', '&Ccedil;', '&acute;', '&uml;', '&brvbar;', '&sect;',
    '&euro;', '&not;', '&pound;', '&sup1;', '&sup2;', '&sup3;', '&ordm;', '&ordf;');
var
  i: Integer;
begin
...
function TBancodeDados.EnviarEmail(Destinatario, Assunto, Corpo, Anexo: String): Boolean;
var
  pass, arq2: String;
...
begin
  ...
  try
    ...
    begin
      if (Trim(ConfigEMAIL_SERV_SMTP.Value) <> '') and (ConfigEMAIL_PORTA_SMTP.Value > 0) then
      begin
        ...
        begin
          ...
          Mensagem('Configuração de email incompleta. Informe o usuário e a senha.', mtWarning, [mbOk], mrOk, 0);
      end
      else
        Mensagem('Configuração de email incompleta. Informe o servidor SMTP e a porta de conexão.', mtWarning, [mbOk], mrOk, 0);
    end
    else
      Mensagem('Configuração de email incompleta. Informe o nome do remetente.', mtWarning, [mbOk], mrOk, 0);
  except
    On E: Exception do
    begin
      ...
      if (Pos(UpperCase('Credentials Rejected'), UpperCase(E.Message)) > 0) then
        Mensagem('Usuário ou senha inválidos.', mtWarning, [mbOk], mrOk, 0)

      else if (Pos(UpperCase('Username and Password not accepted'), UpperCase(E.Message)) > 0) then
        Mensagem('Usuário ou senha inválidos.', mtWarning, [mbOk], mrOk, 0)

      else if (Pos(UpperCase('Host not found'), UpperCase(E.Message)) > 0) then
        Mensagem('Servidor SMTP não encontrado.', mtWarning, [mbOk], mrOk, 0)

      else if (Pos(UpperCase('Socket Error # 11004'), UpperCase(E.Message)) > 0) then
        Mensagem('Servidor SMTP não encontrado.', mtWarning, [mbOk], mrOk, 0)

      else if (Pos(UpperCase('Connection timed out'), UpperCase(E.Message)) > 0) then
        Mensagem('O servidor demorou a responder, verifique se a porta para conexão está correta.', mtWarning, [mbOk], mrOk, 0)

      else if (Pos(UpperCase('read timeout'), UpperCase(E.Message)) > 0) then
        Mensagem('O servidor demorou a responder, talvez o mesmo requeira uma Conexão SSL segura.', mtWarning, [mbOk], mrOk, 0)

      else if (Pos(UpperCase('connection closed'), UpperCase(E.Message)) > 0) then
        Mensagem('A conexão foi encerrada pelo servidor.', mtWarning, [mbOk], mrOk, 0)

      else
        Mensagem('Erro ao enviar email!' + #13#13 + E.Message, mtWarning, [mbOk], mrOk, 0);
    end;
  end;
  if (compactarAnexo = true) then
  begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        DstLine[x].rgbtBlue := (SrcLine1[t3].rgbtBlue * w1 + SrcLine1[t3 + 1].rgbtBlue * w2 + SrcLine2[t3].rgbtBlue * w3 + SrcLine2[t3 + 1].rgbtBlue *
          w4) shr 16;
        Inc(xP, xP2);
      end;
      Inc(yP, yP2);
...
function TBancodeDados.ResizeImage(FileName: string; MaxWidth: Integer): Boolean;
var
  OldBitmap: TBitmap;
  NewBitmap: TBitmap;
  // aWidth: Integer;
...
begin
  ...
  try
      ...
      try
        ...
        begin
          ...
          try
            ...
            Result := SaveJPEGPictureFile(NewBitmap, DiretorioTemp, ExtractFileName(FileName), MaxWidth * 2);
    ...
    begin
      CopyFile(PChar(FileName), PChar(DiretorioTemp + ExtractFileName(FileName)), False);
      Result := true;
    end;
  except
    On E: Exception do
...
procedure TBancodeDados.GravarBlobNaTabela(Campo: TBlobField; CaminhoArquivo: String);
var
...
begin
  ...
  begin
    ...
    try
      ...
      On E: Exception do
      begin
        Mensagem('Ocorreu um erro inesperado ao gravar o campo blob.' + #13 + #13 + E.Message, mtWarning, [mbOk], mrOk, 0);
      end;
    end;
  end
  else
    Mensagem('Arquivo não localizado.', mtInformation, [mbOk], mrOk, 0);
end;
...
begin
  ...
  try
      ...
      On E: Exception do
      begin
        if (Pos('Classe não registrada', E.Message) > 0) then
        begin
          // Mensagem('Classe não registrada. É necessário instalar o .NET 4 e registrar o arquivo sednfse.dll', mtWarning, [mbOk], mrOk, 0)
          if FileExists('C:\Windows\Microsoft.NET\Framework\v4.0.30319\regasm.exe') then
          begin
            WinExec(PAnsiChar(AnsiString('C:\Windows\Microsoft.NET\Framework\v4.0.30319\regasm.exe "' + ExtractFilePath(ParamStr(0)) + 'sednfse.dll"')),
              SW_SHOWNORMAL);
          end;
          nfse := CoSedNFSe_.Create;
          Result := true;
...
var
  ...
  begin
    if (nfse.NotaFiscal.NumeroRps > 0) then
      try
        dirTmp := ExtractFilePath(ParamStr(0)) + 'nfse\' +FormatFloat('0000000',NfsNFS_ID.Value) + FormatDateTime('yyyy-mm', Date) + '\rps\';
        if not DirectoryExists(dirTmp) then
          ForceDirectories(dirTmp);
        nfse.SalvarXML_RPS(dirTmp + 'loterps_' + FormatFloat('000000', nfse.NotaFiscal.NumeroRps) + '.xml');
      except
        On E: Exception do
          TraduzErro(E.Message);
      end;
  ...
  begin
    ...
    try
      dirTmp := ExtractFilePath(ParamStr(0)) + 'nfse\' +FormatFloat('0000000',NfsNFS_ID.Value) + FormatDateTime('yyyy-mm', Date) + '\nfse\';

      if not DirectoryExists(dirTmp) then
        ForceDirectories(dirTmp);
      nfse.SalvarXML_NFSe(dirTmp +'nfse\'+ 'nfse_' + FormatFloat('00000000', nfse.NFSeNumero) + '_loterps_' + FormatFloat('000000', nfse.NFSeIDLote) + '.xml');
    except
      On E: Exception do
        TraduzErro(E.Message);
    end;
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                if (nfse.Certificado.SelecionaCertificadoNroSerie(Trim(ConfigNFSE_CERT_DIG_NUM_SERIE.AsString)+';'+Trim(ConfigCIDADE.AsString)+';')) then
                begin
                 ...
                 // Config.First;
                  ...
                  nfse.Emitente.InscMunicipal := RetornaNumeros(ConfigINSC_MUNIC.AsString);
                 ...
                 if (ConfigOPT_SIMP_NAC.Value = 1) then //0: nao e 1: sim
                  ...
                  nfse.NotaFiscal.Servico.CodigoTributacaoMunicipio :=FormatFloat('0000',ConfigNFSE_COD_TRIB_SERV.Value);

                  if  (ConfigNFSE_COD_ATV_MUN.Value)>0 then
                  nfse.NotaFiscal.Servico.ItemListaServico  := FormatFloat('0000',ConfigNFSE_COD_ATV_MUN.Value);; //podendo ser o mesmo do cod. de trib
                  ...
                  begin // Já tinha sido enviado e esta esperando processamento
                    // ('entrou');
                    // 1 - Não recebido
                    // 2 - Não processado
                    ...
                    try
                      case nfse.AnalisarRespostaEnvio(Trim(NfsRPS_PROTOCOLO.AsString)) of
                        ...
                        begin
                          ...
                          if not FDConnection1.InTransaction then
                            FDConnection1.StartTransaction;
                          if not (Nfs.State in [dsedit]) then Nfs.Edit;
                          NfsSIT.Value := 3;
                          Nfs.Post;
                          FDConnection1.CommitRetaining;
                          nfse.MostarMsgErro:=True;

                          if (Pos('RPS já informado', String(nfse.msgErro)) > 0) then
                          begin
                            // ('Aguardando resposta de envio: 3 - RPS já informado');
                            if nfse.ConsultarNFSePorRps(IntToStr(BancodeDados.NfsNFS_ID.Value), '1') then
                            begin
                              if (nfse.NFSeNumero > 0) then
                              begin
                                if not BancodeDados.FDConnection1.InTransaction then
                                ...
                                vlNotaServdor     := nfse.NotaFiscal.Servico.vlTotal;
                                if (vlNotaServdor <> vlNotaTmp) then
                                begin
                                  NfsSIT.Value := 1;
                                  if Trim(NfsOBS2.AsString) = '' then
                                    NfsOBS2.AsString := FormatDateTime('dd/mm/yy hh:mm:ss', Now) + #13 + 'O valor da nota no sistema (' +
                                      FormatFloat(',0.00', vlNotaTmp) + ') está diferente do valor informado pelo servidor ' +
                                      FormatFloat(',0.00', vlNotaServdor) + '.'
                                  else
                                    NfsOBS2.AsString := FormatDateTime('dd/mm/yy hh:mm:ss', Now) + #13 + 'O valor da nota no sistema (' +
                                      FormatFloat(',0.00', vlNotaTmp) + ') está diferente do valor informado pelo servidor ' +
                                      FormatFloat(',0.00', vlNotaServdor) + '.' + #13#13 + NfsOBS2.AsString;
                                end
                                else
                                begin
                                  BancodeDados.NfsNFSE_NUMERO.Value     := nfse.NFSeNumero;
                                  ...
                                  BancodeDados.NfsNFSE_DT_EMISSAO.Value := nfse.NFSeDtEmissao;
                                  BancodeDados.NfsXML_NFSE.AsString     := nfse.TextoXML_NFSe;
                                  BancodeDados.NfsNFSE_AMBIENTE.Value   := ConfigNFSE_AMBIENTE.Value;
                                end;
                          ...
                          if nfse.ConsultarSituacaoLoteRps((RetornaNumeros(NfsRPS_PROTOCOLO.Value)))>0 then //pegando  o erro do lote
                          begin

                            nfse.ConsultarLoteRps(Trim(NfsRPS_PROTOCOLO.AsString));
                          ...
                          begin
                            ...
                            begin
                              ...
                              if Trim(NfsOBS2.AsString) = '' then
                                ...
                                NfsOBS2.AsString := FormatDateTime('dd/mm/yy hh:mm:ss', Now) + #13 + 'O valor da nota no sistema (' +
                                  FormatFloat(',0.00', vlNotaTmp) + ') está diferente do valor informado pelo servidor ' + FormatFloat(',0.00', vlNotaServdor) +
                      ...
                      on e: Exception do
                      begin
                          Mensagem('Exceção no método - nfse.AnalisarRespostaEnvio(Trim(NfsRPS_PROTOCOLO.Value)) -.'+#13+e.Message, mtInformation, [mbOk], mrOk, 0);
                      end;
                    end;
                  ...
                  begin
                    ...
                    try
                     ...
                     Mensagem('Erro ao atribuir competência. '+E.Message , mtWarning, [mbOk], mrOk, 0);
                    end;
                   ...
                   // nfse.NotaFiscal.
                    Clientes.Close;
                    Clientes.SQL.Text := PrincipalForm.SQLrepositorio(1) +' and c.cli_id = ' + IntToStr(NfsCLI_ID.Value);
                    Clientes.Open;

                    nfse.Cliente.CNPJ_CPF     := RetornaNumeros(ClientesCNPJ.Value);
                    nfse.Cliente.RazaoSocial  := UpperCase(SemAcento(ClientesNOME.Value));
                    nfse.Cliente.Logradouro   := SemAcento(ClientesLOGRADOURO.Value);
                    ...
                    nfse.Cliente.Telefone     := RetornaNumeros(ClientesFONE1.Value);
                    nfse.Cliente.Fax          := RetornaNumeros(ClientesFAX.Value);

                    // Added by Denilson SED 28/04/2020 14:01:54
                    if nfse.CodCidade = 2504009 then //se for campina grande...
                    begin
                      if ClientesINSC_MUNICIPAL.Value <> EmptyStr then //se o cliente tiver inscrição municipal atribui no xml.
                       nfse.Cliente.InscMunicipal:= ClientesINSC_MUNICIPAL.Value;
                    ...
                    if (RetornaNumeros(ClientesCEP.Value) <> '') then
                      nfse.Cliente.CEP := StrToInt(RetornaNumeros(ClientesCEP.Value));

                    nfse.NotaFiscal.RegimeTrib              := tipoRegimeTributacao_MicroEmpresaMunicipal;
                    ...
                    nfse.NotaFiscal.Servico.Descricao := SubstituirString(PegaListaServicos(NfsNFS_ID.Value), #13, '<br />');
                  ...
                  begin
                    ...
                    Nfs.Edit;
                    if Trim(NfsOBS2.AsString) = '' then
                      NfsOBS2.AsString := FormatDateTime('dd/mm/yy hh:mm:ss', Now) + #13 + nfse.msgErro
                    else
                      NfsOBS2.AsString := FormatDateTime('dd/mm/yy hh:mm:ss', Now) + #13 + nfse.msgErro + #13#13 + NfsOBS2.AsString;
                    Nfs.Post;
                    FDConnection1.CommitRetaining;
                  end;
                ...
                on e: Exception do
                begin
                    Mensagem('Exceção no método - EnviarNFSe -.'+#13+e.Message, mtInformation, [mbOk], mrOk, 0);
                end;

              end;
            end else
            begin
              Mensagem('É necessário informar o número de série do certificado digital no cadastro da unidade escolar.', mtInformation, [mbOk], mrOk, 0);

            end;
          end;
          // else
          // Mensagem('Unidade escolar não encontrada.', mtInformation, [mbOk], mrOk, 0);
        end else
        begin
          Mensagem('A nota não está fechada.', mtInformation, [mbOk], mrOk, 0);
        end;

      end else
      begin
       Mensagem('A nota já foi enviada.', mtInformation, [mbOk], mrOk, 0);
      end;
...
begin
      ...
      begin
        ...
        begin
          if (BancodeDados.ConfigNFSE_AMBIENTE.Value = 1) then
            ...
            podeCancelar := nfse.CancelarNFSe(BancodeDados.NfsNFSE_NUMERO.Value,True);
            ...
            begin
              ...
              if trim(BancodeDados.NfsOBS2.AsString) = '' then
                BancodeDados.NfsOBS2.AsString := FormatDateTime('dd/mm/yy hh:mm:ss', PrincipalForm.DataHoraServidor) + #13 + nfse.msgErro
              else
                BancodeDados.NfsOBS2.AsString := FormatDateTime('dd/mm/yy hh:mm:ss', PrincipalForm.DataHoraServidor) + #13 + nfse.msgErro + #13#13 +
  ...
  begin
    ...
    BancodeDados.Adiciona.SQL.Text := 'update boletos set nfs_id = null where nfs_id = ' + IntToStr(BancodeDados.NfsNFS_ID.Value);
    BancodeDados.Adiciona.ExecSQL;
