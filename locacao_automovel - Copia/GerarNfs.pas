...
begin
  ...
  begin
    ...
    GerarNfsForm.qryDtReceb.SQL.Text := 'select b.dt_credito from boletos b' +
    ...
    // ' where b.CLI_ID = '+IntToStr(GerarNfsForm.TempCLI_ID.Value)+
      ' where c.CNPJ = ' + QuotedStr(GerarNfsForm.TempCNPJ.Value) +
      ' and b.situacao_boleto = ' + QuotedStr('RECEBIDO') +
      ' and coalesce(b.nfs_id, 0) = 0' + ' and b.' + CampoData + ' between ' +
      QuotedStr(FormatDateTime('yyyy/mm/dd', GerarNfsForm.dtIni)) + ' and ' +
  ...
  begin
    ...
    // ' left join livro_caixa l on (l.caixa_id = b.caixa_id)' +
      ...
      ' and c.cnpj = ' + QuotedStr(GerarNfsForm.TempCNPJ.Value) +
      ' and b.situacao_boleto = ' + QuotedStr('RECEBIDO') +
      ' and coalesce(b.nfs_id, 0) = 0' + ' and b.' + CampoData + ' between ' +
      QuotedStr(FormatDateTime('yyyy/mm/dd', GerarNfsForm.dtIni)) + ' and ' +
      QuotedStr(FormatDateTime('yyyy/mm/dd', GerarNfsForm.dtFim));
...
begin
  ...
  try
    ...
    nfs.SQL.Clear;
    nfs.SQL.Add
      ('insert into nfs (nfs.nfs_id, nfs.vl_total, nfs.vl_deducoes, nfs.cli_id, nfs.dt_rps, nfs.serie, nfs.nfse_dt_emissao, DT_CAD, PERIODO_PROCS, competencia, perc_iss, vl_iss, sit)');
    nfs.SQL.Add
      ('values (GEN_ID(gen_nfs_cod,1), :Total,:Deducao, :Client ,(select current_timestamp from rdb$database),1 ,:DT_EMISSAO,(select current_timestamp from rdb$database),:PERIODO_PROCS, :competencia, (select perc_iss from config)');
    nfs.SQL.Add(',(((select perc_iss from config) * :Total)/100), 0 )');
    // calculo do iss
    nfs.ExecSQL;
    ...
    cliente.SQL.Clear;
    cliente.SQL.Add
      ('insert into clientes (cli_id, nome, cnpj, logradouro, bairro, ');
    cliente.SQL.Add
      ('cidade, estado, cep, email_contato1,email_contato2, sit, nome as fantasia, fat_nota,FONE1, FONE2) values (GEN_ID(gen_clicod, 1), ');
    cliente.SQL.Add
      (':Nome, :Cnpj, :Logradouro,:Bairro,(select  first 1 municipios.codmun from municipios where municipios.nome =:Cidade), (select  first 1 estados.uf_cod from estados where estados.uf =:Estado),:CEP, :Email,:Email, 1, :Fantasia, 1,:FONE1,:FONE2 )');
    cliente.ExecSQL;

    nfs_serv.Close;
    nfs_serv.SQL.Clear;
    nfs_serv.SQL.Add
      ('insert into nfs_serv  (nfs_it_id, nfs_id, descricao, vl_total) values ');
    nfs_serv.SQL.Add
      (' (gen_id(gen_nfs_serv_cod, 1), :NFS_ID, :Descricao, :Total)');
    nfs_serv.ExecSQL;
  ...
  begin
    ...
    begin
      try
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              if (NUM_COLUMNS = 3) and (StringGrid1.Cells[0, 0] = 'Nome') then
                // identificando a planilha de nomes e cpf
                Importar_Nomes(dsPlanilhaNomes, StringGrid1)
              else
              begin
                Mensagem('O layout da segunda planilha não foi identificado.'#13
                  + 'Por favor, verifique a planilha selecionada.', mtWarning,
                  [mbok], mrok, 0);
                Gauge1.Visible := False;
                // caso o usuario selecione uma planilha errada.
                exit
              end;
          ...
          begin
            if (dsPlanilhaNomes.Fields[2].AsString <> EmptyStr) and
              ((dsPlanilhaNomes.Fields[2].AsString)
              = RetornaNumeros(DataSet.Fields[14].AsString)) then
            begin

              client_nome := dsPlanilhaNomes.Fields[1].AsString;
              // pegando nome do responsavel
          ...
          begin
            ...
            Nfs_nao_importadasVALOR_SERV.AsCurrency := DataSet.Fields[4]
              .AsCurrency;
          ...
          begin
            ...
            cliente.ParamByName('Logradouro').Value :=
              Copy(DataSet.Fields[17].AsString, 1, 60);
            ...
            if (DataSet.Fields[22].AsString = 'Não Informado') or
              (DataSet.Fields[22].AsString = EmptyStr) then
              cliente.ParamByName('Email').Value :=
                BancodeDados.ConfigEMAIL_SUPORT.AsString
            else
        ...
        begin
          ...
          total := (DataSet.Fields[4].AsCurrency - DataSet.Fields[7].AsCurrency)
            - DataSet.Fields[8].AsCurrency;
          nfs.ParamByName('Total').Value := total;
          // (total - dataset.Fields[7].AsCurrency);//valor da planilha
          nfs.ParamByName('Deducao').Value := DataSet.Fields[8].AsCurrency;
          // valor da planilha
          nfs.ParamByName('Client').AsInteger := client_id; // id co cliente
          nfs.ParamByName('DT_EMISSAO').Value :=
            StrToDate(DataSet.Fields[2].AsString);
          ...
          // exit;
          // end;
          nfs_serv.ParamByName('NFS_ID').AsInteger :=
            pegarUltimoID('NFS_ID', 'NFS');
          nfs_serv.ParamByName('Descricao').AsString :=
            DataSet.Fields[12].AsString;
          nfs_serv.ParamByName('Total').Value := total;
          // (dataset.Fields[4].AsCurrency - dataset.Fields[7].AsCurrency);//dataset.Fields[4].AsCurrency; //valor da planilha
          nfs_serv.ExecSQL;
        ...
        on E: Exception do
        begin
          Mensagem('Ocorreu um erro na importação dos dados. ' + E.Message,
    ...
    qryCli.SQL.Clear;
    qryCli.SQL.Text :=
      ('select c.cli_id, c.cnpj, c.nome, c.fat_nota,  1 as contador, (e.vl_total - e.vl_deducoes) as vl_int,  (e.vl_total - e.vl_deducoes) as vl_pago, (e.vl_total) as valor_apagar_orig from clientes c, nfs e where c.cli_id=e.cli_id ');
    qryCli.SQL.Add
      ('  and e.dt_rps = (select current_date from rdb$database) order by c.cli_id ');
    qryCli.Open;
    // (qryCli.SQL.Text);
    Gauge1.Visible := False;
    ...
    begin
      if Mensagem
        ('Alguns registro não foram importados. Deseja visualizar o relatório?',
    ...
    if Mensagem('Dados importados com sucesso, Deseja consultar as notas ?',
      mtConfirmation, [mbYes, mbNo], mrYes, 0) = idYes then
    begin
    ...
    Mensagem('O Período de Processamento já foi importado.', mtInformation,
      [mbok], mrok, 0);
...
begin
  ...
  begin
    ...
    begin
      if nlinha = 0 then
      begin
        DataSet.FieldDefs.Add(StringGrid1.Cells[ncoluna, nlinha], ftString, 40,
          False); // adicionamos campos
...
begin
  ...
  try
    ...
    qry.Close;
    qry.SQL.Add('select distinct ' + campo_id + ' ID from ' + tabela_nome +
...
begin
  ...
  begin
    ...
    if (x = 3) and (em[2] = '0') and (em[1] = '0') then
      // caso o cpf inicie com 000
      letras := '123456789';
...
begin
  ...
  try
    qry.Connection := BancodeDados.FDConnection1;
    qry.Close;
    qry.SQL.Add('select * from nfs where nfs.periodo_procs =' +
      QuotedStr(periodo));
    qry.Open;
...
begin
  ...
  try
    ...
    qryPesq.Close;
    qryPesq.SQL.Add('select cli_id from clientes where cnpj = ' +
      QuotedStr(cpf));
    qryPesq.Open;
...
begin
  ...
  begin
    ...
    lbContadorN.Caption := 'Registro(s) Encontrado(s): ' +
      FormatFloat('000000', Temp.RecordCount) + '   -   Valor Total: ' +
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            DataSet.FieldDefs.Add(StringGrid1.Cells[ncoluna, nlinha], ftString,
              100, False); // adicionamos campos
          end;
        end
    ...
    on E: Exception do
      Mensagem('O Layout da planilha não foi identificado: ' + E.Message,
        mtWarning, [mbok], mrok, 0);
  end;

end;

procedure TGerarNfsForm.EditConsultaKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
begin
  Mensagem('1. Selecione a planilha com as informações das NFS.'#13 +
    '2. Se nescessário, selecione a segunda planilha com as informações de nome e cpf.',
    mtInformation, [mbok], mrok, 0);
  ...
  begin
    ...
    XlsToStringGrid(StringGrid1, OpenDialog1.FileName);
    if (NUM_COLUMNS = 24) and (StringGrid1.Cells[0, 0] = 'Unidade:') then
    // validando planilha
    begin
      Gauge1.MaxValue := 100;
      Gauge1.Visible := true;
      ...
      if Mensagem('Deseja definir a competência agora?', mtConfirmation,
      ...
      Mensagem('O layout da primeira planilha não foi identificado.'#13 +
        'Por favor, verifique a planilha selecionada.', mtWarning,
        [mbok], mrok, 0);
...
procedure TGerarNfsForm.DBGrid1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key = 32 then
  begin
...
procedure TGerarNfsForm.DBGrid1DrawColumnCell(Sender: TObject;
...
begin // add Denilson
  ...
  if { not } (BancodeDados.ConfigTIPO_CLIENT.AsInteger = 2) then
    // cliente Yázigi
    testClient := true;
  // fim add
  try
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        qryCli.SQL.Text :=
          'select c.cli_id, c.cnpj, c.nome, c.fat_nota,  1 as contador, (e.vl_total - e.vl_deducoes) as vl_int, '
          + ' (e.vl_total - e.vl_deducoes) as vl_pago, (e.vl_total) as valor_apagar_orig from clientes c, nfs e where c.cli_id=e.cli_id ';

        qryCli.SQL.Add(' and e.' + CampoData + ' between ' +
          QuotedStr(FormatDateTime('yyyy/mm/dd', dtIni)) + ' and ' +
      ...
      begin
        ...
        qryCli.SQL.Text := 'select c.cli_id, c.cnpj, c.nome, c.fat_nota, ' +
          ...
          + ' from boletos b left join clientes c on (c.cli_id = b.cli_id) left join livro_caixa l on (l.caixa_id = b.caixa_id)'
          + ' where b.situacao_boleto = ' + QuotedStr('RECEBIDO') +
          ' and coalesce(b.nfs_id, 0) = 0';
        ...
        qryCli.SQL.Add(' and b.' + CampoData + ' between ' +
          QuotedStr(FormatDateTime('yyyy/mm/dd', dtIni)) + ' and ' +
          QuotedStr(FormatDateTime('yyyy/mm/dd', dtFim)));
        ...
        try
          ...
          begin
            ...
            begin
              ...
              TempCREDITO.Value := qryDtReceb.FieldByName('dt_credito')
                .AsDateTime; // eraldo trouco esta linha
...
begin
  ...
  try
    ...
    try
      ...
      RangeMatrix := XLSAplicacao.Range['A1', XLSAplicacao.Cells.Item[x,
        y]].Value;
...
begin
  ...
  begin
    if Mensagem
      ('Tem certeza que deseja gerar as notas fiscais selecionadas agora?' + #13
      + 'Só serão geradas as notas que o cliente estiver configurado para faturar nota.',
    ...
    begin
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              BancodeDados.nfs.SQL.Add(' where n.sit = 0 and n.cli_id = ' +
                IntToStr(TempCLI_ID.Value) + ' order by n.dt_rps');
              ...
              begin
                ...
                if Mensagem('Existe uma nota fiscal em aberto para o cliente ' +
                  QuotedStr(TempCLIENTE.Value) + ' no dia ' +
              ...
              begin
                ...
                if not BancodeDados.Config.Active then
                  BancodeDados.Config.Open();
                BancodeDados.NfsNFS_ID.Value :=
                  BancodeDados.PegaSeq_Nfse(BancodeDados.ConfigCONF_ID.Value);
              end
              else
              ...
              try
                BancodeDados.NfsCompetencia.Value := SedDateEdit1.DateTime;
                // FormatDateTime('dd/mm/yyyy',SedDateEdit1.DateTime);
              except
                on Exception do
              ...
              begin
                ...
                if (BancodeDados.BoletosVENCIMENTO_ORIG.Value > 0) then
                  descVenc := FormatDateTime('mmm/yyyy',
                ...
                BancodeDados.NfsServDESCRICAO.Value :=
                  BancodeDados.BoletosDESCRICAO.Value + ' - ' + descVenc;
                ...
                if (BancodeDados.BoletosVALOR_PAGO.Value <
                  BancodeDados.BoletosVALOR_INTEGRAL.Value) then
                  BancodeDados.NfsServVL_UNIT.Value :=
      ...
      begin
        ...
        try
          ...
          BancodeDados.nfs.Close;
          BancodeDados.nfs.SQL.Text :=
            'select n.*, c.nome, c.fantasia, c.cnpj, c.logradouro, c.bairro, c.cidade, c.estado, c.email_contato2,  m.nome as cidade_nome, m.uf as uf_nome';
          BancodeDados.nfs.SQL.Add
            (' from nfs n left join clientes c on (c.cli_id = n.cli_id) left join MUNICIPIOS m on (m.codmun=c.cidade) where n.NFS_ID in ('
            + idNotas + ')');
          BancodeDados.nfs.Open;
    ...
    Mensagem('Marque algum cliente antes de prosseguir.', mtInformation,
      [mbok], mrok, 0);
end;
