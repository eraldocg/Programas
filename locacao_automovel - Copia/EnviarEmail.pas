...
begin
  with TEnviarEmailForm.Create(nil) do
    try
      ...
      begin
        arqTmp := SysTempDir + 'SEDSoft_' + FormatDateTime('ddmmyy_hhnn',
          Now) + '.pdf';
        ...
        BancodeDados.EnviarEmail(Trim(EditEmailDestino.Text),
          Trim(EditAssunto.Text), Trim(Memo1.Lines.Text), arqTmp);
...
begin
  ...
  Memo1.Text := Trim(StringReplace(Memo1.Text, '%texto_livre%',
    WebBrowser1.OleObject.Document.DocumentElement.InnerHtml, []));
      ...
      begin
        ...
        begin
          ...
          if BancodeDados.ClientesEMAIL_CONTATO1.Value <> EmptyStr then
            EditEmailDestino.Text := EditEmailDestino.Text +
              BancodeDados.ClientesEMAIL_CONTATO1.Value + ';';
        end;
