...
if (BancodeDados.Cont_Serv_Clien.State in [dsInsert]) then
  BancodeDados.veiculos.SQL.Text:='select * from veiculos where coalesce(sit,0)=0 and coalesce(c_s_c_id,0)=0 order by dt_cad desc'
else
...
EditVeiculo.Enabled:=False else EditVeiculo.Enabled:=true;
