...
{  Registro := TSEDRegistro.Create;
  ...
  try
    ...
    BancodeDados.Bancos.SQL.Text := 'select * from bancos where conta_corrente <> ' + QuotedStr('')+' and agencia <> ' + QuotedStr('')+' order by banco_nome';
    BancodeDados.Bancos.Open;
    BancodeDados.Bancos.Last;

   qryTipoContas2.Close;
   qryTipoContas2.SQL.Text:='select conta_id, conta from tipo_contas order by conta';
   qryTipoContas2.open;
...
begin
if not Temp.IsEmpty then begin
  lbBanco1.Caption:='Banco: '+BancodeDados.BancosBANCO_NOME.Value;
  if chkIndividual.Checked then
  lbTipoMov_1.Caption:='Opçoes do Livro Caixa: Lançamentos individuais por registro' else
  if chkSintetico.Checked then
  lbTipoMov_1.Caption:='Opçoes do Livro Caixa: Lançamento único sintetizado';
...
begin
  ...
  try
    ...
    if (lnAgencia = StrToInt64(BancodeDados.BancosAGENCIA.Value)) and (lnConta = StrToInt64(BancodeDados.BancosCONTA_CORRENTE.Value)) then
    begin
      TotalLinhas := 1;
      while not Eof(F) do
      begin
      ...
      begin
        ...
        Delete(Linha, 1, Pos(';', Linha));
        Delete(Linha, 1, Pos(';', Linha));
        lnData := EncodeDate(StrToInt(Copy(Linha, 5, 4)), StrToInt(Copy(Linha, 3, 2)), StrToInt(Copy(Linha, 1, 2)));
        Delete(Linha, 1, Pos(';', Linha));
        Delete(Linha, 1, Pos(';', Linha));
        Delete(Linha, 1, Pos(';', Linha));
        Delete(Linha, 1, Pos(';', Linha));
        ...
        if ((lnTipo = 'D') or (lnTipo = 'C')) and not((lnCodOpr = 0) or (lnCodOpr = 999)) then
        begin
          ...
          begin
            ...
            ImpExtratoForm.TempDESCRICAO.AsString := FormatFloat('000', lnCodOpr) + ' - ' + lnDescricao + '   ' + IntToStr(lnAgencia) + '-' + lnAgenciaDV + ' / ' + IntToStr(lnConta) + '-' + lnContaDV;
            ...
            if (Copy(lnDescricao, 0,3)='Cob') and (lnTipo = 'C') or (Copy(lnDescricao, 0,10)='D AT SICOB') then ImpExtratoForm.TempATIVO.Value:=False else ImpExtratoForm.TempATIVO.Value:=True;
      ...
      Mensagem('Este arquivo não pertence à conta selecionada.', mtInformation, [mbOk], mrOk, 0);
  ...
  if (ImpExtratoForm.chkSintetico.Checked) and ((vlTotalEnt > 0) or (vlTotalSaid > 0)) then
  begin
    ImpExtratoForm.Temp.Append;
    ImpExtratoForm.TempDESCRICAO.AsString := 'Extrato Bancário: ' + IntToStr(lnAgencia) + '-' + lnAgenciaDV + ' / ' + IntToStr(lnConta) + '-' + lnContaDV;
...
begin
  ...
  try
    ...
    begin
      ...
      if (lnConta = StrToInt(Trim(BancodeDados.BancosCONTA_CORRENTE.Value) + Trim(BancodeDados.BancosDIG_CONTA_CORRENTE.Value))) and (lnNumBanco = BancodeDados.BancosCODIGO_BANCO.Value) then
      begin
        ...
        begin
          ...
          begin
            ...
            lnData := EncodeDate(StrToInt(Copy(Linha, 1, 4)), StrToInt(Copy(Linha, 5, 2)), StrToInt(Copy(Linha, 7, 2)));
            ReadLn(F, Linha);
            ...
            begin
              ...
              ImpExtratoForm.TempDESCRICAO.AsString := lnDescricao + '   ' + IntToStr(lnConta){ + '-' + lnContaDV} else
              ...
              if (Copy(lnDescricao, 0,3)='COB') and (lnValor>0) or (Copy(lnDescricao, 0,10)='D AT SICOB') then ImpExtratoForm.TempATIVO.Value:=False else ImpExtratoForm.TempATIVO.Value:=True;
      ...
      begin
        ImpExtratoForm.Temp.Append;
        ImpExtratoForm.TempDESCRICAO.AsString := 'Extrato Bancário: ' + IntToStr(lnConta){ + '-' + lnContaDV};
        ImpExtratoForm.TempDATA.Value       := Date;
        ImpExtratoForm.TempENTRADA.Value    := vlTotalEnt;
        ImpExtratoForm.TempSAIDA.Value      := vlTotalSaid;
...
//try
  ...
  if BancodeDados.BancosCODIGO_BANCO.Value =1 then PodIR:=true else   //Banco brasil
  if BancodeDados.BancosCODIGO_BANCO.Value =341 then PodIR:=true;   //Banco Itaú

  if PodIR=true then begin
    ...
    ContaCliente:= Trim(BancodeDados.BancosCONTA_CORRENTE.Value) + Trim(BancodeDados.BancosDIG_CONTA_CORRENTE.Value);

    if YMOFXReader1.BankID=341 then //ajustando a numero de conta Banco Itaú
    ContaCliente:= Trim(BancodeDados.BancosAGENCIA.Value) + Trim(BancodeDados.BancosCONTA_CORRENTE.Value) + Trim(BancodeDados.BancosDIG_CONTA_CORRENTE.Value);

      if (RetornaLimpo(YMOFXReader1.AccountID) = ContaCliente) and  (YMOFXReader1.BankID = BancodeDados.BancosCODIGO_BANCO.Value) then
      begin
        try
          ...
          ImpExtratoForm.Gauge1.MaxValue := YMOFXReader1.Count; //ComponentIndex;
            ...
            begin
                if ImpExtratoForm.chkIndividual.Checked  then begin
                  ImpExtratoForm.Gauge1.Progress := i+1;
                  if not ImpExtratoForm.Temp.Active then ImpExtratoForm.Temp.Open;
                  if not (ImpExtratoForm.Temp.state in [dsinsert,dsedit]) then ImpExtratoForm.Temp.Append;
                  ImpExtratoForm.TempDESCRICAO.AsString:=YMOFXReader1.Get(i).Desc;
                  if YMOFXReader1.Get(i).Value>0 then
                  ImpExtratoForm.TempENTRADA.Value   :=(YMOFXReader1.Get(i).Value) else ImpExtratoForm.TempSAIDA.Value  :=(YMOFXReader1.Get(i).Value) * -1;
                  ImpExtratoForm.TempDATA.Value      :=(YMOFXReader1.Get(i).MovDate);
                  ImpExtratoForm.TempDOCUMENTO.AsString :=YMOFXReader1.Get(i).ID;
                  ImpExtratoForm.TempCONTA_ID.Value  := ContaID;
                  if (Copy(YMOFXReader1.Get(i).Desc, 0,3)='COB') and (YMOFXReader1.Get(i).Value>0) or (Copy(YMOFXReader1.Get(i).Desc, 0,10)='D AT SICOB') then ImpExtratoForm.TempATIVO.Value:=False else ImpExtratoForm.TempATIVO.Value:=True;

                  ImpExtratoForm.Temp.Post;
                end else //rgOpcoes.ItemIndex = 0
            ...
            if (ImpExtratoForm.chkSintetico.Checked) and ((vlTotalEnt > 0) or (vlTotalSaid > 0)) then begin //ImpExtratoForm.rgOpcoes.
              if not ImpExtratoForm.Temp.Active then ImpExtratoForm.Temp.Open;
              if not (ImpExtratoForm.Temp.state in [dsinsert,dsedit]) then ImpExtratoForm.Temp.Append;
              ImpExtratoForm.TempDESCRICAO.AsString := 'Extrato Bancário: ' + (YMOFXReader1.AccountID);
              ImpExtratoForm.TempDATA.Value       := date;
              ImpExtratoForm.TempENTRADA.Value    := vlTotalEnt;
              ImpExtratoForm.TempSAIDA.Value      := vlTotalSaid;
              //ImpExtratoForm.TempOP_PAG_ID.Value  := 3;
      ...
      end else begin  //teste validando banco
         Mensagem('Este arquivo não pertence à conta selecionada.', mtInformation, [mbOk], mrOk, 0);
        ImpExtratoForm.EditBanco1.Enabled   :=true;
        ImpExtratoForm.EditConta.Enabled    :=true;
        ImpExtratoForm.chkIndividual.Enabled:=true;
  ...
  end else begin//apenas os bancos com conciliacao pronta...//PodIR=true
    Mensagem('Este banco/arquivo não está disponível para conciliação bancária.'#13'Dica: Tente outro formato(OFC) de arquivo bancário.', mtInformation, [mbOk], mrOk, 0);
...
if not (DBGrid1.SelectedField.FieldName='ATIVO') then begin
  if not Assigned(CsPagarForm) then CsPagarForm := tCsPagarForm.Create(Application);
  try
    if CsPagarForm.ShowModal = mrok then begin
        if BancodeDados.ContasCONTA_ID.Value>0 then begin
          ImpExtratoForm.Temp.Edit;
          ImpExtratoForm.TempPAGAR_ID.Value      :=BancodeDados.PagarPAGAR_ID.Value;
          ImpExtratoForm.TempPAGAR_NOME.AsString  :=BancodeDados.PagarDESCRICAO.AsString;
...
begin
  ...
  lbRegistro.Caption:='';
    ...
    begin
      ...
      if (ImpExtratoForm.chkSintetico.Checked) and (Mensagem('Tem certeza que deseja realizar um lançamento sintetizado?'#13, mtConfirmation, [mbYes, mbNo], mrNo, 0) = idNo) then
        PodeIr := false;
            ...
            begin
              ...
              begin
                ...
                begin
                  ...
                  LbNumero.Caption := 'Arquivo ' + IntToStr(i + 1) + '/' + IntToStr(Memo1.Lines.Count);
                  ...
                  if LerArquivo(Arquivo) = true then ImpExtratoForm.Temp.First;
...
if not Temp.IsEmpty then PodePassar:=true;
{begin
  HabilitarBotoes(self,false);
  Screen.Cursor:=crSQLWait;
...
if (PodePassar = true) then begin//Mensagem('Existem lançamentos sem o tipo de conta definido.', mtInformation, [mbOk], mrOk, 0) else ModalResult:=mrOk;
...
///if ImpExtratoForm.chkIndividual.Checked then  if ImpExtratoForm.chkSintetico.Checked then
  ...
  begin
        ...
        begin
          ...
          Application.ProcessMessages;
            ...
            begin
              ...
              BancodeDados.Livro_Caixa.SQL.Text :=PrincipalForm.SQLrepositorio(2) + ' and l.data =' + QuotedStr(FormatDateTime('yyyy/mm/dd', TempDATA.Value)) + ' and l.descricao=' + QuotedStr(Trim(TempDESCRICAO.AsString));
             if Trim(Copy(TempDOCUMENTO.AsString, 1, 20)) <> '' then
                BancodeDados.Livro_Caixa.SQL.Add(' and l.documento=' + QuotedStr(Trim(Copy(TempDOCUMENTO.AsString, 1, 20))));
              if TempENTRADA.Value > 0 then
                BancodeDados.Livro_Caixa.SQL.Add(' and l.entrada=' + CurrToStr(TempENTRADA.Value));
              if TempSAIDA.Value > 0 then
                BancodeDados.Livro_Caixa.SQL.Add(' and l.saida=' + CurrToStr(TempSAIDA.Value));
              ...
              BancodeDados.Livro_CaixaDOCUMENTO.Value     := Trim(Copy(TempDOCUMENTO.AsString, 1, 20));
              BancodeDados.Livro_CaixaDESCRICAO.Value     := Trim(Copy(TempDESCRICAO.AsString, 1, 60));
        ...
        if LancouCaixa = true then  begin
        //renomeando o arqivo ja preocessado /evitando q o usuario refaça sem querer
        RenameFile(Arquivo, aDiretorio + aNomeArq + '_' + FormatDateTime('yyyy_mm_dd', Date) + '.' + aExtArq); if not(Pos('PROCESSADO', UpperCase(aDiretorio)) > 0) then
        begin
          if not DirectoryExists(aDiretorio + 'processado\') then
            CreateDir(aDiretorio + 'processado\');
          MoveFile(PChar(aDiretorio + aNomeArq + '_' + FormatDateTime('yyyy_mm_dd', Date) + '.' + aExtArq), PChar(aDiretorio + 'processado\' + aNomeArq + '_' + FormatDateTime('yyyy_mm_dd', Date) + '.' + aExtArq))
        end;

        BancodeDados.FDConnection1.Commit;
        Mensagem('Processo concluído com sucesso.', mtInformation, [mbOk], mrOk, 0);
        Close;
      end else begin //LancouCaixa = true
        Screen.Cursor:=crDefault;
        EditBanco1.Enabled:=false;
        EditConta.Enabled :=false;
        chkIndividual.Enabled  :=false;
