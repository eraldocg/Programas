...
begin
  ...
  if not BancodeDados.Documentos.Active then
    BancodeDados.Documentos.Open;
    ...
    begin
      ...
      begin
        ...
        qryPlaca.SQL.Text :=
          ...
          QuotedStr(cpfTmp) + ' and not veiculo_id = ' +
          IntToStr(BancodeDados.VeiculosVEICULO_ID.Value);
        qryPlaca.Open;

        if not qryPlaca.IsEmpty then
        begin
          cpfTmp := '';
          Mensagem('Veículo já cadastrado:'#13 + FormatFloat('000000',
            qryPlacaVEICULO_ID.Value) + '/' + qryPlacaPLACA.Value + '/' +
            qryPlacaMODELO.Value + '/' + qryPlacaCOR.Value, mtInformation,
            [mbok], mrok, 0);
        end;
      end;
...
procedure TCadVeiculosForm.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  if BancodeDados.Veiculos.State in [dsInsert, dsEdit] then
    BancodeDados.Veiculos.Cancel;
end;
