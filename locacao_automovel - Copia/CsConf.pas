...
procedure TCsConfForm.DBGrid1DrawColumnCell(Sender: TObject; const Rect: TRect;
  DataCol: Integer; Column: TColumn; State: TGridDrawState);
begin
  {
    if not BancodeDados.Servicos.IsEmpty then
...
procedure TCsConfForm.EditNomeKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
begin
  ...
  BancodeDados.Veiculos.SQL.Text := 'select * from veiculos where conf_id = ' +
    IntToStr(BancodeDados.ConfigCONF_ID.Value);
  BancodeDados.Veiculos.Open();

  if not BancodeDados.Veiculos.IsEmpty then
  begin
    podeIR := False;
    Mensagem('Existe(m) veículo(s) associado(s) ao registro!', mtInformation,
      [mbok], mrok, 0);
  end;
