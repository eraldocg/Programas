...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if not (BancodeDados.Clientes.State in [dsEdit]) then  BancodeDados.Clientes.Edit;
        BancodeDados.MarcarAtivo(listaCli, BancodeDados.ClientesCLI_ID.Value, BancodeDados.ClientesATIVO);
      end;
    end;
...
procedure TCsClientesForm.EditNomeKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
...
begin
  ...
  try
    ...
    try
      ...
      CadClientesForm.ShowModal;
      lbContador.Caption := FormatFloat('000000', BancodeDados.Clientes.RecordCount) + ' registros.';
    finally
      CadClientesForm.Release;
      CadClientesForm := nil;
      Screen.Cursor := crDefault;
...
procedure TCsClientesForm.DBGrid1DrawColumnCell(Sender: TObject; const Rect: TRect; DataCol: Integer; Column: TColumn; State: TGridDrawState);
begin
  if BancodeDados.ClientesCLI_ID.Value > 0 then
  begin
    //if (BancodeDados.ClientesENTREGUE_BOL.Value = 1) then
    if (BancodeDados.ClientesSIT.Value =1) and (BancodeDados.ClientesN_TITULOS.Value = 1) and (BancodeDados.ClientesTIPO.Value='001 - Escola') then
      DBGrid1.Canvas.Font.Color := clRed;

    if (BancodeDados.ClientesSIT.Value =1) and (BancodeDados.ClientesN_TITULOS.Value = 0) and (BancodeDados.ClientesTIPO.Value='001 - Escola') then begin
      DBGrid1.Canvas.Font.Color := clRed;
...
procedure TCsClientesForm.DBGrid1KeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  if (Key = 32) then
  begin
...
procedure TCsClientesForm.EditConsultaKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
var
CPFteste : String;
...
begin
  ...
  begin
    ...
    if CBOpcao.ItemIndex=6 then begin
      if Trim(EditConsulta.Text) <> '' then begin
...
begin
  RastrearObjetoCorreios1.Enabled := (Trim(BancodeDados.ClientesCOD_RASTREIO.Value) <> '');

  if (usrNivel in [2]) then Financeiro1.Enabled:=True else Financeiro1.Enabled:=false;
end;
...
begin
    ...
    begin
      ...
      try
        Screen.Cursor := crSQLWait;
        HabilitarBotoes(Self, False);
        RelacaoServicosForm.Caption  :='Cliente: '+BancodeDados.ClientesNOME.Value;
...
begin
  ...
  begin
    ...
    try
      ...
      Boletos1Form.Caption := 'Financeiro do cliente - ' + BancodeDados.ClientesNOME.Value;
