    ...
    begin
      try
        ...
        BancodeDados.Boletos.SQL.Text := PrincipalForm.SQLrepositorio(0) +
          ' and ( b.vencimento <= ' + QuotedStr(FormatDateTime('yyyy/mm/dd',
          Date)) + ' ) and b.situacao_boleto=' + QuotedStr('A RECEBER') +
          ' order by c.nome, b.vencimento';
...
procedure TCsTitulosForm.EditNomeKeyDown(Sender: TObject; var Key: Word;
Shift: TShiftState);
begin
...
procedure TCsTitulosForm.DBGrid1DrawColumnCell(Sender: TObject;
  ...
  try
    ...
    try
      ...
      if BancodeDados.Boletos.IsEmpty then
        Mensagem('Não foi realizada nenhuma consulta!', mtInformation,
          [mbok], mrok, 0)
      else
...
begin
  ...
  begin
    lbContadorN.Caption := 'Registro(s) Encontrado(s): ' +
      FormatFloat('000000', BancodeDados.Boletos.RecordCount);
  end
  else
    lbContadorN.Caption := 'Registro(s) Encontrado(s): 000000';
end;

procedure TCsTitulosForm.EditConsultaKeyDown(Sender: TObject; var Key: Word;
Shift: TShiftState);
begin
  if Key = 13 then
  begin
...
var
  vlConfiavel { , vlTotInt, vlTotJurosCalc, vlTotAPagarCalc, vlTotPago, vlTotFalta }
    : currency;
  qryCli: TFDQuery;
begin
  ...
  begin
    ...
    try
      ...
      begin
        ...
        relTitulosForm.lbPeriodo.Caption := 'Período: ' +
        ...
        qryCli.Close;
        qryCli.SQL.Text :=
          'select cli_id, confiavel from clientes order by cli_id';
        qryCli.Open;
        ...
        relTitulosForm.lbConfiavel.Caption := 'Valor confiável: ' +
          FormatFloat(',0.00', vlConfiavel);
        BancodeDados.PrepararRel(relTitulosForm.RLReport2);
      end
      else
        Mensagem('Não existe informação disponível para este relatório!',
          mtInformation, [mbok], mrok, 0);
    finally
      qryCli.free;
