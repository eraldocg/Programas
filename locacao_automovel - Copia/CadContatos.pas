...
try
   ...
   if BancodeDados.Contatos.State in [dsedit,dsinsert] then BancodeDados.Contatos.Post;
   finally
...
begin
if (key=#13) and not (ActiveControl is TDBMemo) and not (ActiveControl is TDBGrid) then begin
  key:=#0;
  Perform(WM_NextDlgCtl,0,0);
end;
if (key=#27) {and not (BancodeDados.Unidades.State in [dsInsert,dsEdit]) and not (BancodeDados.Inst.State in [dsInsert,dsEdit])} then begin
  key:=#0;
  Close;
end;
end;
...
procedure TCadContatosForm.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin

if BancodeDados.Contatos.State in [dsedit,dsinsert] then BancodeDados.Contatos.Cancel;
end;
