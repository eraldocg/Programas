...
procedure TCsLoginForm.DBGrid1DrawColumnCell(Sender: TObject; const Rect: TRect;
  DataCol: Integer; Column: TColumn; State: TGridDrawState);
begin
...
begin
  if BancodeDados.LoginLOGIN_ID.Value > 0 then
    if (BancodeDados.LoginGERA_NFSE.IsNull) or
      (BancodeDados.LoginGERA_NFSE.Value <> 1) then
      if Mensagem('Definir como responsável por gerar a NFSe automaticamente?',
      ...
      begin
        try
          ...
          BancodeDados.Adiciona.SQL.Text :=
            'update login set gera_nfse=0 where not login_id=' +
            IntToStr(BancodeDados.LoginLOGIN_ID.Value);
          BancodeDados.Adiciona.ExecSQL;
          BancodeDados.Adiciona.SQL.Text :=
            'update login set gera_nfse=1, nivel=2 where login_id=' +
            IntToStr(BancodeDados.LoginLOGIN_ID.Value);
          BancodeDados.Adiciona.ExecSQL;
...
procedure TCsLoginForm.EditNomeKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TCsLoginForm.EditConsultaKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key = 13 then
  begin
