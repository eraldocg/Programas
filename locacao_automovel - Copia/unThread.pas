...
type
  ...
  protected
    ...
    function SolicitaAcesso(Privilegio: Integer = 0; Msg: String = ''; SmSED: Boolean = false): Boolean;
...
procedure TThreadPadrao.MostraMsg(Msg: String; naTela: Boolean; TipoMsg: Integer = 2);
begin
  FMsg := Msg;
  FTipoMsg := TipoMsg;
  if (naTela = True) then
...
function TThreadPadrao.SolicitaAcesso(Privilegio: Integer; Msg: String; SmSED: Boolean): Boolean;
begin
  FAcesso := false;
  FAcPriv := Privilegio;
  FacMsg := Msg;
...
begin
  ...
  begin
    try
      ...
      BancodeDados.SomBoletos.SQL.Text := 'Select sum(valor_apagar) as TT_ARECEBER from boletos where situacao_boleto =' + QuotedStr('A RECEBER') + ' and vencimento < ' + QuotedStr(FormatDateTime('yyyy/mm/dd', DATE));
      ...
      BancodeDados.SomPagar.SQL.Add('Select sum(valor_falta) as tt_pagar from v_pagar where ');
      BancodeDados.SomPagar.SQL.Add(' and situacao_pagar=' + QuotedStr('A PAGAR') + ' and vencimento <= ' + QuotedStr(FormatDateTime('yyyy/mm/dd', DATE)));
...
begin
  PrincipalForm.StatusBar1.Panels[0].Text := 'Inadimplência: ' + FormatFloat(',0.00', FVlInad);
  PrincipalForm.StatusBar1.Panels[1].Text := 'Receber Hoje: ' + FormatFloat(',0.00', FVlARec);
  PrincipalForm.StatusBar1.Panels[2].Text := 'A Pagar Hoje: ' + FormatFloat(',0.00', FVlAPag);
end;
