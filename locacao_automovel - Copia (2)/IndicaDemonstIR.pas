...
begin
  ...
  if (EditConta.Text <> '') then
    podePassar := True
  else if Mensagem('Tem certeza que deseja imprimir com todas as contas?',
  ...
  if (podePassar = True) then
    try
      ...
      RelDemonstIRForm.Receitas.SQL.Text :=
        'select c.nome as cliente, c.cnpj, coalesce(b.valor_mensal, 0) + coalesce(b.valor_lic, 0) as valor_integral, sum(b.valor_pago) as valor_com_juros, '
        + ' min(l.data) as prim_data, max(l.data) as ult_data, count(b.boleto_id) as qtd'
        + ' from boletos b' +
        ' left join livro_caixa l on (l.boleto_id = b.boleto_id)' +
        ' left join clientes c on (c.cli_id = b.cli_id)' + ' where ' + CampoData
        + ' between ' + QuotedStr(FormatDateTime('yyyy/mm/dd', dtIni.DateTime))
        + ' and ' + QuotedStr(FormatDateTime('yyyy/mm/dd', dtFim.DateTime));

      if chkBol.Checked then
        RelDemonstIRForm.Receitas.SQL.Add(' and b.retorno_id>0');

      if EditConta.Text <> '' then
        RelDemonstIRForm.Receitas.SQL.Add(' and l.conta_id = ' +
          IntToStr(EditConta.KeyValue));

      RelDemonstIRForm.Receitas.SQL.Add
        (' group by c.nome, c.cnpj, valor_integral');

      if chkAvulso.Checked then
      begin
        RelDemonstIRForm.Receitas.SQL.Add('union all' + ' select descricao, ' +
          QuotedStr('') +
          ', entrada, coalesce(entrada,0)-coalesce(saida,0) as saldo , data, data, 1'
          + ' from livro_caixa' + ' where data between ' +
          QuotedStr(FormatDateTime('yyyy/mm/dd', dtIni.DateTime)) + ' and ' +
          QuotedStr(FormatDateTime('yyyy/mm/dd', dtFim.DateTime)));
        if EditConta.Text <> '' then
          RelDemonstIRForm.Receitas.SQL.Add(' and conta_id = ' +
            IntToStr(EditConta.KeyValue));

        RelDemonstIRForm.Receitas.SQL.Add
          (' and (boleto_id is null or boleto_id = 0) and entrada>0');
        // RelDemonstIRForm.Receitas.SQL.Add(' and (origem_id is null or origem_id = 0)');
        RelDemonstIRForm.Receitas.SQL.Add
          (' and (conta_id_dest is null or conta_id_dest = 0)');
      end;
      ...
      begin
        ...
        begin
          ...
          vlComJuros := vlComJuros +
            RelDemonstIRForm.ReceitasVALOR_COM_JUROS.Value;
          RelDemonstIRForm.Receitas.Next;
        end;

        if cbData.ItemIndex = 0 then
          RelDemonstIRForm.lbPeriodo.Caption := 'Período de Recebimento: ' +
            dtIni.DateTimeStr + ' até ' + dtFim.DateTimeStr
        else
          RelDemonstIRForm.lbPeriodo.Caption := 'Período de Crédito: ' +
            dtIni.DateTimeStr + ' até ' + dtFim.DateTimeStr;
        ...
        RelDemonstIRForm.lbTotInt.Caption :=
          FormatFloat(',0.00;(,0.00)', vlInt);
        RelDemonstIRForm.lbTotComJuros.Caption :=
          FormatFloat(',0.00;(,0.00)', vlComJuros);
