...
uses
  ...
  Dialogs, StdCtrls, ExtCtrls, ComCtrls, Grids, DBGrids, SedDBGrid, DB, DBClient;
...
begin
  ...
  if Trim(EditConsulta.Text) <> '' then
   begin
    ...
    CadDocHTMLForm.Temp.Filter := 'CLASSIF = ' + QuotedStr(IntToStr(cbVisualizar.ItemIndex)) + ' and CAMPO <> ' + QuotedStr('') + ' and UPPER(DESCRICAO) LIKE ' + QuotedStr('%' + Trim(AnsiUpperCase(EditConsulta.Text)) + '%')  else

    CadDocHTMLForm.Temp.Filter := 'CAMPO <> ' + QuotedStr('') + ' and UPPER(DESCRICAO) LIKE ' + QuotedStr('%' + Trim(AnsiUpperCase(EditConsulta.Text)) + '%');
...
begin
if (cbVisualizar.ItemIndex>0) then begin
  CadDocHTMLForm.Temp.Filter := 'CLASSIF = ' + QuotedStr(IntToStr(cbVisualizar.ItemIndex));
  CadDocHTMLForm.Temp.Filtered := True;
...
procedure TCsCamposForm.DBGrid1DrawColumnCell(Sender: TObject; const Rect: TRect; DataCol: Integer; Column: TColumn; State: TGridDrawState);
begin
