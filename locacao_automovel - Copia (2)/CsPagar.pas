...
procedure TCsPagarForm.DBGrid1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if (Key = 32) then
  ...
  begin // 1
    ...
    begin
      ...
      try
        ...
        RelAnaliticoForm.lbPeriodo.Caption := CBData.Text + ': ' +
...
procedure TCsPagarForm.DBGrid1DrawColumnCell(Sender: TObject; const Rect: TRect;
  DataCol: Integer; Column: TColumn; State: TGridDrawState);
begin
  if BancodeDados.PagarPAGAR_ID.Value > 0 then
  begin
    ...
    if ((BancodeDados.PagarVENCIMENTO.Value < Date) and
      (BancodeDados.PagarSITUACAO_PAGAR.Value = 'A PAGAR') and
      (BancodeDados.PagarVALOR.Value > 0)) then
    ...
    if ((BancodeDados.PagarSITUACAO_PAGAR.Value = 'PAGO') and
      (BancodeDados.PagarPAGAMENTO.Value > BancodeDados.PagarVENCIMENTO.Value))
    then
...
begin
  ...
  begin
    ...
    lbContadorN.Caption := 'Registro(s) Encontrado(s): ' +
      FormatFloat('000000', BancodeDados.Pagar.RecNo);
    BancodeDados.Pagar.First;
  end
  else
  ...
  Try
    ...
    begin
      ...
      if ckAtraz.Checked then
        BancodeDados.Pagar.SQL.Add('or (p.vencimento < ' +
          QuotedStr(FormatDateTime('yyyy/mm/dd', Dt_Inicial.DateTime)) +
          ' and p.situacao_pagar=' + QuotedStr('A PAGAR') + ')');
...
procedure TCsPagarForm.EditConsultaKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
begin
  if not BancodeDados.Pagar.IsEmpty then
    ...
    begin
      if not(BancodeDados.Pagar.State in [dsedit]) then
        BancodeDados.Pagar.Edit;
      BancodeDados.MarcarAtivo(listaPagar, BancodeDados.PagarPAGAR_ID.Value,
        BancodeDados.PagarATIVO);
    end;
end;
...
begin
  ...
  begin
    ...
    BancodeDados.qryPagar.SQL.Text := 'select * from pagar where pagar_id in ('
      + IDs + ')';
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            try
              BancodeDados.Pagar.FieldByName(NomeCampo).Value :=
                BancodeDados.qryPagar.FieldByName(NomeCampo).Value;
            except
            end;
          end;
        end;
        ...
        BancodeDados.PagarVENCIMENTO.Value :=
          PegaData(BancodeDados.PagarVENCIMENTO.Value, 2);
