...
var
  ...
  TipoQuitacao: string;
  // 'Tipo de Quitação: '+BancodeDados.RetornoTipoQuitacao.AsString;
...
procedure TVisualizaRetornoForm.DBGrid1DrawColumnCell(Sender: TObject;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (RetornoForm.RetornoTmpCOD_OCORRENCIA.Value in [1, 2]) then
...
begin
  ...
  TipoQuitacao := 'Tipo de Quitação: ' +
    BancodeDados.RetornoTipoQuitacao.AsString;;
  ...
  begin
    ...
    lbTipoQuitacao.Caption :=
      'Atenção: Não foi encontrado nenhum título referente a este retorno.';
...
begin
  ...
  if not(BancodeDados.Retorno.IsEmpty) and (TabSheet1.Showing) then
  begin

    if not Assigned(RelArquivoRetornoForm) then
      RelArquivoRetornoForm := tRelArquivoRetornoForm.Create(Application);
    try
      RelArquivoRetornoForm.lbSituacao1.Caption := 'Arquivo Recebido';
      // 'Relatório Geral de Arquivo Retorno';
      RelArquivoRetornoForm.lbPeriodo1.Caption := 'Data do Arquivo: ' +
        FormatDateTime('dd/mm/yyyy', BancodeDados.RetornoDATA_ARQUIVO.Value);
      ...
      if not BancodeDados.Bancos.IsEmpty then
        RelArquivoRetornoForm.lbArquivo1.Caption := 'Banco: ' +
          BancodeDados.BancosBANCO_NOME.Value + ' - Arquivo Ret.: ' +
          QuotedStr(BancodeDados.RetornoNOME_ARQ.Value) + ' -  N.º ' +
      ...
      BancodeDados.PrepararRel(RelArquivoRetornoForm.RLReport1, false, nil,
        chkMala.Checked);
    finally
  ...
  if not(RetornoForm.RetornoTmp.IsEmpty) and (TabSheet2.Showing) then
  begin
    if not Assigned(RelArquivoRetornoForm) then
      RelArquivoRetornoForm := tRelArquivoRetornoForm.Create(Application);
    try
      RelArquivoRetornoForm.lbSituacao5.Caption := 'Arquivo Recebido';
      // 'Relatório Geral de Arquivo Retorno';
      RelArquivoRetornoForm.lbPeriodo5.Caption := 'Data do Arquivo: ' +
        FormatDateTime('dd/mm/yyyy', RetornoForm.RetornoTmpDATA_ARQUIVO.Value);
      // +' - Cedente: '+BancodeDados.RetornoNOME_CEDENTE.Value;
      ...
      if not BancodeDados.Bancos.IsEmpty then
        RelArquivoRetornoForm.lbArquivo5_1.Caption := 'Banco: ' +
          BancodeDados.BancosBANCO_NOME.Value + ' | Agência: ' +
          (RetornoForm.RetornoTmpCODIGO_AGENCIA.Value) + '' +
          (RetornoForm.RetornoTmpDIGITO_AGENCIA.Value) + ' | Conta: ' +
          (RetornoForm.RetornoTmpNUMERO_CONTA.Value) + '' +
          IntToStr(RetornoForm.RetornoTmpDIGITO_CONTA.Value);
      // else
      ...
      // RelArquivoRetornoForm.lbTipoQuitacao1.Caption :=TipoQuitacao;
      // BancodeDados.PrepararRel(RelArquivoRetornoForm.RLReport1);
      BancodeDados.PrepararRel(RelArquivoRetornoForm.RLReport5, false, nil,
        chkMala.Checked);
    finally
      RelArquivoRetornoForm.Release;
      RelArquivoRetornoForm := nil;
...
begin // PopupMenu1
  ...
  if (trim(BancodeDados.RetornoSEU_NUMERO.Value) <> '') or
    (trim(BancodeDados.RetornoNOSSO_NUMERO.Value) <> '') then
  begin
        ...
        begin
          ...
          if BancodeDados.RetornoVALOR_DOCUMENTO.Value > 0 then
            BancodeDados.Emissoes.SQL.Add(' AND VALOR_DOC =' +
              CurrToStr(BancodeDados.RetornoVALOR_DOCUMENTO.Value));
          FormatSettings.DecimalSeparator := ',';
        end;
      end
      else // 2
    ...
    begin
      if trim(BancodeDados.RetornoSEU_NUMERO.Value) <> '' then
        Mensagem('Não foi encontrada nenhuma emissão referente a este retorno.'#13
          + 'Emissão: ' + (BancodeDados.RetornoSEU_NUMERO.Value), mtInformation,
          [mbOk], mrOk, 0)
      else
    ...
    begin
      ...
      BancodeDados.Boletos.SQL.Text := PrincipalForm.SQLrepositorio(0) +
        ' and b.emissao_id=' + IntToStr(BancodeDados.EmissoesEMISSAO_ID.Value);
      ...
      begin
        try
          ...
          begin
            ...
            LimpoNome := StringReplace('|' + LimparNome, '-', '|',
            ...
            Memo1.Lines.Add('CLIENTE: ' +
              AnsiUpperCase(qryTitulosCLIENTE_NOME.Value));
            Memo1.Lines.Add('PARCELA: ' + qryTitulosPARCELA.Value);
            Memo1.Lines.Add('DATA DE RECEBIDO: ' +
              DateToStr(qryTitulosDATA_RECEBIMENTO.Value));
            Memo1.Lines.Add('VALOR PAGO R$: ' + FormatFloat(',0.00',
              qryTitulosVALOR_PAGO.Value));
            // Memo1.Lines.Add('ALUNO(S): ');

            for i := 0 to NmAlunos.Count - 1 do
            begin
              Memo1.Lines.Add(NmAlunos[i]);
            end;

            Mensagem('O(s) título(s) (' +
              (BancodeDados.EmissoesID_BOLETOS.Value) +
              ') referente(s) a este retorno não foi(foram) encontrado(s).'#13 +
              'Dados do boleto:'#13#13 + Memo1.Lines.Text + #13 +
              'Possivelmente o(s) título(s) foi(foram) excluído(s), após a emissão do boleto em '
              + DateToStr(qryTitulosEMISSAO.Value) + '.', mtInformation,
              [mbOk], mrOk, 0);
            NmAlunos.Free;
          end
          else
          begin
            if trim(BancodeDados.RetornoSEU_NUMERO.Value) <> '' then
              Mensagem('Não foi encontrada nenhuma emissão referente a este retorno.'#13
                + 'Emissão: ' + (BancodeDados.RetornoSEU_NUMERO.Value),
                mtInformation, [mbOk], mrOk, 0)
            else
              Mensagem('Não foi encontrada nenhuma emissão referente a este retorno.',
                mtInformation, [mbOk], mrOk, 0);
...
begin // PopupMenu2
  ...
  if (trim(RetornoForm.RetornoTmpSEU_NUMERO.AsString) <> '') or
    (trim(RetornoForm.RetornoTmpNOSSO_NUMERO.AsString) <> '') then
  begin
    podeIr := false;
      ...
      begin // 2
        ...
        begin
          ...
          if RetornoForm.RetornoTmpVALOR_DOCUMENTO.Value > 0 then
            BancodeDados.Emissoes.SQL.Add(' AND VALOR_DOC =' +
              CurrToStr(RetornoForm.RetornoTmpVALOR_DOCUMENTO.Value));
          FormatSettings.DecimalSeparator := ',';
        end;
      end
      else // 2
    ...
    begin
      if trim(RetornoForm.RetornoTmpSEU_NUMERO.Value) <> '' then
        Mensagem('Não foi encontrada nenhuma emissão referente a este retorno.'#13
          + 'Emissão: ' + (RetornoForm.RetornoTmpSEU_NUMERO.Value),
          mtInformation, [mbOk], mrOk, 0)
      else
        Mensagem('Não foi encontrada nenhuma emissão referente a este retorno.',
          mtInformation, [mbOk], mrOk, 0)
...
begin
  if BancodeDados.Retorno.RecordCount > 0 then
  begin
