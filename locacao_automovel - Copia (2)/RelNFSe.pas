...
procedure TRelNFSeForm.RLReport1BeforePrint(Sender: TObject;
  var PrintIt: Boolean);
begin
...
procedure TRelNFSeForm.RLGroup1BeforePrint(Sender: TObject;
  var PrintIt: Boolean);
begin
  lbTotalNota.Caption := 'VALOR TOTAL DA NOTA = ' + FormatFloat('R$ ,0.00',
    qryNFSeVL_TOTAL.Value);
  if (qryNFSeNFSE_AMBIENTE.Value = 1) then
    bandHomologacao.Visible := false
  else
    bandHomologacao.Visible := true;
  memoServicos.Lines.Text := Bancodedados.PegaListaServicos
    (qryNFSeNFS_ID.Value);
end;
