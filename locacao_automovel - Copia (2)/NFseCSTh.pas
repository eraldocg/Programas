...
begin
 ...
 // PrincipalForm.StatusBar1.Panels[3].Text := ''; //Aguardando Proces
  ...
  //begin
    ...
    PrincipalForm.CsNFSe.SQL.Text:='select nfs_id, rps_protocolo, sit, serie,vl_total, nfse_numero, nfse_cod_verif, nfse_dt_emissao, xml_nfse, nfse_ambiente from nfs where sit=3';

    if Assigned(CsNfsForm) then
    begin
    if not BancodeDados.Nfs.IsEmpty then
    PrincipalForm.CsNFSe.SQL.add(' and nfs_id = '+IntToStr(BancodeDados.NfsNFS_ID.Value));
    end;
    ...
    begin
      ...
      begin
        ...
        begin
          Inc(J);
          PrincipalForm.StatusBar1.Panels[4].Text := 'Nfse Atual: '+ FormatFloat('000000',PrincipalForm.CsNFSeNFS_ID.Value);
          PrincipalForm.StatusBar1.Panels[5].Text := 'Processo: '+IntToStr(J)+'/'+IntToStr(Nnfse);

          ObterDadosdaNFSeNFseCSTh(PrincipalForm.CsNFSeNFS_ID.AsString, PrincipalForm.CsNFSeSERIE.AsString);
...
begin
  ...
  try
    ...
    begin
     if not BancodeDados.Config.Active then BancodeDados.Config.Open;
      ...
      if (nfse.Certificado.SelecionaCertificadoNroSerie(Trim(BancodeDados.ConfigNFSE_CERT_DIG_NUM_SERIE.Value)+';'+RetornaNumeros(BancodeDados.ConfigCIDADE.AsString)+';'))  then
      begin

        nfse.Emitente.CNPJ          := BancodeDados.ConfigCNPJ.AsString;
        nfse.Emitente.InscMunicipal := RetornaNumeros(BancodeDados.ConfigINSC_MUNIC.Value);
  ...
  begin
   ...
   PrincipalForm.StatusBar1.Panels[5].Text := 'Erro ao obter dados da nota.' + #13 + E.Message;
  end;
  end;
