...
begin
  ...
  begin
    ...
    lbContadorN.Caption := 'Registro(s) Encontrado(s): ' +
      FormatFloat('000000', BancodeDados.Contatos.RecNo);
  end
  else
    lbContadorN.Caption := 'Registro(s) Encontrado(s): 000000';
end;

procedure TCsContatosForm.DBGrid1DrawColumnCell(Sender: TObject;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          if not(BancodeDados.Contatos.State in [dsEdit]) then
            BancodeDados.Contatos.Edit;
          BancodeDados.MarcarAtivo(listaContatos,
            BancodeDados.ContatosCONTATO_ID.Value, BancodeDados.ContatosATIVO);
        end;
...
procedure TCsContatosForm.DBGrid1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key = 32 then
  ...
  Try
    ...
    begin
      ...
      if ckPeriodo.Checked then
        SQL.Add(' and ( dt_cadastro between ' +
...
procedure TCsContatosForm.EditConsultaKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
      ...
      if not(BancodeDados.Contatos.State in [dsEdit, dsinsert]) then
        ...
        begin
          ...
          begin
            ...
            if (listaContatos.IndexOf(BancodeDados.ContatosCONTATO_ID.AsString)
              >= 0) then
            begin
...
begin
  if not(listaContatos.Count > 0) then
    ...
    try
      ...
      RelContatosForm.Contatos.SQL.Text :=
        'select * from contatos where contato_id in (' + IDs + ')';
