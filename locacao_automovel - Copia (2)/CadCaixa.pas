...
procedure TCadCaixaForm.DBGrid1KeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  if Key = 32 then
  begin
    Key := 0;
...
//      Delete(condicao, Pos('ORDER', condicao), length(condicao));
      ...
      RelLivroCaixaForm.lbDinheiro.Caption := 'Dinheiro:  ' + FormatFloat(',0.00', RelLivroCaixaForm.QryTotaisTOTAL.Value);

      RelLivroCaixaForm.QryTotais.close;
      RelLivroCaixaForm.QryTotais.sql.Text := 'select sum(coalesce(l.entrada,0)-coalesce(l.saida,0)) as TOTAL from livro_caixa l where l.descricao_pago=' + QuotedStr('Cheque') + ' and ' + condicao;
      RelLivroCaixaForm.QryTotais.Open;
      RelLivroCaixaForm.lbCheque.Caption := 'Cheque:  ' + FormatFloat(',0.00', RelLivroCaixaForm.QryTotaisTOTAL.Value);

      RelLivroCaixaForm.QryTotais.close;
      RelLivroCaixaForm.QryTotais.sql.Text := 'select sum(coalesce(l.entrada,0)-coalesce(l.saida,0)) as TOTAL from livro_caixa l where l.descricao_pago=' + QuotedStr('Cartao') + ' and ' + condicao;
      //('Cartao:  ' +RelLivroCaixaForm.QryTotais.sql.Text);
      RelLivroCaixaForm.QryTotais.Open;
      RelLivroCaixaForm.lbCartao.Caption := 'Cartao:  ' + FormatFloat(',0.00', RelLivroCaixaForm.QryTotaisTOTAL.Value);

      RelLivroCaixaForm.QryTotais.close;
      RelLivroCaixaForm.QryTotais.sql.Text := 'select sum(coalesce(l.entrada,0)-coalesce(l.saida,0)) as TOTAL from livro_caixa l where l.descricao_pago=' + QuotedStr('Negociado') + ' and ' +
        condicao;
//      ('Negociado:  ' +RelLivroCaixaForm.QryTotais.sql.Text);
      RelLivroCaixaForm.QryTotais.Open;
      RelLivroCaixaForm.lbNegociado.Caption := 'Negociado:  ' + FormatFloat(',0.00', RelLivroCaixaForm.QryTotaisTOTAL.Value);

      RelLivroCaixaForm.QryTotais.close;
      RelLivroCaixaForm.QryTotais.sql.Text := 'select sum(coalesce(l.entrada,0)-coalesce(l.saida,0)) as TOTAL from livro_caixa l where l.descricao_pago=' + QuotedStr('Boleto') + ' and ' + condicao;
//      ('Boleto:  ' +RelLivroCaixaForm.QryTotais.sql.Text);
      RelLivroCaixaForm.QryTotais.Open;
      RelLivroCaixaForm.lbBoleto.Caption := 'Boleto:  ' + FormatFloat(',0.00', RelLivroCaixaForm.QryTotaisTOTAL.Value);

      RelLivroCaixaForm.QryTotais.close;
      RelLivroCaixaForm.QryTotais.sql.Text := 'select sum(coalesce(l.entrada,0)-coalesce(l.saida,0)) as TOTAL from livro_caixa l where l.descricao_pago=' + QuotedStr('Outros') + ' and ' + condicao;
...
procedure TCadCaixaForm.DBGrid1DrawColumnCell(Sender: TObject; const Rect: TRect; DataCol: Integer; Column: TColumn; State: TGridDrawState);
begin
  ...
  begin
    ...
    if (BancodeDados.Livro_CaixaBOLETO_ID.Value > 0) { or (BancodeDados.Livro_CaixaPAGAR_ID.Value>0) } or (BancodeDados.Livro_CaixaCHEQUE_ID.Value > 0) or
...
begin
  ...
  begin
    ...
    begin
      if not (BancodeDados.Livro_Caixa.State in [dsEdit]) then  BancodeDados.Livro_Caixa.Edit;
      BancodeDados.MarcarAtivo(listaCaixa, BancodeDados.Livro_CaixaCAIXA_ID.Value, BancodeDados.Livro_CaixaATIVO);
    end;
  end;
...
begin
  ...
  Try
    ...
    begin
      Close;
      SQL.Clear;
      SQL.Text:= PrincipalForm.SQLrepositorio(2) + ' and (l.data between ' + QuotedStr(FormatDateTime('yyyy/mm/dd', Dt_Inicial.DateTime)) + ' and ' + QuotedStr(FormatDateTime('yyyy/mm/dd', Dt_Final.DateTime)) + ')';
      ...
      SQL.Add(' order by l.data desc, l.caixa_id');
      Open;
...
begin
  ...
  begin
    ...
    try
      ...
      begin
        if (listaCaixa.IndexOf(BancodeDados.Livro_CaixaCAIXA_ID.AsString) >= 0) then
        begin
...
procedure TCadCaixaForm.EditConsultaKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
