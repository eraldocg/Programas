...
if not (EditGrade.Value>0) then
begin
  HabilitarBotoes(Self,false);
  Mensagem('Informe uma grade financeira válida.', mtInformation, [mbOk], mrok, 0)  ;
  HabilitarBotoes(Self,true);
  EditGrade.SetFocus;
  Validar:=false;
end else
if not (EditVlMensal.Value>0) then
begin
  HabilitarBotoes(Self,false);
...
begin
  ...
  begin
    inc(ivencimento);
    Temp.Append;
    TempPARCELA.Value := FormatFloat('00', i) + '/' + FormatFloat('00', qtdMaior);
    ...
    begin
      if Trim(TempOBS.AsString) = '' then
        TempOBS.AsString := 'Adesão: ' + FormatFloat('R$ ,0.00', vlLic[i]) { +', parcela ' + FormatFloat('00', i) + '/' + FormatFloat('00', qtdLic) }
      else
        TempOBS.AsString := TempOBS.AsString + #13 + 'Adesão: ' + FormatFloat('R$ ,0.00', vlLic[i]);
    ...
    begin
      if Trim(TempOBS.AsString) = '' then
        TempOBS.AsString := 'Mensalidade: ' + FormatFloat('R$ ,0.00', vlMensal[i]) { +', parcela ' + FormatFloat('00', i) + '/' + FormatFloat('00', qtdMensal) }
      else
        TempOBS.AsString := TempOBS.AsString + #13 + 'Mensalidade: ' + FormatFloat('R$ ,0.00', vlMensal[i]);
...
begin
  if Temp.IsEmpty then
    Mensagem('Não há informações para serem geradas.', mtInformation, [mbOk], mrok, 0)
  else if (Editbanco.Value <= 0) or (EditGrade.Value <= 0) then
    Mensagem('Banco ou Grade Financeira não encontrado(a)!', mtInformation, [mbOk], mrok, 0)
  else
  begin
    ...
    begin
      ...
      BancodeDados.BoletosVALOR_APAGAR.Value  := TempVALOR_MENSAL.Value + TempVALOR_LIC.Value;
...
begin
  ...
  begin
    ...
    Temp.Edit;
    TempPARCELA.Value := FormatFloat('00', Temp.RecNo) + '/' + FormatFloat('00', Temp.RecordCount);
    Temp.Post;
    Temp.Next;
  end;
end;
...
begin
  if Mensagem('Deseja excluir?', mtConfirmation, [mbYes, mbNo], mrNo, 0) = idNo then
    abort;
end;
