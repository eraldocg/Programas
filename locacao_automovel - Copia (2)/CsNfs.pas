...
begin
  ...
  begin
    ...
    lbContadorN.Caption := 'Registro(s) Encontrado(s): ' +
      FormatFloat('000000', BancodeDados.Nfs.RecordCount) +
      '   -   Valor Total: ' + FormatFloat('R$ ,0.00', vlTotalTmp);
...
begin
  ...
  if ModificarIDdoRegistro1.Enabled then
    ...
    begin
      ...
      if Mensagem('Tem certeza que deseja modificar o ID deste registro?',
      ...
      begin
        ...
        // begin
        if BancodeDados.Nfs.Locate('NFS_ID', BancodeDados.NfsNFS_ID.Value, [])
        then
        begin
          Reabrir;
          ...
          begin
            ...
            try
              ...
              BancodeDados.Adiciona.SQL.Text := 'update nfs set nfs_id = ' +
                IntToStr(idNovo) + ' where nfs_id = ' + IntToStr(idAnt);
              BancodeDados.Adiciona.ExecSQL;

              BancodeDados.Adiciona.SQL.Text := 'update nfs_serv set nfs_id = '
                + IntToStr(idNovo) + ' where nfs_id = ' + IntToStr(idAnt);
              BancodeDados.Adiciona.ExecSQL;
              ...
              Mensagem('ID alterado com sucesso!', mtInformation,
                [mbOk], mrOk, 0);
              // BtExecutarClick(nil);
          ...
          begin
            Mensagem('Só é possível modificar o ID de notas com a situação ''Aberta'' ou ''Fechada''.',
...
procedure TCsNfsForm.SedDBGrid1DrawColumnCell(Sender: TObject;
...
begin
  ...
  begin
    ...
    begin // 0 aberta
      ...
      SedDBGrid1.DefaultDrawDataCell(Rect, SedDBGrid1.Columns[DataCol]
        .Field, State);
    end;
    ...
    begin
      ...
      DrawText(SedDBGrid1.Canvas.Handle, pchar(BancodeDados.NfsOBS2.AsString),
        length(BancodeDados.NfsOBS2.AsString), R, DT_WORDBREAK);
    end;
...
begin
  ...
  try
    ...
    RelNFSeForm.qryNFSe.SQL.Add(' and n.nfs_id = ' +
...
begin
  if (BancodeDados.NfsSIT.Value = 0) then
    ...
    Mensagem('Esta nota não pode ser reaberta pois já foi cancelada.',
      mtInformation, [mbOk], mrOk, 0)
  else if (BancodeDados.NfsNFSE_NUMERO.Value > 0) then
    Mensagem('Esta nota não pode ser reaberta pois já foi enviada.',
      mtInformation, [mbOk], mrOk, 0)
  else
  begin
    if PrincipalForm.LiberaAcesso then
    begin
      if Mensagem('Deseja reabrir esta nota agora?', mtConfirmation,
...
begin
  ...
  begin
    ...
    if not(BancodeDados.Nfs.IsEmpty) then // se a consulta houve retorno
      if Mensagem('Atualizar nota(s) com a situação aguardando processamento?',
        mtConfirmation, [mbYes, mbNo], mrYes, 0) = idYes then
      begin
        // HabilitarBotoes(Self,false);
        ...
        begin
          ...
          if (BancodeDados.NfsSIT.Value = 3) then
          // se falta os dados da nota ou seja foram processados e esta aguardando
          begin
            ListBox1.Items.Add(BancodeDados.NfsNFS_ID.AsString + ',' +
              BancodeDados.NfsSERIE.AsString); // adiciona dados na lista
        ...
        // faz uma contagem percorrendo os itens adicionado na lista com rps e serie
        begin
          txt := ListBox1.Items.Strings[i];
          // captura o item selecionado pelo cursor
          Rps := Copy(txt, 1, Pos(',', txt) - 1);
          // pega somente o rps antes da virgula
          serie := Copy(txt, length(Rps) + 2, length(txt));
          // pega a serie antes da virgula
          // ('Rps '+rps+'serie '+serie);
          // Sleep(2000);

          BancodeDados.ObterDadosdaNFSe(Rps, serie);
          // atualiza os dados da nota no banco de dados caso retornar sim!
        end;
  ...
  begin
    ...
    case cbTipoData.ItemIndex of
      1:
        TipoData := 'n.dt_rps';
      2:
        TipoData := 'n.nfse_dt_emissao';
      3:
        TipoData := 'n.dt_fechada';
      4:
        TipoData := 'n.dt_enviada';
    end;
    ...
    Try
      ...
      begin
        ...
        SQL.Clear;
        SQL.Text :=
          'select n.*, c.nome, c.cnpj, c.logradouro, c.bairro, c.cidade, c.estado, c.email_contato2,  m.nome as cidade_nome, m.uf as uf_nome '
        ...
        if cbTipoData.ItemIndex <> 0 then
          SQL.Add(' and ( cast(' + TipoData + ' as date) between ' +
...
procedure TCsNfsForm.EditConsultaKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
begin
  ...
  try
    ...
    begin
      ...
      Arquivo := DiretorioTemp + 'SEDSoft_NFSe_' +
        IntToStr(RelNFSeForm.qryNFSeNFSE_NUMERO.Value) + '.pdf';
      ...
      memoCorpo.Lines.Add('');
      memoCorpo.Lines.Add
        ('Você está recebendo o aviso de Emissão da Nota Fiscal de Serviço Eletrônica Nº. '
        + IntToStr(RelNFSeForm.qryNFSeNFSE_NUMERO.Value));
      memoCorpo.Lines.Add('');
      memoCorpo.Lines.Add
        ('Emitida pelo prestador(a) de serviços: SED SOFT ASSESSORIA EM INFORMÁTICA LTDA');
      ...
      memoCorpo.Lines.Add('');
      memoCorpo.Lines.Add
        ('Para confirmar a autenticidade desta nota fiscal acesse o portal da Prefeitura Municipal de Campina Grande');
      ...
      BancodeDados.EnviarEmail(Trim(RelNFSeForm.qryNFSeEMAIL_CONTATO2.Value),
        'SED Soft - Nota Fiscal Eletrônica No ' +
        IntToStr(RelNFSeForm.qryNFSeNFSE_NUMERO.Value),
        memoCorpo.Lines.Text, Arquivo);
...
begin
  ...
  begin
    ...
    begin
      if (Trim(BancodeDados.NfsEMAIL_CONTATO2.Value) <> '')
      { or (Trim(BancodeDados.NfsEMAIL_CONTATO2.Value) <> '') } then
      begin
        if Mensagem
          ('Tem certeza que deseja enviar uma cópia da NFSe para este cliente?',
        ...
        Mensagem('Este cliente não possui email cadastrado.', mtWarning,
          [mbOk], mrOk, 0);
    end
    else
      Mensagem('Esta nota ainda não foi enviada para a prefeitura.',
        mtInformation, [mbOk], mrOk, 0);
  end;
end;
...
begin
  ...
  try
    RelNfsForm.lbPeriodo.Caption := 'Período: ' + Dt_Inicial.DateTimeStr + ' a '
      + Dt_Final.DateTimeStr;
...
begin
  ...
  try
    ...
    if (CadNfsForm.ShowModal = mrOk) then
      if (BancodeDados.NfsSIT.Value <> 1) then
      begin
        if Mensagem('Deseja fechar esta nota agora?', mtConfirmation,
          [mbYes, mbNo], mrYes, 0) = idYes then
          BancodeDados.FecharNota;
      end;
  finally
...
begin
  if Mensagem
    ('Atenção! Somente serão fechadas as notas listadas que estiverem na situação aberta (Preto) e que possuírem CNPJ.'
    + #13 + 'Deseja prosseguir?', mtConfirmation, [mbYes, mbNo], mrYes, 0) = idYes
  then
  begin
    ...
    begin
      if (BancodeDados.NfsSIT.Value = 0) and
        (Trim(BancodeDados.NfsCNPJ.Value) <> '') then
      begin
        BancodeDados.FecharNota;
      end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        BancodeDados.Nfs.First;
        while not BancodeDados.Nfs.Eof do
        // Added by Denilson SED 12/12/2019 14:29:15
        begin
          if (BancodeDados.NfsSIT.Value = 3) or (BancodeDados.NfsSIT.Value = 1)
          then // reabrindo todas as notas fiscais na situacao fechada
          begin
            BancodeDados.Nfs.Edit;
            BancodeDados.NfsSIT.Value := 0;
            BancodeDados.NfsRPS_PROTOCOLO.Value := '';
...
begin
  ...
  begin
    ...
    if MensagemChk('Deseja enviar está nota agora?                            ',
      mtConfirmation, [mbYes, mbNo], mrYes, 0, True,
      'Enviar um e-mail para o cliente com uma cópia da NFSe?', envEmail) = idYes
    then
      podePassar := True
    else
      podePassar := false;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          podePassar := false;
          Mensagem('Este cliente não possui e-mail cadastrado.', mtWarning,
            [mbOk], mrOk, 0);
        end;
      end;
    end;
...
begin
  ...
  // PrincipalForm.AjustaHora;
  if MensagemChk
    ('Deseja enviar todas as notas listadas?                        ',
    mtConfirmation, [mbYes, mbNo], mrYes, 0, True,
  ...
  begin
    ...
    begin
      ...
      if (BancodeDados.NfsSIT.Value in [1, 3]) and
        (not(BancodeDados.NfsNFSE_NUMERO.Value > 0) and
        (Trim(BancodeDados.NfsRPS_PROTOCOLO.Value) = '')) then
      begin
        podePassar := True;
        if (envEmail) then
        begin
...
begin
  ...
  if (BancodeDados.NfsSIT.Value <> 2)
  { and (BancodeDados.NfsNFSE_NUMERO.Value > 0) } then
  begin
