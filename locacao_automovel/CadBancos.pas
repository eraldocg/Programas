  ...
  begin
    ...
    try
      ...
      begin
        ...
        if trim(BancodeDados.BancosCONTA_CORRENTE.Value)<>'' then begin
          HabilitarBotoes(self,false);
...
begin
  if not(trim(BancodeDados.BancosBANCO_NOME.Value) = '') then
    CadBancosForm.Caption := 'Cadastro de Banco - ' + BancodeDados.BancosBANCO_NOME.Value
  else
...
begin
  ...
  if Mensagem(Msg+'Tem certeza que deseja sair?', mtConfirmation, [mbyes, mbNo], mryes, 0) = idNo then
    CanClose := false;
end;
...
begin
  if PrincipalForm.OpenPictureDialog1.Execute then
    if BancodeDados.Bancos.State in [dsInsert, dsEdit] then
      PrincipalForm.CarregaFotoParaBanco(PrincipalForm.OpenPictureDialog1.FileName);
end;
...
if (BancodeDados.BancosCODIGO_BANCO.Value = 748) then
    Dadosnecessriosparageraodeboletos2.Enabled:=true else Dadosnecessriosparageraodeboletos2.Enabled:=false;
...
begin
if (BancodeDados.BancosCODIGO_BANCO.Value = 748) then
    DadosPgeracaoBoleto1.Enabled:=true else DadosPgeracaoBoleto1.Enabled:=false;
...
begin
  ...
  begin
    if btCamRem.Enabled then
      ...
      begin
        HabilitarBotoes(self, false);
        caminho := SelecionarDiretorio('Arquivo de retorno', BancodeDados.BancosCAMINHO_RETORNO.Value);
...
WinExec(PAnsiChar(AnsiString('explorer.exe /n,/e, '+BancodeDados.BancosCAMINHO_RETORNO.Value)),SW_SHOWNORMAL);
end;

procedure TCadBancosForm.EditCamRemDblClick(Sender: TObject);
begin
if trim(BancodeDados.BancosCAMINHO_REMESSA.Value)<>'' then
WinExec(PAnsiChar(AnsiString('explorer.exe /n,/e, '+BancodeDados.BancosCAMINHO_REMESSA.Value)),SW_SHOWNORMAL);
end;
...
begin
  if BancodeDados.BancosBANCO_ID.Value > 0 then
    ...
    begin
      if not(BancodeDados.Bancos.State in [dsinsert, dsedit]) then
        BancodeDados.Bancos.Edit;
      if BancodeDados.ResizeImage(PrincipalForm.OpenPictureDialog1.FileName, 100) then
        BancodeDados.GravarBlobNaTabela(BancodeDados.BancosLOGOMARCA, DiretorioTemp + ExtractFileName(PrincipalForm.OpenPictureDialog1.FileName));
...
procedure TCadBancosForm.DBNavigator1Click(Sender: TObject; Button: TNavigateBtn);
begin
  FormShow(Sender);
end;
...
begin
  ...
  begin
    if btCamRem.Enabled then
      ...
      begin
        HabilitarBotoes(self, false);
        caminho := SelecionarDiretorio('Arquivo de remessa', BancodeDados.BancosCAMINHO_REMESSA.Value);
        if (trim(caminho) <> '') then
        begin
          if not(BancodeDados.Bancos.State in [dsinsert, dsedit]) then
            BancodeDados.Bancos.Edit;
