...
type
  ...
  public
    { Public declarations }
    procedure PegaArquivos(Diretorio,mascara, codCed, codConv, CodAg, CodConta : string; Banco, LayOut : integer; CaminhoCompleto : Boolean);
  end;
...
begin
  if not (Copy(diretorioInicial, Length(diretorioInicial), 1)='\') then diretorioInicial:=diretorioInicial + '\';
...
procedure TListarRetornoForm.PegaArquivos(Diretorio,mascara, codCed, codConv, CodAg, CodConta : string; Banco, LayOut : integer; CaminhoCompleto : Boolean);
var
i : integer;
mascTmp : String;
  ...
  try
    ...
    try
      if (Banco = 1) then begin
        if (LayOut=1) or (LayOut=2) then
        CodCedente:=IntToStr(StrToInt64(Trim(copy(Linha,32,8)))) else
        if (LayOut=3) then
        CodConvenio:=Trim(copy(Linha,33,20));
      end else

      if (Banco = 104) then begin
        if (LayOut=1) or (LayOut=2) then
        CodCedente:=IntToStr(StrToInt64(Trim(copy(Linha,27,16)))) else
        if (LayOut=3) then begin
          //CodCedente:=Trim(copy(Linha,33,16));
          if Length(codCed)<=6 then begin
      ...
      if (Banco = 356) then begin
        Agencia:=Copy(Linha, 28, 4);
        Conta:=Copy(Linha, 33, 7);
      end else

      if (Banco = 409) then begin
        Agencia:=Copy(Linha, 27, 4);
    ...
    if (Banco = 1) then begin
      if (LayOut=1) or (LayOut=2) then begin
        if CodCedente = codCed then result:=true;
        
      end else
      if (LayOut=3) then begin
        if Pos(codConv, CodConvenio)>0 then Result:=true;
      end;
    end else
    ...
    if (Banco = 341) then begin
      if (Agencia = CodAg) and (Conta = CodConta) then Result:=True;
    end else
    ...
    if (Banco = 237) then begin
      ...
      //if CodCedente = codCed then result:=true;
      if (CodConvenio = codConv) then Result:=True;
    end else

    if (Banco = 399) then begin
      if (Pos(CodAg, Agencia)>0) and (Pos(CodConta, Conta)>0)  then Result:=True;
    end else
...
begin
if not (Copy(Diretorio, Length(Diretorio), 1)='\') then Diretorio:=Diretorio + '\';
with Temp do begin
...
while Length(mascTmp)>0 do begin
  if pos(';', mascTmp)>0 then begin
    ListarArquivos(Diretorio, Copy(mascTmp, 1, pos(';', mascTmp)-1), true, false);
...
if listtemp2.Count>0 then begin
  for i:=0 to listtemp2.Count - 1 do begin
    Temp.Append;
  ...
  while not Temp.Eof do begin
    ...
    Temp2ARQUIVO.Value:=TempARQUIVO.Value else
    Temp2ARQUIVO.Value:=Copy(TempARQUIVO.Value, Length(Diretorio)+1, Length(TempARQUIVO.Value));
    ...
    Temp2TIPO.Value:=1 else Temp2TIPO.Value:=0;
    Temp2.Post;
    Temp.Next;
  end;

  ListarRetornoForm.Caption:='Total de arquivos: ' + FormatFloat('000', Temp2.RecordCount);
  Temp2.First;
end;
listtemp2.Free;
end;
...
if key = 32 then begin
  key:=0;
  if not Temp2.IsEmpty then begin
    Temp2.Edit;
    Temp2ATIVO.AsBoolean:= not Temp2ATIVO.AsBoolean;
    Temp2.Post;
  end;
