...
begin
  res := Mensagem
    ('Sua conexão com o banco de dados foi perdida. Deseja conectar?',
...
begin
  ...
  begin
    ...
    try
      ...
      on E: Exception do
      begin
        MessageDlg('Erro ao carregar o código: ' + #13 + #13 +
...
begin
  case cod of
    00:
      Result := 'Débito efetuado';
    01:
      Result := 'Débito não efetuado - Insuficiência de fundos';
    02:
      Result := 'Débito não efetuado - Conta não cadastrada';
    04:
      Result := 'Débito não efetuado - Outras restrições';
    05:
      Result := 'Débito não efetuado – valor do débito excede valor limite aprovado.';
    10:
      Result := 'Débito não efetuado - Agência em regime de encerramento';
    12:
      Result := 'Débito não efetuado - Valor inválido';
    13:
      Result := 'Débito não efetuado - Data de lançamento inválida';
    14:
      Result := 'Débito não efetuado - Agência inválida';
    15:
      Result := 'Débito não efetuado - conta inválida';
    18:
      Result := 'Débito não efetuado - Data do débito anterior à do processamento';
    30:
      Result := 'Débito não efetuado - Sem contrato de débito automático';
    31:
      Result := 'Débito efetuado em data diferente da data informada – feriado na praça de débito';
    96:
      Result := 'Manutenção do Cadastro';
    97:
      Result := 'Cancelamento - Não encontrado';
    98:
      Result := 'Cancelamento - Não efetuado, fora do tempo hábil';
    99:
      Result := 'Cancelamento - cancelado conforme solicitação';
  end;
...
begin
  ...
  if (ClientesCONFIAVEL.Value = 1) and (ClientesSIT.Value = 1) then
    ...
    ClientesConfiavelNome.Value := 'NÃO';
...
begin
  ...
  if LigacoesSIT.Value = 4 then
    LigacoesSituacao_Nome.Value := 'NÃO ATENDIDO';
  ...
  if LigacoesTIPO.Value = 1 then
    ...
    LigacoesTipo_Nome.Value := 'COBRANÇA'
...
begin
  ...
  if BancodeDados.LoginGERA_NFSE.Value > 0 then
    ...
    BancodeDados.LoginNfseGerar.AsString := 'NÃO';
...
begin
  ...
  begin
    ...
    RetornoTipoQuitacao.Value := 'Retorno Cobrança';
...
begin
  ...
  begin
    Mensagem('Informe um nome válido para o documento.', mtInformation,
      [mbOk], mrOk, 0);
    Abort;
  end;
end;
    ...
    if Boletos.Active then
      ...
      begin
        ...
        begin
          Result := false;
          RecBol.Close;
          Mensagem('Não foi encontrato nenhum título a receber.', mtInformation,
            [mbOk], mrOk, 0);
        end;
...
begin
  ...
  begin
    Abort;
    Mensagem('Cliente não informado. Operação cancelada.', mtInformation,
      [mbOk], mrOk, 0);
  end;
  ...
  begin
    Abort;
    Mensagem('Chave serial inválida. Operação cancelada.', mtInformation,
      [mbOk], mrOk, 0);
  end;
...
begin
  iRes := MessageDlg
    ('A conexão está perdida. Off-line: sim, tente novamente: ok, falhe: cancele.',
    ...
    try
      ...
      begin
        ...
        begin
          ...
          begin
            (ComponenteTmp as TRLLabel).Caption := 'Endereço: ' +
...
begin
  ...
  begin
    Mensagem('Informe a quantidade do serviço.', mtInformation, [mbOk],
      mrOk, 0);
    Abort;
  end;
  if not(NfsServVL_UNIT.Value > 0) then
  begin
    Mensagem('Informe o valor unitário do serviço.', mtInformation,
      [mbOk], mrOk, 0);
    Abort;
  end;
...
begin
  ...
  begin
    ...
    if oExc.Kind = ekRecordLocked then
      oExc.Message :=
        'Por favor, tente a operação mais tarde. No momento, o registro está ocupado'
    else if (oExc.Kind = ekUKViolated) and SameText(oExc[0].ObjName,
      'UniqueKey_Orders') then
      oExc.Message :=
        'Forneça as informações solicitadas. Parece que o registro já foi feito';
  end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          Mensagem('Configuração de email incompleta. Informe o usuário e a senha.',
            mtWarning, [mbOk], mrOk, 0);
      end
      else
        Mensagem('Configuração de email incompleta. Informe o servidor SMTP e a porta de conexão.',
          mtWarning, [mbOk], mrOk, 0);
    end
    else
      Mensagem('Configuração de email incompleta. Informe o nome do remetente.',
        mtWarning, [mbOk], mrOk, 0);
  except
    On E: Exception do
    begin
      ...
      then
        Mensagem('O servidor demorou a responder, verifique se a porta para conexão está correta.',
          mtWarning, [mbOk], mrOk, 0)

      else if (pos(UpperCase('read timeout'), UpperCase(E.Message)) > 0) then
        Mensagem('O servidor demorou a responder, talvez o mesmo requeira uma Conexão SSL segura.',
          mtWarning, [mbOk], mrOk, 0)

      else if (pos(UpperCase('connection closed'), UpperCase(E.Message)) > 0)
      then
        Mensagem('A conexão foi encerrada pelo servidor.', mtWarning,
          [mbOk], mrOk, 0)
...
begin
  ...
  try
          ...
          begin
            ...
            begin
              ...
              try
                  ...
                  begin
                    ...
                    begin // Já tinha sido enviado e esta esperando processamento
                      ...
                      try
                        case nfse.AnalisarRespostaEnvio
                          ...
                          3:
                            begin
                              ...
                              if (pos('RPS já informado', String(nfse.msgErro)
                                ) > 0) then
                              begin
                                  ...
                                  begin
                                    ...
                                    begin
                                      ...
                                      if Trim(NfsOBS2.AsString) = '' then
                                        ...
                                        ') está diferente do valor informado pelo servidor '
                                        + FormatFloat(',0.00',
                                        vlNotaServdor) + '.'
                                      else
                                        NfsOBS2.AsString :=
                        ...
                        on E: Exception do
                        begin
                          Mensagem('Exceção no método - nfse.AnalisarRespostaEnvio(Trim(NfsRPS_PROTOCOLO.Value)) -.'
                    ...
                    begin
                      ...
                      try
                        ...
                        on E: Exception do
                          Mensagem('Erro ao atribuir competência. ' + E.Message,
                            mtWarning, [mbOk], mrOk, 0);
                      end;
                      ...
                      begin
                        if ClientesINSC_MUNICIPAL.Value <> EmptyStr then
                          // se o cliente tiver inscrição municipal atribui no xml.
                ...
                on E: Exception do
                begin
                  Mensagem('Exceção no método - EnviarNFSe -.' + #13 +
            ...
            begin
              Mensagem('É necessário informar o número de série do certificado digital no cadastro da unidade escolar.',
    ...
    begin
      ...
      if (pos('Classe não registrada', E.Message) > 0) then
        Mensagem('Classe não registrada. É necessário instalar o .NET 4 e registrar o arquivo sednfse.dll',
          mtWarning, [mbOk], mrOk, 0)
      else
      begin
