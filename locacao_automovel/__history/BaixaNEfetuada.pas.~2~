...
procedure TBaixaNEfetuadaForm.DBGrid1DrawColumnCell(Sender: TObject;
...
begin
  ...
  Try
    ...
    begin
      Close;
      SQL.Clear;
      SQL.Text := 'select * from retorno where (' + tipoData + ' between ' +
...
begin
  ...
  if (BaixaNTIPO.Value = 2) then
    BaixaNRetornoDebAuto.Value := BancodeDados.PegaDescricaoRetornoDebAuto
      (BaixaNCOB_DEB.Value);
  ...
  begin
    ...
    begin
      ...
      txtAlunos := txtAlunos + 'Cliente: ' + txtTmp +
        Preenche(BancodeDados.BoletosCLIENTE.Value, 40) + ' - Parc.: ' +
        Preenche(BancodeDados.BoletosPARCELA.Value, 5) + ' Venc.: ' +
        FormatDateTime('dd/mm/yyyy', BancodeDados.BoletosVENCIMENTO.Value) +
        ' - Vl. A Pagar: ' + FormatFloat(',0.00',
        BancodeDados.BoletosVALOR_APAGAR.Value) + ' - Vl. Pago: ' +
        FormatFloat(',0.00', BancodeDados.BoletosVALOR_PAGO.Value);
...
procedure TBaixaNEfetuadaForm.EditConsultaKeyDown(Sender: TObject;
...
begin
  ...
  begin
    if BaixaN.IsEmpty then
      Mensagem('Não há informações para serem impressas.', mtInformation,
        [mbok], mrok, 0)
    else
      try
        ...
        RelArquivoRetornoForm.lbTitulo1.Caption :=
          'Relatório de Arquivo Retorno - Baixa bancária';
        RelArquivoRetornoForm.lbPeriodo2.Caption := TipoPeriodo +
          (Dt_Inicial.DateTimeStr) + ' a ' + (Dt_Final.DateTimeStr);
        RelArquivoRetornoForm.lbSituacao2.Caption := 'Situação: ' +
          CbSituacao.Text;
  ...
  else if rbTitulosN.Checked then
  begin
    Screen.Cursor := crSQLWait;
    with TitulosTmp do
    begin
    ...
    begin
      ...
      BancodeDados.Emissoes.SQL.Text :=
        'select * from boletos_emissao where banco_id = ' +
        IntToStr(BaixaNBANCO_ID.Value) + ' and nosso_numero=' +
        QuotedStr(BaixaNNOSSO_NUMERO.Value);
      BancodeDados.Emissoes.Open;
      if not BancodeDados.Emissoes.IsEmpty then
      begin
        ...
        begin
          ...
          begin
            ...
            if (vlDoc <> vlIntegCalc) and (vlDoc <> vlAPagCalc) then
              TitulosTmpCLIENTE_NOME.AsString := '* ' +
            ...
            TitulosTmpCLI_ID.Value := BancodeDados.BoletosCLI_ID.Value;
            TitulosTmpVALOR_APAGAR.Value :=
              BancodeDados.BoletosVALOR_APAGAR.Value;
            TitulosTmpVALOR_INTEGRAL.Value :=
            ...
            TitulosTmpSITUACAO_BOLETO.AsString :=
    ...
    if TitulosTmp.IsEmpty then
      ...
      try
        if not Assigned(RelArquivoRetornoForm) then
          RelArquivoRetornoForm := tRelArquivoRetornoForm.Create(Application);
        RelArquivoRetornoForm.lbTitulo3.Caption :=
          'Relatório de Arquivo Retorno - Baixa bancária';
        RelArquivoRetornoForm.lbPeriodo3.Caption := 'Período: ' +
          (Dt_Inicial.DateTimeStr) + ' a ' + (Dt_Final.DateTimeStr);
        RelArquivoRetornoForm.lbSituacao3.Caption := 'Situação: ' +
          CbSituacao.Text;
        BaixaN.DisableControls;
...
begin
  ...
  begin
    ...
    BancodeDados.V_Boletos.SQL.Text :=
      'select * from v_boletos where nosso_numero=' +
      QuotedStr(BaixaNNOSSO_NUMERO.Value);
