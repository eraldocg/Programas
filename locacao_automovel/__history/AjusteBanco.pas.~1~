...
type
  ...
  public
    procedure MarcarAtivoPorNome(ListaTmp: TStringList; Nome : string; var Campo: TBooleanField);
    procedure CalcularAtivoPorNome(ListaTmp: TStringList; Nome: string; var Campo: TBooleanField);
...
var
  ...
  Function Mensagem(const Msg: string; AType: TMsgDlgType; AButtons: TMsgDlgButtons; DefButton: TModalResult; HelpCtx: Longint; NomeFonte: String = ''): Word; External 'recursos.dll';
...
procedure TAjusteBancoForm.CalcularAtivoPorNome(ListaTmp: TStringList; Nome : string; var Campo: TBooleanField);
begin
...
procedure TAjusteBancoForm.MarcarAtivoPorNome(ListaTmp: TStringList; Nome: string; var Campo: TBooleanField);
var
  NomeTmp: string;
begin
 // ListaTmp.Duplicates := dupIgnore;
...
begin
  ...
  if Antigo = false then
    ConteudoMSG := 'Deseja executar a rotina simples para'#13 + 'ajustar o banco de dados?'
  else
    ConteudoMSG := 'Verifique se o SED Escolar não esteja sendo'#13 + 'utilizado por algum computador/terminal!'#13 + ''#13 + 'Deseja executar a rotina COMPLETA para'#13 + 'ajustar o banco de dados?';
    ...
    case RemotoDriveID of
        ...
        begin
          ...
          try
            ...
            On E: Exception do
            begin
              Mensagem('Ocorreu uma falha inesperada. Entre em contato com o suporte técnico e informe a seguinte mensagem:' + #13 + #13 + E.Message, mtWarning, [mbok], mrok, 0);
            end;
          end;
          BancodeDados.FDConnection1.close;
          BancodeDados.FDConnection1.Open;
        ...
        begin
            ...
            if (Antigo = True) then
            begin
              if Mensagem('Atenção! Não é recomendado que a função de ajuste de campos calculados seja utilizada.' + #13 + 'Em caso de dúvidas, entre em contado com o suporte técnico da SED Soft.' + #13 + #13 + 'Tem certeza que deseja prosseguir?', mtWarning, [mbYes, mbNo], mrNo, 0) = idYes then
              begin
        ...
        Mensagem('A rotina de banco de dados foi realizada com sucesso, é necessário reiniciar a aplicação.' + #13 + 'Pressione o botão <Ok> para sair.', mtInformation, [mbok], mrok, 0);
...
podeIR:=False;
  ...
  begin
    if  MessageDlg('Deseja realizar o processo de unificação do banco de dados Firebird?',mtConfirmation,[mbYes, mbNo],mrOk)=IDYES then
    podeIR:=True;
  end else
  begin
            ...
            begin
              ...
              begin
                  try
                    ...
                    BancodeDados.csTabExiste.SQL.Text:='SELECT COUNT(*) QTDE FROM RDB$RELATIONS WHERE RDB$FLAGS=1 and RDB$RELATION_NAME='+QuotedStr(QryListaFB2Table.Value);
                    BancodeDados.csTabExiste.Open;
                  except
                  end;

                  if not (BancodeDados.csTabExisteQTDE.Value>0) then //verificando a existencia da tabela banco 1
                  //if podeIR then
                  begin
                      //Application.ProcessMessages;
                    try
                      SQLReader2.TableName             :=(Trim(QryListaFB2Table.Value));
                      BancodeDados.SQLWriter2.TableName:=(Trim(QryListaFB2Table.Value));
                      ...
                      begin
                        MemoMySql.Clear;
                        MemoMySql.Lines.Add((QryListaFB2Table.Value)+' >>> '+E.Message);
                        StatusBar1.Panels[0].Text  := 'Erro...';
                      end;
                    end;//'try'


                    if (Trim(QryListaFB2Table.Value) = 'SMS_SERV')
                    then
                    begin

                      qrySql2.Close;
                      qrySql2.SQL.Text:='select max(sms_serv_id) as total from sms_serv where sms_serv_id>0';
                      qrySql2.Open();
                      ...
                      begin
                        try
                          ...
                          begin
                            ...
                            Add('ALTER SEQUENCE GEN_SMS_SERVCOD RESTART WITH '+IntToStr(qrySql2.FieldByName('total').Value) +';');
                          end;
                    ...
                    if (Trim(QryListaFB2Table.Value) = 'EMAILS_SERV')
                    then
                    begin
                      qrySql2.Close;
                      qrySql2.SQL.Text:='select max(em_serv_id) as total from emails_serv';
                      qrySql2.Open();
                      ...
                      begin
                        try
                          ...
                          begin
                            if qrySql2.FieldByName('total').Value>0 then
                            Add('ALTER SEQUENCE GEN_EM_SERVCOD RESTART WITH '+IntToStr(qrySql2.FieldByName('total').Value) +';');
                          end;
                          ValidateAll;
                          ExecuteAll;
                          BancodeDados.FDConnection1.CommitRetaining;    //1
          ...
          begin
            ...
            begin
              ...
              begin
                  ...
                  try
                    BancodeDados.csTabExiste.Close;
                    BancodeDados.csTabExiste.SQL.Text:='SELECT COUNT(*) QTDE FROM RDB$RELATIONS WHERE RDB$FLAGS=1 and RDB$RELATION_NAME='+QuotedStr(QryListaFB3Table.Value);
                    BancodeDados.csTabExiste.Open;
                  except
                  end;
                  ...
                  //Application.ProcessMessages;
                    try
                      ...
                      BancodeDados.SQLWriter3.TableName :=(Trim(QryListaFB3Table.Value));
                      ...
                      begin
                        MemoMySql.Clear;
                        MemoMySql.Lines.Add((QryListaFB3Table.Value)+' >>> '+E.Message);
                        StatusBar1.Panels[0].Text  := 'Erro...';
                      end;
                    end;//'try'
                    ...
                    begin
                      ...
                      qrySql3.Close;
                      qrySql3.SQL.Text:='select max(s_doc_id) as total from s_doc where s_doc_id>0';
                      qrySql3.Open();
                      ...
                      begin
                        ...
                        try
                          ...
                          begin
                            ...
                            Add('ALTER SEQUENCE GEN_SDOCCOD RESTART WITH '+IntToStr(qrySql3.FieldByName('total').Value) +';');
                          end;
...
//            BancodeDados.FDConnection1.Commit;
            ...
            if FDConnection3.Connected then  FDConnection3.Close;
            if FDConnection2.Connected then   FDConnection2.Close;
            ...
            AbcFBtmp:=EmptyStr;
            MessageDlg('Processo concluído com sucesso!',mtInformation,[mbOk],mrOk);
      ...
      do
       begin
          MessageDlg('Falha no processo: '+#13+E.Message,mtWarning,[mbOk],mrOk);
          AbcFBtmp:=EmptyStr;
        end;
      end;
...
procedure TAjusteBancoForm.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
begin
...
// if not BancodeDados.FDConnection1.Connected then BancodeDados.FDConnection1.Open;
   ...
   try
     ...
     QryCs.Open();
       ...
       if not (QryCs.FieldByName('import_bc_fb').AsInteger > 0)  then
         PodeImpor :=True  else PodeImpor:=False;
     except
     end;
 ...
 if PodeImpor then
 PodeImpor:=Mensagem('Deseja realizar a importação dos bancos II e III?', mtConfirmation, [mbYes, mbNo] ,mrOk, 0)=idYes;
...
begin
  ...
  begin
    ...
    begin
      if (QryListaFB2.Active) then (QryListaFB2.close);
      if (QryListaFB3.Active) then (QryListaFB3.close);
      ...
      //begin
       ...
       // if not BancodeDados.FDConnection1.Connected then
        try //1
          ...
          MessageDlg('Problema ao conectar ao banco principal.',mtWarning,[mbok],mrok);
        ...
        begin
          ...
          begin
            ...
            try //2 conexao ao banco 2
              ...
              MessageDlg('Problema ao conectar ao banco II.',mtWarning,[mbok],mrok);
            ...
            try //2
              ...
              MessageDlg('Problema ao conectar ao banco III.',mtWarning,[mbok],mrok);
            ...
            begin
              ...
              QryListaFB2.Close;
              QryListaFB2.SQL.Text:='SELECT RDB$RELATION_NAME as '+('"Table"')+' FROM RDB$RELATIONS WHERE RDB$VIEW_BLR IS NULL'+
                                   ' and not (UPPER(RDB$RELATION_NAME) starting with '+QuotedStr('RDB$')+')'+
                                   ' and not (UPPER(RDB$RELATION_NAME) starting with '+QuotedStr('MON$')+')'+
              ...
              begin
                ...
                while not QryListaFB2.Eof do
                begin
                  if (Trim(QryListaFB2Table.Value) = 'EMAILS_SERV') or (Trim(QryListaFB2Table.Value) = 'SMS_SERV')
                  then
                  begin
                    if not (QryListaFB2.State in [dsEdit]) then QryListaFB2.Edit;
                    MarcarAtivoPorNome(ListaTabelas2, QryListaFB2Table.Value, QryListaFB2ATIVO);
                  end;
                  QryListaFB2.Next;
                end;
                QryListaFB2.EnableControls;
              ...
              QryListaFB3.Close;
              QryListaFB3.SQL.Text:='SELECT RDB$RELATION_NAME as '+('"Table"')+' FROM RDB$RELATIONS WHERE RDB$VIEW_BLR IS NULL'+
                                   ' and not (UPPER(RDB$RELATION_NAME) starting with '+QuotedStr('RDB$')+')'+
                                   ' and not (UPPER(RDB$RELATION_NAME) starting with '+QuotedStr('MON$')+')'+
                                   ' order by  RDB$RELATION_NAME';
              QryListaFB3.Open;
              ...
              begin
                ...
                while not QryListaFB3.Eof do
                begin
                  if (Trim(QryListaFB3Table.Value) = 'S_DOC')
                  then
                  begin
                    if not (QryListaFB3.State in [dsEdit]) then QryListaFB3.Edit;
                    MarcarAtivoPorNome(ListaTabelas3, QryListaFB3Table.Value, QryListaFB3ATIVO);
                  end;
                  QryListaFB3.Next;
                end;
                QryListaFB3.EnableControls;
           ...
           MemoMySql.Lines.Add(StringReplace(FDConnection2.ConnectionString, ';',#13, [rfReplaceAll] ) );
           MemoMySql.Lines.Add('3 ------------------------------------');
           MemoMySql.Lines.Add(StringReplace(FDConnection3.ConnectionString, ';',#13, [rfReplaceAll] ) );
          end else
          begin
           Mensagem('Arquivo(s) não encontrado(s): Problema ao conectar o banco II e III.',mtWarning,[mbOk],mrOk,0);
           AbcFBtmp:=EmptyStr;
          end;
        end else
        begin
          Mensagem('Falha na conexão com o banco principal.',mtWarning,[mbOk],mrOk,0);
          AbcFBtmp:=EmptyStr;
        end;
...
if podeIR  then
begin
  if  MessageDlg('Deseja finalizar o processo?',mtConfirmation,[mbYes, mbNo],mrOk)=IDYES then
  begin
  podeIR :=False;
  Close;
  end;
