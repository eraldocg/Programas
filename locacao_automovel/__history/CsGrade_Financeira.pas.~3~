...
procedure TCsGrade_FinanceiraForm.EditNomeKeyDown(Sender: TObject;
  ...
  begin
    ...
    try
      ...
      BancodeDados.QrySql.SQL.Text :=
        'select distinct grade_id from cont_serv_clien where grade_id = ' +
        IntToStr(BancodeDados.GradeFinGRADE_ID.Value);
      ...
      begin
        ...
        Mensagem('Registro com vínculo ao serviço de locação!'#13 +
          'Ação não permitida!', mtInformation, [mbOk], mrOk, 0);
    except
      on E: Exception do
        TraduzErro(E.Message);
    end;
...
begin
  if BancodeDados.GradeFinGRADE_ID.Value > 0 then
    if (BancodeDados.GradeFinPADRAO.IsNull) or
      (BancodeDados.GradeFinPADRAO.Value <> 1) then
      if Mensagem('Definir o registro como grade financeira padrão?',
      ...
      begin
        try
          ...
          BancodeDados.Adiciona.SQL.Text :=
            'update grade_financeira set padrao=0 where not grade_id=' +
            IntToStr(BancodeDados.GradeFinGRADE_ID.Value);
          BancodeDados.Adiciona.ExecSQL;
          BancodeDados.Adiciona.SQL.Text :=
            'update grade_financeira set padrao=1 where grade_id=' +
            IntToStr(BancodeDados.GradeFinGRADE_ID.Value);
          BancodeDados.Adiciona.ExecSQL;
          BancodeDados.FDConnection1.CommitRetaining;
          BancodeDados.GradeFin.Close;
...
procedure TCsGrade_FinanceiraForm.EditConsultaKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
var
