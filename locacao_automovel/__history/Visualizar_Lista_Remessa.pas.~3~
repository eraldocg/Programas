...
begin
  ...
  begin
    ...
    begin
      if not(RemessaForm.Boletos.State in [dsEdit]) then
        RemessaForm.Boletos.Edit;
      BancodeDados.MarcarAtivo(listaV_Boletos,
        RemessaForm.BoletosEMISSAO_ID.Value, RemessaForm.BoletosATIVO);
    end;
  end;
end;
  ...
  begin
    ...
    try
      ...
      BancodeDados.Clientes.SQL.Text := PrincipalForm.SQLrepositorio(1) +
        ' and c.cli_id = ' + IntToStr(RemessaForm.BoletosCLI_ID.Value);;
...
begin
  if RemessaForm.BoletosEMISSAO_ID.Value > 0 then
  begin
