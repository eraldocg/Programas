...
type
  ...
  private
    ...
    aCodInicNossNum     : Smallint; //eraldo  alterando o inicio do do nosso numero, para trabalhar com a mesma conta para mais de uma escola
  ...
  public
    ...
    property Valor_Documento: Currency read aValor_Documento write aValor_Documento;
    ...
    property IniciNosNum: Smallint read aCodInicNossNum write aCodInicNossNum default 0;
  ...
  public
    ...
    property IniciNosNum: Smallint read aIniciNosNum write aIniciNosNum; //eraldo alterando o inicio do do nosso numero, para trabalhar com a mesma conta para mais de uma escola
    property RegistroComProblema: TStringList read aRegProblema write aRegProblema; //28/08/16
...
function Modulo11Safra(Valor: String; Base: Integer = 9; Resto: Boolean = False): string;
var
  Soma: Integer;
  Contador, Peso, Digito: Integer;
begin
...
begin
  ...
  begin
    CodigoAuxiliar := CodigoAuxiliar + Codifi[i] + Codifi[i + 5] + Codifi[i + 1] + Codifi[i + 6] + Codifi[i + 2] + Codifi[i + 7] + Codifi[i + 3] + Codifi[i + 8]
...
var
  p1, p2, p3, p4, p5, p6, cdbarras, rNossoNumero, rNossoNumero2, campo1, campo2, campo3, campo4, campo5, DACNossoNum, chaveBrb, campoLivre: String;
...
begin
  ...
  if Trim(Moeda) = '' then
    Raise Exception.Create('Código de Moeda não foi informado')
      ...
      if (CodBanco > 0) and (Trim(CodCedente) = '') then
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            if (Carteira='24') and (IniciNosNum>0) then //alterando a carteira 24 para usarmos a mesma conta em dois bancos de dados
              rNossoNumero := IntToStr(IniciNosNum) + Formatar(IntToStr(SeqNossoNumero), 13, False, '0')
            else  //devendo incluir no campo 2400 >> tirando o nove
              rNossoNumero := Carteira + '90' + Formatar(IntToStr(SeqNossoNumero), 13, False, '0');

            rNossoNumero2 := Copy(rNossoNumero, 3, 3) + Copy(rNossoNumero, 1, 1) + Copy(rNossoNumero, 6, 3) + Copy(rNossoNumero, 2, 1) +
            ...
            p1 := Formatar(CalcularFatorVencimento(DataVencimento + DiasTolerancia), 4, False, '0'); //p1 := Formatar(CalcularFatorVencimento(DataVencimento + DiasTolerancia), 4, False, '0');
            p2 := Formatar(RetornaLimpo(formatFloat('0.00', Valor_Documento)), 10, False, '0');
            campo5 := p1 + p2;

            {
              Campo 4 - digito verificador do codigo de barras
            }
            p3 := formatFloat('000', CodBanco) + Moeda + campo5 + CodCedente + DvCodCedente + rNossoNumero2 +
            ...
            cdbarras := formatFloat('000', CodBanco) + Moeda + campo4 + campo5 + CodCedente + DvCodCedente + rNossoNumero2 +
              Modulo11(CodCedente + DvCodCedente + rNossoNumero2);
            ...
            aLinhaDigitavel := campo1 + ' ' + campo2 + ' ' + campo3 + ' ' + campo4 + ' ' + campo5;
          ...
          begin
            if Carteira = '11' then // carteira 11 CS
                 ...
                 rNossoNumero := '9' + Formatar(IntToStr(SeqNossoNumero), 9, False, '0')
            else if Carteira = '14' then begin // carteira 14 SR
              if IniciNosNum>0 then
                 rNossoNumero := IntToStr(IniciNosNum) + Formatar(IntToStr(SeqNossoNumero), 8, False, '0') else
                 rNossoNumero := '82' + Formatar(IntToStr(SeqNossoNumero), 8, False, '0') // alterando a carteira 14 para usarmos a mesma conta em dois bancos de dados
              end else                                                                   // alterando o codigo inicial do Nosso Numero : 29/12/2015 rNossoNumero := '82' + Formatar(IntToStr(SeqNossoNumero), 8, False, '0')
            begin
                 Carteira := '11';
                 rNossoNumero := Formatar(IntToStr(SeqNossoNumero), 10, False, '0');
            end;
            ...
            p3 := formatFloat('000', CodBanco) + Moeda + campo5 + rNossoNumero + CodCedente;
            campo4 := Modulo11(p3, 9);
            //Atualização - Alex Eloy - 14/02/2017
            if (campo4 = '0') or (strtointdef(campo4,0) > 9 )then
              campo4 := '1';

            cdbarras := formatFloat('000', CodBanco) + Moeda + campo4 + campo5 + rNossoNumero + CodCedente;
          ...
          if (CodBanco = 1) then
            ...
            begin
              ...
              begin
                ...
                Convenio      := Formatar(Copy(RetornaLimpo(Convenio), 1, 6), 6, False, '0');
                Agencia       := Formatar(RetornaLimpo(Agencia), 4, False, '0');
                Carteira      := Formatar(RetornaLimpo(Carteira), 2, False, '0');
                rNossoNumero  := Convenio + Formatar(IntToStr(SeqNossoNumero), 5, False, '0');
                { Campo 1 - composto pelo código do banco, código da moeda, as cinco primeiras posições
                ...
                p3 := formatFloat('000', CodBanco) + Moeda + campo5 + rNossoNumero + Agencia + Conta + Carteira;
                campo4 := Modulo11(p3);
                if campo4 = '0' then
                  campo4 := '1';

                cdbarras := formatFloat('000', CodBanco) + Moeda + campo4 + campo5 + rNossoNumero + Agencia + Conta + Carteira;
                ...
                aNossoNumero := formatFloat('00,000,000,000', StrToInt64(rNossoNumero)) + '-' + DVMod11BB(rNossoNumero);
                aCodigoBarras := cdbarras;
                aLinhaDigitavel := campo1 + ' ' + campo2 + ' ' + campo3 + ' ' + campo4 + ' ' + campo5;
              end
              else
              begin
                ...
                Convenio := Formatar(Copy(RetornaLimpo(Convenio), 1, 7), 7, False, '0');
                Carteira := Formatar(RetornaLimpo(Carteira), 2, False, '0');
                rNossoNumero := Convenio + Formatar(IntToStr(SeqNossoNumero), 10, False, '0');
                ...
                p3 := formatFloat('000', CodBanco) + Moeda + campo5 + '000000' + rNossoNumero + Carteira;
                campo4 := Modulo11(p3);
                if campo4 = '0' then
                  campo4 := '1';

                cdbarras := formatFloat('000', CodBanco) + Moeda + campo4 + campo5 + '000000' + rNossoNumero + Carteira;
                // 3                 1        1         14        6            17           2
            ...
            begin
              ...
              begin
                ...
                if (Carteira = '106') or (Carteira = '107') or (Carteira = '122') or (Carteira = '142') or (Carteira = '143') or (Carteira = '195') or
                  (Carteira = '196') or (Carteira = '198') then
                begin
                  Raise Exception.Create('Função não disponível para está carteira.');
                end
                else
                begin
                  ...
                  DVConta := Modulo10(Agencia + Conta);  //alex e eraldo em 16/11/17 às 17:41
                  ...
                  begin
                    DACNossoNum := Modulo10(Agencia + Conta + Carteira + rNossoNumero);
                  end;
                  ...
                  p3 := formatFloat('000', CodBanco) + Moeda + campo5 + Carteira + rNossoNumero + DACNossoNum + Agencia + Conta + DVConta + '000';
                  campo4 := Modulo11(p3, 9);
                  if (campo4 = '0') then
                    campo4 := '1';

                  // 1-3                   4       5       6-19      20-22        23-30         31          32-35    36-40      41     42-44
                  cdbarras := formatFloat('000', CodBanco) + Moeda + campo4 + campo5 + Carteira + rNossoNumero + DACNossoNum + Agencia + Conta +
                    DVConta + '000';

                  //Correção - Alex Eloy - 31/01/2017
                  campo3 := (Copy(cdbarras, 35, 10));//campo3 := (Copy(cdbarras, 34, 10));
                  p1 := Copy(campo3, 1, 5);
              ...
              begin
                ...
                begin
                  ...
                  if (aCobReg = True) then
                    rNossoNumero := Formatar(IntToStr(SeqNossoNumero), 7, False, '0')
                  else
                    rNossoNumero := Formatar(IntToStr(SeqNossoNumero), 13, False, '0');
                  ...
                  p3 := formatFloat('000', CodBanco) + Moeda + campo5 + Agencia + Conta + DVConta + rNossoNumero;
                  campo4 := Modulo11(p3);
                  if campo4 = '0' then
                    campo4 := '1';

                  cdbarras := formatFloat('000', CodBanco) + Moeda + campo4 + campo5 + Agencia + Conta + DVConta + Formatar(rNossoNumero, 13, False, '0');
                  // 3                 1        1        14        4        7        1           13
                ...
                begin
                  ...
                  rNossoNumero := Formatar(IntToStr(SeqNossoNumero), 14, False, '0');
                  ...
                  p3 := formatFloat('000', CodBanco) + Moeda + campo5 + '5' + CodCedente + DvCodCedente + '00' + rNossoNumero + Modulo11(rNossoNumero);
                  campo4 := Modulo11(p3);
                  if campo4 = '0' then
                    campo4 := '1';

                  cdbarras := formatFloat('000', CodBanco) + Moeda + campo4 + campo5 + '5' + CodCedente + '00' + rNossoNumero + Modulo11(rNossoNumero);
                  // 3                 1        1        14      1        7         2          14                  1
                  ...
                  begin
                    ...
                    rNossoNumero := Formatar(IntToStr(SeqNossoNumero), 11, False, '0');
                    ...
                    p3 := formatFloat('000', CodBanco) + Moeda + campo5 + Agencia + aCarteira + rNossoNumero + Conta + '0';
                    campo4 := Modulo11(p3);
                    if campo4 = '0' then
                      campo4 := '1';

                    cdbarras := formatFloat('000', CodBanco) + Moeda + campo4 + campo5 + Agencia + aCarteira + rNossoNumero + Conta + '0';
                    // 3                 1        1        14        4         2            11           7      1
                    ...
                    begin
                      ...
                      begin
                        ...
                        begin
                          CodCedente := Formatar(RetornaLimpo(CodCedente), 5, False, '0');
                          Carteira := Formatar(RetornaLimpo(Carteira), 2, False, '0');
                          Agencia := Formatar(RetornaLimpo(Agencia), 4, False, '0');

                          rNossoNumero := CodCedente + Formatar(IntToStr(SeqNossoNumero), 5, False, '0');
                          ...
                          p3 := formatFloat('000', CodBanco) + Moeda + campo5 + rNossoNumero + Agencia + Formatar(RetornaLimpo(Conta + DVConta), 7, False, '0')
                            + Carteira + '1';
                          campo4 := Modulo11(p3);
                          if campo4 = '0' then
                            campo4 := '1';

                          cdbarras := formatFloat('000', CodBanco) + Moeda + campo4 + campo5 + rNossoNumero + Agencia +
                            Formatar(RetornaLimpo(Conta + DVConta), 7, False, '0') + Carteira + '1';
                          // 3                 1        1        14        11                              11                                            2        1
                      ...
                      begin
                        CodCedente := Formatar(RetornaLimpo(CodCedente), 7, False, '0');
                        rNossoNumero := Formatar(IntToStr(SeqNossoNumero), 13, False, '0');
                        rNossoNumero2 := rNossoNumero + Modulo11(rNossoNumero) + '5';
                        rNossoNumero2 := IntToStr(StrToInt64(rNossoNumero2) + StrToInt64(CodCedente));
                        rNossoNumero := rNossoNumero + Modulo11(rNossoNumero) + '5' + Modulo11(rNossoNumero2);
                        ...
                        p3 := formatFloat('000', CodBanco) + Moeda + campo5 + CodCedente + Copy(rNossoNumero, 1, 13) + '0000' + '2';
                        campo4 := Modulo11(p3);
                        if campo4 = '0' then
                          campo4 := '1';

                        cdbarras := formatFloat('000', CodBanco) + Moeda + campo4 + campo5 + CodCedente + Copy(rNossoNumero, 1, 13) + '0000' + '2';
                      ...
                      begin
                        ...
                        CodCedente := Formatar(RetornaLimpo(CodCedente), 6, False, '0');   //alex: 01/12/16 as 11:29min
                        CodCedente := CodCedente + Modulo11(CodCedente);                   //alex: 01/12/16 as 11:29min


                        rNossoNumero := Formatar(IntToStr(SeqNossoNumero), 12, False, '0');
                        ...
                        p3 := formatFloat('000', CodBanco) + Moeda + campo5 + '9' + CodCedente + rNossoNumero + Modulo11(rNossoNumero) + '0' + aCarteira;
                        campo4 := Modulo11(p3);
                        if campo4 = '0' then
                          campo4 := '1';

                        cdbarras := formatFloat('000', CodBanco) + Moeda + campo4 + campo5 + '9' + CodCedente + rNossoNumero + Modulo11(rNossoNumero) + '0' +
                          aCarteira;
                        // 3                 1        1        14      1        7                    13                           1        3
                        ...
                        begin
                          CodCedente := Formatar(RetornaLimpo(CodCedente), 6, False, '0');
                          rNossoNumero := { FormatDateTime('yy', Date) + } Formatar(IntToStr(SeqNossoNumero), 7, False, '0');
                          Agencia := Formatar(RetornaLimpo(Agencia), 4, False, '0');
                          ...
                          aCarteira := Formatar(RetornaLimpo(aCarteira), 3, False, '0');


                          DACNossoNum := DVMod11Bancoob(Agencia + Formatar(CodCedente + DvCodCedente, 10, False, '0') + rNossoNumero);
                          ...
                          p3 := formatFloat('000', CodBanco) + Moeda + campo5 + Copy(aCarteira, 1, 1) + Agencia + Copy(aCarteira, 2, 2) + CodCedente +
                            DvCodCedente + rNossoNumero + DACNossoNum + '001';
                          campo4 := Modulo11(p3);
                          if campo4 = '0' then
                            campo4 := '1';

                          cdbarras := formatFloat('000', CodBanco) + Moeda + campo4 + campo5 + Copy(aCarteira, 1, 1) + Agencia + Copy(aCarteira, 2, 2) +
                            CodCedente + DvCodCedente + rNossoNumero + DACNossoNum + '001';
                          ...
                          begin
                            ...
                            begin
                              Agencia := Formatar(RetornaLimpo(Agencia), 3, False, '0');
                              Conta := Formatar(RetornaLimpo(Conta), 6, False, '0');
                              DVConta := Formatar(RetornaLimpo(DVConta), 1, False, '0');
                              aCarteira := Formatar(aCarteira, 1, False, '0');
                              rNossoNumero := Formatar(IntToStr(SeqNossoNumero), 6, False, '0');

                              chaveBrb := '000' + Agencia + Conta + DVConta + aCarteira + rNossoNumero + formatFloat('000', CodBanco);
                              ...
                              p3 := formatFloat('000', CodBanco) + Moeda + campo5 + chaveBrb;
                              campo4 := Modulo11(p3);
                              if campo4 = '0' then
                                campo4 := '1';

                              cdbarras := formatFloat('000', CodBanco) + Moeda + campo4 + campo5 + chaveBrb;
                              // 3                 1        1        14        25
                              ...
                              aNossoNumero := aCarteira + rNossoNumero + formatFloat('000', CodBanco) + DACNossoNum;
                              aCodigoBarras := cdbarras;
                              aLinhaDigitavel := campo1 + ' ' + campo2 + ' ' + campo3 + ' ' + campo4 + ' ' + campo5;
                            end;
                          end
                            ...
                            begin
                              ...
                              begin
                                if (SeqNossoNumero > 99999) then
                                  Raise Exception.Create('O valor máximo permitido para a sequência do nosso número é 99.999.');

                                Agencia   := Formatar(RetornaLimpo(Agencia), 4, False, '0');
                                Conta     := Formatar(RetornaLimpo(Conta), 5, False, '0');
                                aCarteira := Formatar(aCarteira, 2, False, '0');

                                rNossoNumero := Formatar(IntToStr(SeqNossoNumero), 5, False, '0');
                                ...
                                rNossoNumero := FormatDateTime('yy', Date) + '1' + rNossoNumero;


                                //rNossoNumero := rNossoNumero + Modulo11(Agencia + Carteira + Conta + rNossoNumero);

                                aux := modulo11(Agencia + Carteira + Conta + rNossoNumero);
                                ...
                                campoLivre := campoLivre + '1' + rNossoNumero + Agencia + aCarteira + Conta + '1' + '0';
                                campoLivre := campoLivre + Modulo11(campoLivre);

                                p3 := formatFloat('000', CodBanco) + Moeda + campo5 + campoLivre;
                                campo4 := Modulo11(p3);
                                if campo4 = '0' then
                                  campo4 := '1';

                                cdbarras := formatFloat('000', CodBanco) + Moeda + campo4 + campo5 + campoLivre;
                                // 3                 1        1        14
                                // 1-3               4-4      5-5      6-19
                              ...
                              begin
                                ...
                                begin
                                  Agencia := Formatar(RetornaLimpo(Agencia), 4, False, '0');
                                  CodCedente := Formatar(RetornaLimpo(CodCedente), 9, False, '0');
                                  // aCarteira:=Formatar(aCarteira, 2, false, '0');
                                  ...
                                  campoLivre := Agencia + rNossoNumero + DACNossoNum + CodCedente + '2';
                                ...
                                begin
                                  ...
                                  begin
                                    ...
                                    aContrato := Formatar(RetornaLimpo(aContrato), 2, False, '0');
                                    aConvenio := Formatar(RetornaLimpo(aConvenio), 3, False, '0');
                                    // aCarteira := Formatar(RetornaLimpo(aCarteira), 2, False, '0');
                                    aCarteira2 := Formatar(RetornaLimpo(aCarteira2), 2, False, '0');
                                    rNossoNumero := Formatar(IntToStr(SeqNossoNumero), 6, False, '0');

                                    // calculo do NossoNumero2 - apenas para impressão
                                    DACNossoNum := Modulo11Bradesco(aCarteira2 + aContrato + aConvenio + rNossoNumero);
                                    aNossoNumero2 := aContrato + aConvenio + rNossoNumero + '-' + DACNossoNum;

                                    rNossoNumero := Formatar(IntToStr(SeqNossoNumero), 6, False, '0');
                                    DACNossoNum := Modulo11BicBanco(Agencia + rNossoNumero);

                                    Conta2 := Formatar(RetornaLimpo(Conta2), 7, False, '0');
                                    Agencia2 := Formatar(RetornaLimpo(Agencia2), 4, False, '0');
                                    ...
                                    p3 := formatFloat('000', 237) + Moeda + campo5 + Agencia2 + aCarteira2 + aContrato + aConvenio + rNossoNumero +
                                      Conta2 + '0';
                                    campo4 := Modulo11(p3);
                                    if campo4 = '0' then
                                      campo4 := '1';
                                    // radical    matricula
                                    cdbarras := formatFloat('000', 237) + Moeda + campo4 + campo5 + Agencia2 + aCarteira2 + aContrato + aConvenio + rNossoNumero
                                      + Conta2 + '0';
                                  ...
                                  begin
                                    ...
                                    begin
                                      ...
                                      Agencia2  := Formatar(RetornaLimpo(Agencia2), 4, False, '0');
                                      Carteira2 := Formatar(RetornaLimpo(Carteira2), 2, False, '0');
                                      Conta2    := Formatar(RetornaLimpo(Conta2), 7, False, '0');

                                      rNossoNumero := Formatar(IntToStr(SeqNossoNumero), 11, False, '0');
                                      DACNossoNum := Modulo11Bradesco(Carteira2 + rNossoNumero);
                                      ...
                                      p3 := formatFloat('000', 237) + Moeda + campo5 + Agencia2 + Carteira2 + rNossoNumero + Conta2 + '0';
                                      campo4 := Modulo11(p3);
                                      if campo4 = '0' then
                                        campo4 := '1';

                                      cdbarras := formatFloat('000', 237) + Moeda + campo4 + campo5 + Agencia2 + Carteira2 + rNossoNumero + Conta2 + '0';
                                      // 3            1        1        14        4         2            11           7      1
                                      // 1-3          4-4      5-5      6-19     20-23     24-25        26-36        37-43    44
                                    ...
                                    begin
                                      ...
                                      begin
                                        ...
                                        p3 := formatFloat('000', CodBanco) + Moeda + campo5 + Agencia + Conta + DVConta + rNossoNumero + DACNossoNum +
                                        Carteira + '000';
                                        campo4 := Modulo11(p3, 9);
                                        if (campo4 = '0') then
                                        campo4 := '1';
                                        ...
                                        cdbarras := formatFloat('000', CodBanco) + Moeda + campo4 + campo5 + Agencia + Conta + DVConta + rNossoNumero +
                                        DACNossoNum + Carteira + '000';
                                      ...
                                      begin
                                        ...
                                        Raise Exception.Create('Código do cedente');
                                        if (Trim(Agencia) = '') then
                                        Raise Exception.Create('Agência não informada');
                                        if (Trim(Conta) = '') then
                                        Raise Exception.Create('Conta não informada');
                                        ...
                                        Raise Exception.Create('A sequência do nosso número não pode ser maior que 9999')
                                        else
                                        begin

                                        Agencia := Formatar(RetornaLimpo(Agencia + DVAgencia), 5, False, '0');
                                        Conta := Formatar(RetornaLimpo(Conta + DVConta), 9, False, '0');
                                        ...
                                        rNossoNumero := '2592' + Formatar(IntToStr(SeqNossoNumero), 4, False, '0');

                                        // faixa de nosso numero para o olimpo 99990000 a 99999999

                                        DACNossoNum := Modulo11Safra(rNossoNumero);

                                        rNossoNumero2 := FormatDateTime('yy', Date) + rNossoNumero + DACNossoNum;
                                        aNossoNumero2 := rNossoNumero2 + '-' + Modulo11Bradesco(Carteira2 + rNossoNumero2);
                                        ...
                                        p3 := formatFloat('000', 237) + Moeda + campo5 + Agencia2 + Carteira2 + rNossoNumero2 + Conta2 + '0';
                                        campo4 := Modulo11(p3, 9);
                                        if (campo4 = '0') then
                                        campo4 := '1';


                                        cdbarras := formatFloat('000', 237) + Moeda + campo4 + campo5 + Agencia2 + Carteira2 + rNossoNumero2 + Conta2 + '0';
                                        ...
                                        if (CodBanco <= 0) then
                                        begin
                                        aNossoNumero := formatFloat('000000', SeqNossoNumero);
                                        aCodigoBarras := '';
                                        aLinhaDigitavel := '00000.00000 00000.000000 00000.000000 0 00000000000000';
                                        end
                                        else { Banco 0 }

                                        Raise Exception.Create('Função não disponível para este banco no momento.' + #13 +
                                        'Entre em contato com o suporte da SEDSoft Ltda.');

      end;
end;

function PegaNossoNum(CodBanco, Layout: Integer; linha, aCodCed, aConvenio: String; CobReg: Boolean): String;
var
  NNumeroTmp: String;
 // i : integer;
begin
...
//3. CNAB240
  ...
  begin
    ...
    begin
      if Length(aConvenio) <= 6 then
        result := formatFloat('00,000,000,000', StrToInt64(Copy(linha, 63, 11))) + '-' + Copy(linha, 74, 1)
      else
    ...
    begin
      if Length(aConvenio) <= 6 then
        result := formatFloat('00,000,000,000', StrToInt64((Copy(linha, 38, 11)))) + '-' + Copy(linha, 49, 1)
      else
    ...
    begin
      ...
      result := formatFloat('00,000,000,000', StrToInt64(NNumeroTmp)); // +'-'+DVMod11BB(NNumeroTmp);
    end;
  ...
  begin
    ...
    begin
     ...
     if  Length(trim(Copy(linha, 57, 17)))=17 then  result := Copy(linha, 57, 17) + '-' + Modulo11(Copy(linha, 57, 17)) else //eraldo incluiu em 22/03/17
  ...
  else if (CodBanco = 756) then //Banco Cooperativo do Brasil S.A. - BANCOOB (SICOOB)
  begin
...
{ TSedRetorno }
procedure TSedRetorno.SetCedente(cod: string);  //verificando se o banco tem um cedente válido
begin
  if (aBanco <= 0) then
    raise Exception.Create('Informe um banco antes de informar o Código do Cedente.')
  else
  begin
    if (aBanco = 1) then //Banco do Brasil S.A.
      aCodCed := cod
    else
    if (aBanco = 104) then begin //Caixa Econômica Federal
      aCodCed := cod;
    end else if (aBanco = 341) then //Itaú Unibanco S.A.
      aCodCed := cod
      ...
      aCodCed := cod
    else if (aBanco = 756) then //Banco Cooperativo do Brasil S.A. - BANCOOB (SICOOB)
      aCodCed := cod
...
var
  ...
  linha, NomeCedente, NossoNumTmp, SeuNumeroTmp, CodCedente, CodConv, Obs: String;
  RegAtual, RegTotal, NumeroArquivo, i, CarteiraTmp, CodMov, CodigoOcorrencia {igual ao CodMov}, CodigoOcRejeijao : Integer;
  DataArquivo, datacredito, Datarecebimento, dtVencTmp, dtCredArq, DtOcorrencia: TDate;
...
begin
 ...
 try//except
  ...
  begin
    ...
    if ((Length(linha) = 150) and (Copy(linha, 82, 17) = 'DEBITO AUTOMATICO')) then
    begin
      ...
      begin
        ...
        try
          ...
          try
            ...
            if (aBanco = 104) then begin //Caixa Economica Federal
              if Length(aCodCed)>6 then               
              CodCedente := IntToStr(StrToInt64(Trim(Copy(linha, 27, 16)))) else
              CodCedente := IntToStr(StrToInt64(Trim(Copy(linha, 31, 6))));

              if Suporte then
              MostrarErro:='COMPARATIVO: '+#13+
                            'Banco: '+ Copy(linha, 77, 3) +' | '+IntToStr(aBanco)+#13+
                            'Agência: '+ Copy(linha, 27, 4)+' | '+(Agencia) +#13+
                            //'N.º Conta:    '+ Copy(linha, 34, 4)+' | '+(Conta) +#13+
                            'Cod. Cedente: '+ CodCedente+' | '+(aCodCed) else  MostrarErro:='';

            end else
            if (aBanco = 409) then //UNIBANCO
              ...
              if (aBanco = 237) then //Bradesco
                ...
                if (aBanco = 756) then //Banco Cooperativo do Brasil S.A. - BANCOOB (SICOOB)
                  CodCedente := IntToStr(StrToInt64(Trim(Copy(linha, 32, 8))))
                else

                if (aBanco = 748) then begin //Banco Cooperativo Sicredi S.A. //UNICRED PB
                  ...
                  CodCedente := IntToStr(StrToInt64(Trim(Copy(linha, 27, 10)))) // ContaCorrente
                else

                  if (aBanco = 707) then // Banco Daycoval S.A.
                  CodCedente := IntToStr(StrToInt64(Trim(Copy(linha, 27, 20)))) // ContaCorrente
                else

                  if (aBanco = 422) then // Banco Safra S.A.
                  CodCedente := IntToStr(StrToInt64(Trim(Copy(linha, 27, 14)))) // ContaCorrente
                else

                  if (aBanco = 004) then  //Banco do Nordeste do Brasil S.A.
                  CodCedente := IntToStr(StrToInt64(Trim(Copy(linha, 33, 7)))); // ContaCorrente
          except
            CodCedente := '';
            CodConv := '';
          end;
          ...
          if (Copy(linha, 1, 19) <> '02RETORNO01COBRANCA') and (Copy(linha, 1, 19) <> '02RETORNO01COBRANÇA')  then
          begin
            PodeIr := False;
            MessageBox(Application.Handle, 'O arquivo de retorno é inválido!', 'SED Soft Informa...', Mb_ok + MB_ICONWARNING);
          end
          else
          begin
            ...
            begin
              ...
              if (formatFloat('000', aBanco) <> Copy(linha, 77, 3)) or (Trim(Agencia) <> Copy(linha, 27, 4))
              or (Trim(Conta) <> Copy(linha, 33, 5)) then
              begin
                PodeIr := False;
               // ('aBanco: ' + IntToStr(aBanco) + ' | '+Copy(linha, 77, 3) +' - Agencia: '+Agencia +' | '+Copy(linha, 27, 4) + ' - Conta' +Conta + ' | '+Copy(linha, 34, 4)  );
                ...
                MostrarErro:='COMPARATIVO: '+#13+
                  ...
                  MessageBox(Application.Handle, Pchar('O arquivo ' + aArquivo + ' não pertence a este banco!'+#13 + MostrarErro ), 'SED Soft Informa...', Mb_ok + MB_ICONWARNING);
            ...
            begin
              if ((formatFloat('000', aBanco) <> Copy(linha, 77, 3)) and ('275' <> Copy(linha, 77, 3))) or (Trim(Agencia) <> Copy(linha, 28, 4)) or
                (Trim(Conta) <> Copy(linha, 33, 7)) { or (IntToStr(aBanco)<>Copy(Linha, 77, 3)) } then
              begin
                PodeIr := False;
            ...
            begin
              if (formatFloat('000', aBanco) <> Copy(linha, 77, 3)) or (Trim(Agencia + aCodCed) <> Trim(CodCedente)) then
              begin
                PodeIr := False;
                MessageBox(Application.Handle, Pchar('3. O arquivo ' + aArquivo + ' não pertence a este banco!'), 'SED Soft Informa...', Mb_ok + MB_ICONWARNING);
              end;
            end
            else if (aBanco = 237) then //Banco Bradesco S.A.
            begin

              if (formatFloat('000', aBanco) <> Copy(linha, 77, 3)) or (Trim(CodConv) <> Trim(aConvenio)) then
              begin
                PodeIr := False;
                MessageBox(Application.Handle, Pchar('O arquivo ' + aArquivo + ' não pertence a este banco!'), 'SED Soft Informa...', Mb_ok + MB_ICONWARNING);
              end;
            ...
            begin
              if (formatFloat('000', aBanco) <> Copy(linha, 77, 3)) or (Trim(CodCedente) <> Trim(aCodCed)) then
              begin
                PodeIr := False;
                MessageBox(Application.Handle, Pchar('O arquivo ' + aArquivo + ' não pertence a este banco!'), 'SED Soft Informa...', Mb_ok + MB_ICONWARNING);
              end;
            ...
            begin
              ...
              if (formatFloat('000', aBanco) <> Copy(linha, 77, 3)) or (Trim(CodCedente) <> Trim(aCodCed)) then // faltou catalogar
              begin                  
                PodeIr := False;
                //MessageBox(Application.Handle, Pchar('O arquivo ' + aArquivo + ' não pertence a este banco!'), 'SED Soft Informa...', Mb_ok + MB_ICONWARNING);
                MessageBox(Application.Handle, Pchar('O arquivo ' + aArquivo + ' não pertence a este banco!' +#13+ MostrarErro ), 'SED Soft Informa...', Mb_ok + MB_ICONWARNING);
          ...
          begin
            ...
            if (aBanco = 070) then //BRB - Banco de Brasília S.A.
            begin
              DataArquivo := EncodeDate(StrToInt(Copy(linha, 99, 4)), StrToInt(Copy(linha, 97, 2)), StrToInt(Copy(linha, 95, 2)));
            end
            else if (aBanco = 748) then ////Banco Cooperativo Sicredi S.A. //UNICRED PB
            begin
              DataArquivo := EncodeDate(StrToInt(Copy(linha, 95, 4)), StrToInt(Copy(linha, 99, 2)), StrToInt(Copy(linha, 101, 2)));
            end
            else if (aBanco = 707) then //Banco Daycoval S.A.
            begin
              DataArquivo := EncodeDate(StrToInt('20' + Copy(linha, 99, 2)), StrToInt(Copy(linha, 97, 2)), StrToInt(Copy(linha, 95, 2)));
            end
            else
            begin
              DataArquivo := EncodeDate(StrToInt('20' + Copy(linha, 99, 2)), StrToInt(Copy(linha, 97, 2)), StrToInt(Copy(linha, 95, 2)));
                    ...
                    if aBanco = 409 then
                      ...
                      if aBanco = 237 then
                        ...
                        if aBanco = 389 then
                          ...
                          // 070 - brb
                          if aBanco = 070 then
                            NumeroArquivo := StrToInt(Copy(linha, 99, 4) + Copy(linha, 97, 2) + Copy(linha, 95, 2)) // Data do arquivo
                          else
                            // 756 - bancoob
                            if aBanco = 756 then
                              NumeroArquivo := StrToInt(Trim(Copy(linha, 101, 7)))
                              ...
                              if aBanco = 748 then
                                NumeroArquivo := StrToInt(Trim(Copy(linha, 111, 7)))
                              else
                                // 320 - bicBanco
                                if aBanco = 320 then
                                  NumeroArquivo := StrToInt(Trim(Copy(linha, 109, 7)))
                                else
                                  // 707 - daycoval
                                  if aBanco = 707 then
                                    NumeroArquivo := StrToInt(Trim(Copy(linha, 109, 5)))
                                    ...
                                    begin
                                      NumeroArquivo := StrToInt(Trim(Copy(linha, 109, 5)));
                                      try
                                        dtCredArq := EncodeDate(StrToInt('20' + Copy(linha, 124, 2)), StrToInt(Copy(linha, 122, 2)),
            ...
            aTabela.Close;
            aTabela.SQL.Text := 'select * from retorno where banco_id = ' + IntToStr(BancoID) + ' and numero_arquivo = ' + IntToStr(NumeroArquivo) +' and coalesce(TIPO, 1) = ' + IntToStr(aTipoRet); //
            ...
            begin
              ...
              begin
                ...
                begin
                  try
                    datacredito := EncodeDate(StrToInt('20' + Copy(linha, 180, 2)), StrToInt(Copy(linha, 178, 2)), StrToInt(Copy(linha, 176, 2)));
                  except
                    datacredito := 0;
                  end;
                  Datarecebimento := EncodeDate(StrToInt('20' + Copy(linha, 115, 2)), StrToInt(Copy(linha, 113, 2)), StrToInt(Copy(linha, 111, 2)));
                ...
                begin
                  try
                    datacredito := EncodeDate(StrToInt('20' + Copy(linha, 298, 2)), StrToInt(Copy(linha, 296, 2)), StrToInt(Copy(linha, 294, 2)));
                ...
                begin
                  ...
                  try
                    datacredito := EncodeDate(StrToInt('20' + Copy(linha, 300, 2)), StrToInt(Copy(linha, 298, 2)), StrToInt(Copy(linha, 296, 2)));
                  except
                    datacredito := 0;
                  end;
                ...
                begin
                  try
                    datacredito := EncodeDate(StrToInt('20' + Copy(linha, 115, 2)), StrToInt(Copy(linha, 113, 2)), StrToInt(Copy(linha, 111, 2)));
                  except
                    datacredito := 0;
                  end;
                  Datarecebimento := EncodeDate(StrToInt('20' + Copy(linha, 297, 2)), StrToInt(Copy(linha, 295, 2)), StrToInt(Copy(linha, 293, 2)));
                  CarteiraTmp := StrToInt(Copy(linha, 108, 1));
                end
                ...
                begin
                  try
                    datacredito := EncodeDate(StrToInt(Copy(linha, 304, 4)), StrToInt(Copy(linha, 302, 2)), StrToInt(Copy(linha, 300, 2)));
                  except
                    datacredito := 0;
                  end;
                  try
                    Datarecebimento := EncodeDate(StrToInt(Copy(linha, 115, 4)), StrToInt(Copy(linha, 113, 2)), StrToInt(Copy(linha, 111, 2)));
                  except
                    Datarecebimento := 0;
                  end;
                  CarteiraTmp := 0;
                ...
                if (aBanco = 748) then //Banco Cooperativo Sicredi S.A. //UNICRED PB
                begin
                 // SeuNumeroTmp:='';
                  try
                    datacredito := EncodeDate(StrToInt(Copy(linha, 329, 4)), StrToInt(Copy(linha, 333, 2)), StrToInt(Copy(linha, 335, 2)));
                  except
                    datacredito := 0;
                  end;
                  ...
                  try
                   vlDescto:= (Strtofloat(Copy(linha, 228, 13)) / 100) + (Strtofloat(Copy(linha, 241, 13)) / 100);
                  except
                   vlDescto:=0;
                  end;

                  try
                  dtVencTmp := EncodeDate(StrToInt('20' + Copy(linha, 151, 2)), StrToInt(Copy(linha, 149, 2)), StrToInt(Copy(linha, 147, 2)));
                  except
                  dtVencTmp:=0;
                  end;
                ...
                else if (aBanco = 320) then //Banco Industrial e Comercial S.A.
                begin
                  try
                    datacredito := EncodeDate(StrToInt('20' + Copy(linha, 297, 2)), StrToInt(Copy(linha, 295, 2)), StrToInt(Copy(linha, 293, 2)));
                  except
                    datacredito := 0;
                  end;
                  try
                ...
                begin
                  try
                    datacredito := EncodeDate(StrToInt('20' + Copy(linha, 390, 2)), StrToInt(Copy(linha, 388, 2)), StrToInt(Copy(linha, 386, 2)));
                  except
                    datacredito := 0;
                  end;
                  try
              ...
              try
                ...
                try
                  ...
                  if (aBanco = 341) then //Itaú Unibanco S.A.
                  valorPago := (Strtofloat(Copy(linha, 254, 13)) / 100) + (Strtofloat(Copy(linha, 176, 13)) / 100) //eraldo
                  else
                ...
                NossoNumTmp := Trim(PegaNossoNum(aBanco, aLayout, linha, aCodCed, aConvenio, aCobReg));
                ...
                if (aBanco = 237) and (CodMov = 16) then  //Banco Bradesco S.A.
                begin // cheque
                ...
                if (aBanco = 756) and (Copy(linha,109,2) = '09' ) then //BANCOOB (SICOOB)
                datacredito:=0;
                ...
                begin
                  ...
                  if (aBanco = 748) then  ////Banco Cooperativo Sicredi S.A. //UNICRED PB
                  begin
                    if (Copy(linha, 319, 2) <> '00') and (Copy(linha, 319, 2) <> 'A8') and (Copy(linha, 319, 2) <> 'H5') and (Copy(linha, 319, 2) <> 'H6') and
                      (Copy(linha, 319, 2) <> 'H8') and (Copy(linha, 319, 2) <> 'X1') and (Copy(linha, 319, 2) <> 'X2') and (Copy(linha, 319, 2) <> 'X3') and
                      (Copy(linha, 319, 2) <> 'X4') and (Copy(linha, 319, 2) <> 'X5') then
                      podeGravar := False;

                    if (Copy(linha, 319, 2) <> '03') and (Copy(linha, 319, 2) <> '04') and (Copy(linha, 319, 2) <> '08') and (Copy(linha, 319, 2) <> 'A9') and
                      (Copy(linha, 319, 2) <> 'B1') and (Copy(linha, 319, 2) <> 'B2') and (Copy(linha, 319, 2) <> 'B3') and (Copy(linha, 319, 2) <> 'E1') and
                  ...
                  aTabela.FieldByName('DATA_RECEBIMENTO').Value := Datarecebimento;
                  Tabela.FieldByName('COD_OCORRENCIA').Value    := CodigoOcorrencia;
                    ...
                    try                                                        
                      if (aBanco = 070) then //BRB - Banco de Brasília S.A.
                        dtVencTmp := EncodeDate(StrToInt(Copy(linha, 115, 2)), StrToInt(Copy(linha, 151, 2)), StrToInt(Copy(linha, 149, 2)))
                      else
                        dtVencTmp := EncodeDate(StrToInt('20' + Copy(linha, 151, 2)), StrToInt(Copy(linha, 149, 2)), StrToInt(Copy(linha, 147, 2)));
                    ...
                    //(DateToStr(dtVencTmp));
                    if (dtVencTmp > 0) then begin
                      aTabela.FieldByName('DATA_VENCIMENTO').Value := dtVencTmp;
                    end;

                    if (aBanco = 070) then  //BRB - Banco de Brasília S.A.
                    begin
                      aTabela.FieldByName('CODIGO_AGENCIA').Value := Copy(linha, 173, 5);
                      aTabela.FieldByName('VALOR_DOCUMENTO').Value := Strtofloat(Copy(linha, 157, 13)) / 100;
                      aTabela.FieldByName('VALOR_MORA').Value := (Strtofloat(Copy(linha, 206, 13)) / 100);
                      aTabela.FieldByName('VALOR_MULTA').Value := Strtofloat(Copy(linha, 284, 13)) / 100;
                    end
                    else
                    begin
                      aTabela.FieldByName('CODIGO_AGENCIA').Value := Copy(linha, 169, 5);
                      aTabela.FieldByName('VALOR_DOCUMENTO').Value := Strtofloat(Copy(linha, 153, 13)) / 100;
                      aTabela.FieldByName('VALOR_MORA').Value := (Strtofloat(Copy(linha, 267, 13)) / 100);
                    end;
                    ...
                    begin
                      try
                        if (aCobReg = True) then
                        begin
                          aTabela.FieldByName('VALOR_DESCONTO').Value := (Strtofloat(Copy(linha, 241, 13)) / 100) + (Strtofloat(Copy(linha, 228, 13)) / 100);
                          aTabela.FieldByName('VALOR_MULTA').Value := Strtofloat(Copy(linha, 215, 13)) / 100;
                        end
                        else
                        begin
                          aTabela.FieldByName('VALOR_DESCONTO').Value := (Strtofloat(Copy(linha, 241, 13)) / 100);
                    ...
                    begin
                      try
                        aTabela.FieldByName('VALOR_DESCONTO').Value := (Strtofloat(Copy(linha, 245, 13)) / 100) + (Strtofloat(Copy(linha, 232, 13)) / 100) +
                          (Strtofloat(Copy(linha, 271, 13)) / 100);
                        aTabela.FieldByName('VALOR_IOF').Value := Strtofloat(Copy(linha, 219, 13)) / 100;
                      except
                      end;
                    end
                    else if (aBanco = 748) then ////Banco Cooperativo Sicredi S.A. //UNICRED PB
                    begin
                      try
                        aTabela.FieldByName('VALOR_DESCONTO').Value := (Strtofloat(Copy(linha, 228, 13)) / 100) + (Strtofloat(Copy(linha, 241, 13)) / 100);
                        aTabela.FieldByName('VALOR_MULTA').Value := Strtofloat(Copy(linha, 280, 13)) / 100;
                    ...
                    begin
                      try
                        aTabela.FieldByName('VALOR_DESCONTO').Value := (Strtofloat(Copy(linha, 228, 13)) / 100);
                        aTabela.FieldByName('VALOR_IOF').Value := Strtofloat(Copy(linha, 215, 13)) / 100;
                      except
                      end;
                    end
                    else if (aBanco = 707) then  //Banco Daycoval S.A.
                    ...
                    begin
                      try
                        aTabela.FieldByName('VALOR_DESCONTO').Value := (Strtofloat(Copy(linha, 241, 13)) / 100) + (Strtofloat(Copy(linha, 228, 13)) / 100);
                        aTabela.FieldByName('VALOR_MULTA').Value := Strtofloat(Copy(linha, 202, 13)) / 100;
                        aTabela.FieldByName('VALOR_IOF').Value := Strtofloat(Copy(linha, 215, 13)) / 100;
                      except
                      end;
                    end
                    ...
                    if (aBanco <> 341) and (aBanco <> 356) and (aBanco <> 275) and (aBanco <> 409) and (aBanco <> 237) and (aBanco <> 389) and
                      (aBanco <> 070) and (aBanco <> 707) then
                      try
                        aTabela.FieldByName('VALOR_MULTA').Value := Strtofloat(Copy(linha, 280, 13)) / 100;
                      except
                      end;
                  ...
                  begin
                    ...
                    begin
                      aTabela.FieldByName('SEU_NUMERO').Value := Copy(linha, 117, 10);
                      aTabela.FieldByName('TX_BANCO').Value := Strtofloat(Copy(linha, 182, 7)) / 100;
                    end
                    else if (aBanco = 104) then //Caixa Econômica Federal
                    begin
                      //aTabela.FieldByName('SEU_NUMERO').Value := Copy(linha, 38, 25); ERRADO
                      aTabela.FieldByName('SEU_NUMERO').Value := Copy(linha, 32, 25);//CORRETO ERALDO EM 08/03/18
                      aTabela.FieldByName('TX_BANCO').Value := Strtofloat(Copy(linha, 176, 13)) / 100;
                    end
                    else if (aBanco = 341) then // Itaú Unibanco S.A.
                    begin
                      aTabela.FieldByName('SEU_NUMERO').Value := Copy(linha, 38, 25);
                      aTabela.FieldByName('TX_BANCO').Value := Strtofloat(Copy(linha, 176, 13)) / 100;
                    end
                    else if (aBanco = 356) then //Banco Real S.A.
                    begin
                    ...
                    else if (aBanco = 756) then //Banco Cooperativo do Brasil S.A. - BANCOOB
                    begin
                      aTabela.FieldByName('SEU_NUMERO').Value := Copy(linha, 117, 10);
                      aTabela.FieldByName('TX_BANCO').Value := Strtofloat(Copy(linha, 182, 7)) / 100;
                    end
                    ...
                    begin
                      aTabela.FieldByName('SEU_NUMERO').Value := Copy(linha, 93, 13);
                    end
                    else if (aBanco = 748) then //Banco Cooperativo Sicredi S.A. //UNICRED PB
                    begin
                      //aTabela.FieldByName('SEU_NUMERO').Value := Copy(linha, 117, 10);
                      aTabela.FieldByName('TX_BANCO').Value := Strtofloat(Copy(linha, 153, 13)) / 100;
                    end
                    else if (aBanco = 389) then //Banco Mercantil do Brasil S.A.
                    begin
                      aTabela.FieldByName('SEU_NUMERO').Value := Copy(linha, 38, 25);
                    ...
                    begin
                      aTabela.FieldByName('SEU_NUMERO').Value := Copy(linha, 114, 10);
                      aTabela.FieldByName('TX_BANCO').Value := Strtofloat(Copy(linha, 176, 13)) / 100;
                    end;
                  end; // podeGravarTaxa
                ...
                end else begin //if (datacredito > 0) and (valorPago > 0)   GRAVANDO TODAS AS OCORRENCIAS DO RETORNO
...
begin
  result := False;
  if (aBanco = 1) or (aBanco = 104) then  //Banco do Brasil, Caixa Economica Federal
  begin
    try
      AssignFile(F, aArquivo);
      Reset(F);
      ...
      if ((Copy(linha, 2, 1) = '2') and (Copy(linha, 82, 17) = 'DEBITO AUTOMATICO')) then
      begin
        if (CodConvenio = aConvenio) then
        begin
          DataArquivo := EncodeDate(StrToInt(Copy(linha, 66, 4)), StrToInt(Copy(linha, 70, 2)), StrToInt(Copy(linha, 72, 2)));
          ...
          aTabela.Close;
          aTabela.SQL.Text := 'select * from retorno where banco_id = ' + IntToStr(BancoID) + ' and numero_arquivo = ' + IntToStr(NumeroArquivo) +
            ' and coalesce(TIPO, 1) = ' + IntToStr(aTipoRet);
          aTabela.Open;
          ...
          begin
            ...
            begin
              ...
              begin
                try
                  datacredito := EncodeDate(StrToInt(Copy(linha, 45, 4)), StrToInt(Copy(linha, 49, 2)), StrToInt(Copy(linha, 51, 2)));
                except
                  datacredito := 0;
                end;
                try
                ...
                begin
                  ...
                  begin
                    ...
                    aTabela.FieldByName('CODIGO_BANCO').Value := aBanco;
                    aTabela.FieldByName('DATA_CREDITO').Value := datacredito;
                    aTabela.FieldByName('DATA_RECEBIMENTO').Value := datacredito;
                    aTabela.FieldByName('VALOR_DOCUMENTO').Value := valorPago;
                    aTabela.FieldByName('TIPO').Value := aTipoRet;
                    aTabela.FieldByName('COD_DEB').Value := StrToInt(Trim(Copy(linha, 68, 2)));
                    if (aTabela.FieldByName('COD_DEB').AsInteger in [0,31]) then //18/08/15 eraldo, corrigindo o cod 31 para debito automatico
                      aTabela.FieldByName('VALOR_PAGO').Value := valorPago;
          ...
          Mensagem('O arquivo não pertente a este banco.', mtInformation, [mbOk], mrOk, 0);
      end
      else
        Mensagem('O arquivo de retorno é inválido.', mtInformation, [mbOk], mrOk, 0);
    ...
    Raise Exception.Create('O retorno de débito automático não está disponível para este banco.');
end;
...
var
  ...
  RegAtual, RegTotal, NumeroArquivo, i, CarteiraTmp , CodigoOcorrencia, CodigoOcRejeijao    : Integer;
...
begin
 ...
 // MostrarErro:='';
  ...
  if (aBanco = 1) or (aBanco = 104) or (aBanco = 399) or (aBanco = 033) then // banco do brasil, Caixa Economica Federal, HSBC Bank Brasil S.A, Banco Santander (Brasil) S.A.
  begin
    try
      ...
      try
        ...
        begin
          ...
          if ((Copy(Linha1, 143, 1) = '2') and (Pos(aConvenio, CodConvenio) > 0)) then
            podePassar := True;
        end
        else if (aBanco = 104) then
        begin
          ...
          if Length(aCodCed) <= 6 then
          begin
            if ((Copy(Linha1, 143, 1) = '2') and (Pos(aCodCed, Trim(Copy(Linha1, 59, 6))) > 0)) then//
              podePassar := True;
          end
          else
          begin
            if ((Copy(Linha1, 143, 1) = '2') and (Pos(aCodCed, Trim(Copy(Linha1, 33, 16))) > 0)) then
              podePassar := True;
          end;
        ...
        if (aBanco = 399) then
        begin
          if ((Copy(Linha1, 143, 1) = '2') and (Pos(aAgencia, Trim(Copy(Linha1, 53, 5))) > 0) and (Pos(aConta, Trim(Copy(Linha1, 59, 12))) > 0)) then
            podePassar := True;
        end
        else if (aBanco = 033) then
        begin
          if ((Copy(Linha1, 143, 1) = '2') and (Pos(aAgencia, Trim(Copy(Linha1, 33, 4))) > 0) and (Pos(aConta, Trim(Copy(Linha1, 38, 9))) > 0)) then
            podePassar := True;
        end;


        if Copy(Linha1, 1, 8) <> formatFloat('000', aBanco) + '00000' then begin
        //if (Copy(linha1, 1, 19) <> '02RETORNO01COBRANCA') and (Copy(linha1, 1, 19) <> '02RETORNO01COBRANÇA') then begin //eraldo
          MessageBox(Application.Handle, 'O arquivo de retorno é inválido!', 'SED Soft Informa...', Mb_ok + MB_ICONWARNING);
        end else
        if not(podePassar = True) then begin
          MessageBox(Application.Handle, Pchar('O arquivo ' + aArquivo + ' não pertence a este banco!'), 'SED Soft Informa...', Mb_ok + MB_ICONWARNING)
        end else
        begin
          ...
          DataArquivo := EncodeDate(StrToInt(Copy(Linha1, 148, 4)), StrToInt(Copy(Linha1, 146, 2)), StrToInt(Copy(Linha1, 144, 2)));
          ...
          begin
            ...
            if not((Copy(Linha1, 14, 1) = 'T') and (Copy(Linha2, 14, 1) = 'U')) then
            begin
              Raise Exception.Create('O nº de segmentos do arquivo está variado. Por favor, entre em contato com o administrador do sistema.');
            end
            else
            begin

              try
                Datarecebimento := EncodeDate(StrToInt(Copy(Linha2, 142, 4)), StrToInt(Copy(Linha2, 140, 2)), StrToInt(Copy(Linha2, 138, 2)));
              except
                Datarecebimento := 0;
              end;
              try
                datacredito := EncodeDate(StrToInt(Copy(Linha2, 150, 4)), StrToInt(Copy(Linha2, 148, 2)), StrToInt(Copy(Linha2, 146, 2)));
              except
                datacredito := 0;
              end;
              try
              ...
              NossoNumTmp := Trim(PegaNossoNum(aBanco, aLayout, Linha1, aCodCed, aConvenio, aCobReg));
                ...
                //begin
                  ...
                  begin
                   ...
                   begin
                      ...
                      aTabela.FieldByName('DATA_CREDITO').Value := datacredito;
                      aTabela.FieldByName('DATA_RECEBIMENTO').Value := Datarecebimento;
                      aTabela.FieldByName('COD_OCORRENCIA').Value := CodigoOcorrencia;
                      try
                        if (aBanco = 033) then
                          dtVencTmp := EncodeDate(StrToInt(Copy(Linha1, 74, 4)), StrToInt(Copy(Linha1, 72, 2)), StrToInt(Copy(Linha1, 70, 2)))
                        else
                          dtVencTmp := EncodeDate(StrToInt(Copy(Linha1, 78, 4)), StrToInt(Copy(Linha1, 76, 2)), StrToInt(Copy(Linha1, 74, 2)));
                      except
                        dtVencTmp := 0;
                      end;
                      if (dtVencTmp > 0) then
                      ...
                      if (aBanco = 033) then
                      begin
                        aTabela.FieldByName('VALOR_DOCUMENTO').Value := Strtofloat(Copy(Linha1, 78, 15)) / 100;
                      end
                      else
                      begin
                        aTabela.FieldByName('VALOR_DOCUMENTO').Value := Strtofloat(Copy(Linha1, 82, 15)) / 100;
                      end;

                      aTabela.FieldByName('VALOR_PAGO').Value := valorPago; // Strtofloat(copy(Linha2,78,15))/100;
                      aTabela.FieldByName('VALOR_IOF').Value := Strtofloat(Copy(Linha2, 63, 15)) / 100;
                      // aTabela.FieldByName('VALOR_ABATIMENTO').Value:=(strtofloat(copy(Linha2,33,15))/100);
                      aTabela.FieldByName('VALOR_DESCONTO').Value := (Strtofloat(Copy(Linha2, 33, 15)) / 100) + (Strtofloat(Copy(Linha2, 48, 15)) / 100);

                      vlDescto:= (Strtofloat(Copy(Linha2, 33, 15)) / 100) + (Strtofloat(Copy(Linha2, 48, 15)) / 100);

                      aTabela.FieldByName('VALOR_MORA').Value := Strtofloat(Copy(Linha2, 18, 15)) / 100;
                      ...
                      begin
                        ...
                        aTabela.FieldByName('TX_BANCO').Value := Strtofloat(Copy(Linha1, 199, 15)) / 100;
                      end
                      else if (aBanco = 104) then
                      begin
                        aTabela.FieldByName('SEU_NUMERO').Value := Copy(Linha1, 106, 25);
                      ...
                      begin
                        // aTabela.FieldByName('SEU_NUMERO').Value:=copy(Linha1,106,25);
                        aTabela.FieldByName('SEU_NUMERO').Value := Copy(Linha1, 38, 16);
                        aTabela.FieldByName('TX_BANCO').Value := Strtofloat(Copy(Linha1, 199, 15)) / 100;
                      end
                      else if (aBanco = 033) then
                      begin
                        aTabela.FieldByName('SEU_NUMERO').Value := Copy(Linha1, 55, 15);
                        aTabela.FieldByName('TX_BANCO').Value := Strtofloat(Copy(Linha1, 194, 15)) / 100;
                      end;

                      aTabela.FieldByName('TIPO').Value := aTipoRet;
                      aTabela.Post;
                   end else begin //if (datacredito > 0) and (valorPago > 0)
    ...
    Raise Exception.Create('O leiaute CNAB240 não está disponível para este banco.');
end;
...
begin
  ...
  begin
    campo1 := Copy(codBarra, 1, 4) + Copy(codBarra, 20, 1) + '.' + Copy(codBarra, 21, 4);
    ...
    result := campo1 + Modulo10(campo1) + ' ' + campo2 + Modulo10(campo2) + ' ' + campo3 + Modulo10(campo3) + ' ' + campo4 + ' ' + campo5;
  end;
end;
