...
begin
  ...
  BancodeDados.CsClientes.SQL.Text := 'select * from clientes where tipo = ' +
    QuotedStr('001 - Escola') + ' order by nome';
  BancodeDados.CsClientes.Open;
  BancodeDados.CsClientes.Last;

  if (Assigned(CsClientesForm)) then
    EditClienteID.Enabled := False
  else
    EditClienteID.Enabled := true;
...
begin
  ...
  if not(BancodeDados.LigacoesDT_LIGAR.Value > 0) then
  begin
    Mensagem('Informe a data de ligação.', mtInformation, [mbOk], mrOk, 0);
    EditData.SetFocus;
    PodeIR := False;
  end
  else if (BancodeDados.LigacoesCLI_ID.Value = 0) or
  ...
  else if Trim(BancodeDados.LigacoesASSUNTO.Value) = '' then
  begin
  ...
  begin
    try
      if Mensagem('Confirmar inclusão da ligação?', mtConfirmation,
