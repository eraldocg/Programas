...
type
  ...
  public
    { Public declarations }
    function PegarOCorrenciaRet(CodigoBanco, CodOcorrencia : integer; layout : string) : string;
    function PegarOCorrenciaRejeicao(CodigoBanco, CodOcorrencia, CodigoOcRejeijao: integer; layout: string): string;
  end;
...
function TRetornoForm.PegarOCorrenciaRejeicao(CodigoBanco, CodOcorrencia, CodigoOcRejeijao: integer; layout: string): string;
var
PoderContinuar : Boolean;

begin

 PoderContinuar:= (CodigoBanco=104) and (CodOcorrencia=99) and (layout = 'cnab400'); // and (CodigoOcRejeijao>0);//caixa
 //PoderContinuar:=(CodigoBanco=1) and (CodOcorrencia=3);//banco do brasil

if PoderContinuar=true then begin
  ...
  begin
   ...
   if (layout = 'cnab400') then  begin

    case CodigoOcRejeijao of
      01: Result := 'Remessa sem registro tipo 0';
      02: Result := 'Identificação inválida da Empresa no BANCO';
      ...
      17: Result := 'Identificação no banco inválida (Nosso Número)';
      ...
      56: Result := 'Identificacao inválida da empresa no BANCO';
      57: Result := 'Identificacao inválida da empresa no BANCO';
      58: Result := 'Identificacao inválida da empresa no BANCO';
      59: Result := 'Identificacao inválida da empresa no BANCO';
...
function TRetornoForm.PegarOCorrenciaRet(CodigoBanco, CodOcorrencia : integer; layout : string) : string;
begin
Result := '';

  if CodigoBanco=341 then begin  //Itau
  ...
  if CodigoBanco=1 then begin  //banco do Brasil 

    if (layout = 'cnab240') then begin
      case CodOcorrencia of
        02: Result:= '02 – Entrada confirmada';
        03: Result:= '03 – Entrada Rejeitada';
  ...
  if CodigoBanco=104 then begin  //Caixa
  ...
  if CodigoBanco=237 then begin  //bradesco
  ...
  if CodigoBanco=748 then begin  //SICRED
  ...
  if CodigoBanco=33 then begin  //SANTANDER
  ...
  if CodigoBanco=004 then begin  //NORDESTE      
    case CodOcorrencia of
      02: Result:='02-Entrada Confirmada' ;
      03: Result:='03-Entrada Rejeitada' ;
      06: Result:='06-Liquidação normal' ;
  ...
  if CodigoBanco=756 then begin  //BANCOOB
  ...
  if CodigoBanco=399 then begin  //HSBC
    Case CodOcorrencia Of
      02: Result := '02-Entrada Confirmada';
      03: Result := '03-Entrada Rejeitada';
      06: Result := '06-Liquidação normal';
  ...
  if CodigoBanco=320 then begin  //BIC BANCO  - Banco Industrial e Comercial S.A.
   case CodOcorrencia of
     02: Result:='02-Entrada Confirmada' ;
     03: Result:='03-Entrada Rejeitada' ;
     06: Result:='06-Liquidação normal' ;
...
begin
  ...
  try
    ...
    if anexarelviabanc=1 then cbAnexarRel.Checked:=true else  cbAnexarRel.Checked:=false;
    try
      BancodeDados.Contas.Close;
      BancodeDados.Contas.SQL.Text:= 'select * from tipo_contas where coalesce(oculto,0)=0 order by conta';
...
var
  qryB : TFDQuery;
  i, j, k, CodBanco, ContaID, TipoContaID, NTitulosLvCaixa, Layout, CobReg, tipoConta, sitPreInsc, sitInsc, aNArquivo : Integer;
  ...
  vlrecebido, vlProporcional, vlTotal, PercBol, vlMulta, vlJuros, vlIntegralTmp, vlAPagarTmp, vlTotalLvCaixa, vlBoletosCompara, vlBoletosCompara2, vlTxRecebido: Currency;
...
begin
  ...
  if not BancodeDados.Bancos.Locate('BANCO_NOME', RetornoForm.EditBanco.Text, [loCaseInsensitive]) then
  ...
  begin
    ...
    try
      CodCedente      := IntToStr(StrToInt64(BancodeDados.BancosCODIGO_CEDENTE.Value));
    except
    ...
    AlterandoNomeRet:= (BancodeDados.BancosNOME_ARQ_RET.Value);
     ...
     case BancodeDados.BancosLAYOUT.Value of
        1 :  LayoutRet :='cnab400';
        2 :  LayoutRet :='cbr641';
        3 :  LayoutRet :='cnab240' else
             LayoutRet :='';
     end;

    //CEF
    //if (CodBanco=104) and (BancodeDados.BancosCARTEIRA.Value=24) or (BancodeDados.BancosCARTEIRA.Value=14) then
    VerifNossNum    :=0; // BancodeDados.BancosINIC_NOSS_NUM.Value else VerifNossNum:=0; // carregara esta variavel IniciNosNum
    ...
    begin
      ...
      begin
        ...
        try
          if not Assigned(ListarRetornoForm) then ListarRetornoForm := TListarRetornoForm.Create(Application);
          ListarRetornoForm.PegaArquivos(CaminhoRetorno, '*.ret;*.dat;*.txt;*.crt;*.lid;*.sai', CodCedente, Convenio, Agencia, Conta, CodBanco, Layout, false);
              ...
              begin
                ...
                begin
                  ...
                  begin
                    ...
                    begin
                      RetornoForm.Memo1.Lines.Add(ListarRetornoForm.Temp2ARQUIVO_COMPLETO.AsString);
                    end;
    ...
    begin
      ...
      begin
        ...
        RetornoForm.LbNomeNumero.Caption := 'Arquivo ' + IntToStr(i + 1) + '/' + IntToStr(RetornoForm.Memo1.Lines.Count);
        idsTitulos    := '';
        idsTitProtest :='';
...
//            ('3' + DateToStr(Retorno.DataArquivo));
            ...
            while Pos('\', aNomeArq) > 0 do
              ...
              aDiretorio := Copy(Retorno.Arquivo, 1, Length(Retorno.Arquivo) - Length(aNomeArq));

            RetornoForm.LbNomeOperacao.Caption := 'Preparando Dados...';
            BancodeDados.Adiciona.SQL.Text := 'delete from livro_caixa where numero_arquivo = ' + IntToStr(Retorno.NArquivo)+' and coalesce(boleto_id,0)=0';
            ...
            BancodeDados.Retorno.SQL.Text:='select * from retorno where banco_id = ' + IntToStr(BancoID)+ ' and numero_arquivo = '+IntToStr(Retorno.NArquivo)+' and (tipo = ' + IntToStr(Retorno.TipoRet)+')'; //eraldo  18/08/15

            //TipoRet=1 arquivo de CNAB400; //2 : arquivo de debito automatico //3 : arquivo de CNAB240
            if Retorno.TipoRet=2 then  //and coalesce(cod_deb, 0) = 0';   //eraldo  18/08/15
            BancodeDados.Retorno.SQL.Add(' and cod_deb in (0, 31)');

            if VerifNossNum>0 then //eraldo inclui esta linda para filtrar os registros de retorno da mesma conta com unidades diferentes 08/01/16
            BancodeDados.Retorno.SQL.Add(' and (UPPER(nosso_numero) '+PrincipalForm.csContainingLIKE(IntToStr(VerifNossNum) ,0)+')');
            ...
            if not Assigned(VisualizaRetornoForm) then VisualizaRetornoForm := TVisualizaRetornoForm.Create(Application);
            try
              VisualizaRetornoForm.Caption := 'Lista de Retorno -  Arquivo N.º ' + IntToStr(Retorno.NArquivo);

             if (Retorno.RegistroComProblema.Count>0) then begin
               ...
               if FileExists(DiretorioTemp + 'RetornoTmp.xml') then DeleteFile(DiretorioTemp + 'RetornoTmp.xml');
                ...
                begin
                  ...
                  try
                    ...
                    begin
                      ...
                      CamposRegistro.DelimitedText := Retorno.RegistroComProblema.Strings[iReg];
                      RetornoForm.RetornoTmpNUMERO_ARQUIVO.Value := StrToInt(CamposRegistro.Strings[0]);
                      if CamposRegistro.Strings[1] <> '30/12/1899' then
                      RetornoForm.RetornoTmpDATA_ARQUIVO.Value := StrToDate(CamposRegistro.Strings[1]);
                      RetornoForm.RetornoTmpNOSSO_NUMERO.Value := CamposRegistro.Strings[2];

                      if CamposRegistro.Strings[3] <> '30/12/1899' then
                      RetornoForm.RetornoTmpDATA_CREDITO.Value := date;//StrToDate(CamposRegistro.Strings[3]);

                      if CamposRegistro.Strings[4] <> EmptyStr then
                      begin
                      CamposRegistro.Strings[4] :=  ReplaceStr(ReplaceStr(CamposRegistro.Strings[4],',',''),'.','');
                      CamposRegistro.Strings[4] := formatfloat('#0.00',StrToInt(CamposRegistro.Strings[4])/100);
                        RetornoForm.RetornoTmpVALOR_PAGO.Value      := strtofloat(CamposRegistro.Strings[4]);
                      end;

                      if CamposRegistro.Strings[5] <> '30/12/1899' then
                       RetornoForm.RetornoTmpDATA_RECEBIMENTO.Value := StrToDate(CamposRegistro.Strings[5]);

                      if StrToInt(CamposRegistro.Strings[6])>0 then
                      RetornoForm.RetornoTmpCODIGO_BANCO.Value       := StrToInt(CamposRegistro.Strings[6]);
                      if StrToInt(CamposRegistro.Strings[7])>0 then
                      RetornoForm.RetornoTmpTIPO.Value               := StrToInt(CamposRegistro.Strings[7]);
                      RetornoForm.RetornoTmpCODIGO_AGENCIA.Value     := CamposRegistro.Strings[8];
                      RetornoForm.RetornoTmpNUMERO_CONTA.Value       := CamposRegistro.Strings[9];

                      if StrToInt(CamposRegistro.Strings[10])>0 then
                      RetornoForm.RetornoTmpBANCO_ID.Value           := StrToInt(CamposRegistro.Strings[10]);

                      if Trim(CamposRegistro.Strings[11])<>'' then
                      RetornoForm.RetornoTmpSEU_NUMERO.Value     := (CamposRegistro.Strings[11]);

                      if StrToInt(CamposRegistro.Strings[12])>0 then
                      RetornoForm.RetornoTmpCOD_OCORRENCIA.Value     := StrToInt(CamposRegistro.Strings[12]);

                      if CamposRegistro.Strings[13] <> EmptyStr then
                      begin
                      CamposRegistro.Strings[13] :=  ReplaceStr(ReplaceStr(CamposRegistro.Strings[13],',',''),'.','');
                      CamposRegistro.Strings[13] := formatfloat('#0.00',StrToInt(CamposRegistro.Strings[13])/100);
                      RetornoForm.RetornoTmpVALOR_DOCUMENTO.Value := strtofloat(CamposRegistro.Strings[13]);
                      end;



                      if CamposRegistro.Strings[14] <> '30/12/1899' then
                       RetornoForm.RetornoTmpDATA_VENCIMENTO.Value := StrToDate(CamposRegistro.Strings[14]);



                      if StrToInt(CamposRegistro.Strings[15])>0 then
                      RetornoForm.RetornoTmpCODIGO_REJEICAO.Value     := StrToInt(CamposRegistro.Strings[15]);


                      if CamposRegistro.Strings[16] <> EmptyStr then
                      begin
                      CamposRegistro.Strings[16] :=  ReplaceStr(ReplaceStr(CamposRegistro.Strings[16],',',''),'.','');
                      CamposRegistro.Strings[16] := formatfloat('#0.00',StrToInt(CamposRegistro.Strings[16])/100);
                      RetornoForm.RetornoTmpVALOR_DESCONTO.Value := strtofloat(CamposRegistro.Strings[16]);
                      end;


                    RetornoForm.qryV_Boletos.Close;
                    RetornoForm.qryV_Boletos.SQL.Text:='select cli_id, cli_nome, cli_cnpj_cpf, emissao_id, parcela, nosso_numero from v_boletos where cod_banco = '+ IntToStr(RetornoForm.RetornoTmpCODIGO_BANCO.Value);

                     if (BancodeDados.qryBancoBAIX_BOL_PARAM.Value>0) then
                     begin //OK
                        if (RetornoForm.RetornoTmpTIPO.Value <> 2) then begin //2
                          RetornoForm.qryV_Boletos.SQL.Add(' AND NOSSO_NUMERO = ' + QuotedStr(RetornoForm.RetornoTmpNOSSO_NUMERO.Value));
                          ...
                          RetornoForm.qryV_Boletos.SQL.Add(' AND EMISSAO_ID = ' + QuotedStr(RetornoForm.RetornoTmpSEU_NUMERO.Value))
                           ...
                           begin
                            ...
                            RetornoForm.qryV_Boletos.SQL.Add(' AND VENCIMENTO ='+QuotedStr(FormatDateTime('yyyy/mm/dd',RetornoForm.RetornoTmpDATA_VENCIMENTO.Value)));

                            if RetornoForm.RetornoTmpVALOR_DOCUMENTO.Value>0 then
                            RetornoForm.qryV_Boletos.SQL.Add(' AND VALOR_DOC ='+CurrToStr(RetornoForm.RetornoTmpVALOR_DOCUMENTO.Value));

                            if RetornoForm.RetornoTmpVALOR_DESCONTO.Value>0 then
                            RetornoForm.qryV_Boletos.SQL.Add(' AND DESCONTO ='+CurrToStr(RetornoForm.RetornoTmpVALOR_DESCONTO.Value));
                      ...
                      begin //1
                          ...
                          RetornoForm.qryV_Boletos.SQL.Add(' AND BANCO_ID = ' + IntToStr(RetornoForm.RetornoTmpBANCO_ID.Value));
                        if (RetornoForm.RetornoTmpTIPO.Value = 2) then begin
                          RetornoForm.qryV_Boletos.SQL.Add(' AND emissao_id = ' + QuotedStr(RetornoForm.RetornoTmpSEU_NUMERO.Value))
                        end else
                        begin
                          RetornoForm.qryV_Boletos.SQL.Add(' AND nosso_numero = ' + QuotedStr(RetornoForm.RetornoTmpNOSSO_NUMERO.Value));
...
//                      (RetornoForm.qryV_Boletos.SQL.Text);
                      ...
                      if not RetornoForm.qryV_Boletos.IsEmpty then begin

                       if trim(RetornoForm.qryV_BoletosCLI_NOME.Value)<>'' then begin
                       RetornoForm.RetornoTmpOBS_1.Value         :='CLIENTE: '+RetornoForm.qryV_BoletosCLI_NOME.Value+' CÓDIGO: '+FormatFloat('000000',RetornoForm.qryV_BoletosCLI_ID.Value);
                       RetornoForm.RetornoTmpNOME_SACADO.Value         :=RetornoForm.qryV_BoletosCLI_NOME.Value;

                       end;
                       if trim(RetornoForm.qryV_BoletosPARCELA.Value)<>'' then
                       RetornoForm.RetornoTmpOBS_2.Value         :=' | PARCELA: '+RetornoForm.qryV_BoletosPARCELA.Value+' EMISSÃO: '+FormatFloat('000000',RetornoForm.qryV_BoletosEMISSAO_ID.Value);

                       if trim(RetornoForm.qryV_BoletosCLI_CNPJ_CPF.Value)<>'' then
                       RetornoForm.RetornoTmpNOME_SACADO.Value   :=' | C.P.F: '+RetornoForm.qryV_BoletosCLI_CNPJ_CPF.Value;

                       RetornoForm.RetornoTmpEMISSAO_ID.Value     :=RetornoForm.qryV_BoletosEMISSAO_ID.Value;
                      end else
                      RetornoForm.RetornoTmpOBS_1.Value         :='Emissão não encontrada';


                      if (RetornoForm.RetornoTmpCODIGO_REJEICAO.AsInteger>0) and not (RetornoForm.RetornoTmpCOD_OCORRENCIA.AsInteger=1) then
                      RetornoForm.RetornoTmpREJEICAO_NOME.Value := RetornoForm.PegarOCorrenciaRejeicao(RetornoForm.RetornoTmpCODIGO_BANCO.AsInteger, RetornoForm.RetornoTmpCOD_OCORRENCIA.AsInteger, RetornoForm.RetornoTmpCODIGO_REJEICAO.AsInteger, LayoutRet);


                      if trim(RetornoForm.RetornoTmpREJEICAO_NOME.AsString)<>'' then
                      RetornoForm.RetornoTmpOCORRENCIA_NOME.Value := RetornoForm.PegarOCorrenciaRet(RetornoForm.RetornoTmpCODIGO_BANCO.AsInteger, RetornoForm.RetornoTmpCOD_OCORRENCIA.AsInteger, LayoutRet) +': '+copy(trim(RetornoForm.RetornoTmp.Fieldbyname('REJEICAO_NOME').AsString),0,100) else
                      RetornoForm.RetornoTmpOCORRENCIA_NOME.Value := RetornoForm.PegarOCorrenciaRet(RetornoForm.RetornoTmpCODIGO_BANCO.AsInteger, RetornoForm.RetornoTmpCOD_OCORRENCIA.AsInteger, LayoutRet);
                   ...
                   nomeCampoImpp  :='';
                   if not RetornoForm.RetornoTmp.IsEmpty then begin
                     Screen.Cursor := crSQLWait;
                     RetornoForm.RetornoTmp.First;
                     while not RetornoForm.RetornoTmp.eof do begin
                      Application.ProcessMessages;
                       BancodeDados.Retorno_Ocor.Close;
                       BancodeDados.Retorno_Ocor.SQL.Text:='select * from retorno_ocorrencia where banco_id = ' + IntToStr(RetornoForm.RetornoTmpBANCO_ID.Value)+ ' and numero_arquivo = '+IntToStr(RetornoForm.RetornoTmpNUMERO_ARQUIVO.Value)+ ' AND NOSSO_NUMERO = ' + QuotedStr(RetornoForm.RetornoTmpNOSSO_NUMERO.Value)+' and (tipo = ' + IntToStr(RetornoForm.RetornoTmpTIPO.Value)+')';
                       //(BancodeDados.Retorno_Ocor.SQL.Text);
                       BancodeDados.Retorno_Ocor.Open;

                         if BancodeDados.Retorno_Ocor.IsEmpty then BancodeDados.Retorno_Ocor.Append else bancodeDados.Retorno_Ocor.Edit;

                          BancodeDados.Retorno_Ocor.Fieldbyname('VALOR_DOCUMENTO').AsInteger :=RetornoForm.RetornoTmp.Fieldbyname('VALOR_DOCUMENTO').AsInteger;



                          for K:=0 to BancodeDados.Retorno_Ocor.FieldCount - 1 do begin
                            nomeCampoImpp:=BancodeDados.Retorno_Ocor.Fields[K].FieldName;
                            try
                            if not (nomeCampoImpp='ATIVO') and not (nomeCampoImpp='RET_OC_ID') and not (nomeCampoImpp='DT_ALTERADO') and not (nomeCampoImpp='USUARIO_ID') and not (nomeCampoImpp='USR_CAD') and not (nomeCampoImpp='VALOR_DOCUMENTO')  then
                            BancodeDados.Retorno_Ocor.FieldByName(nomeCampoImpp).Value:=RetornoForm.RetornoTmp.FieldByName(nomeCampoImpp).Value;
                            except
                            end;
                          end; //deixar antes do post;
                          BancodeDados.Retorno_Ocor.Post;

                          if (RetornoForm.RetornoTmpCODIGO_BANCO.Value=104) then begin//verificando o cnab400
                            BancodeDados.QrySql.close;
                            BancodeDados.QrySql.SQL.Text:='select layout as cnab from bancos where banco_id = ' + IntToStr(RetornoForm.RetornoTmpBANCO_ID.Value);
                            BancodeDados.QrySql.Open;
                          end;

                            ///cnab400
                        if (RetornoForm.RetornoTmpCODIGO_BANCO.Value=104)  and (RetornoForm.RetornoTmpEMISSAO_ID.Value>0) and (BancodeDados.QrySql.FieldByName('CNAB').AsInteger=1) and (RetornoForm.RetornoTmpCOD_OCORRENCIA.Value=1) then
                        begin
                          try
                          BancodeDados.Adiciona.SQL.Text := 'update boletos_emissao set cod_mov = 2 where emissao_id = ' + IntToStr(RetornoForm.RetornoTmpEMISSAO_ID.Value);
                          BancodeDados.Adiciona.ExecSQL;
                          except
                          end;
                        end else
                        if (RetornoForm.RetornoTmpEMISSAO_ID.Value>0) and (RetornoForm.RetornoTmpCOD_OCORRENCIA.Value=2) then
                        begin
                          try
                          BancodeDados.Adiciona.SQL.Text := 'update boletos_emissao set cod_mov =' + IntToStr(RetornoForm.RetornoTmpCOD_OCORRENCIA.Value) + ' WHERE EMISSAO_ID = ' + IntToStr(RetornoForm.RetornoTmpEMISSAO_ID.Value);
                          BancodeDados.Adiciona.ExecSQL;
                          except
                          end;
                        end;
                    ...
                    if Retorno.RegistroComProblema.Count >0 then begin
                      ...
                      try
                        ...
                        QryCs.Close;
                        QryCs.SQL.Text :='select layout from bancos where banco_id = '+IntToStr(RetornoForm.RetornoTmpBANCO_ID.AsInteger);
                        QryCs.Open;
                    ...
                    on E:Exception do
                    begin
                      MessageDlg('Ocorreu um erro: '+#13+e.Message, mtWarning, [mbOK], 0);
                      Cancel;
                      Break;
                    end;
             ...
             end else begin //RegistroComProblema
              ...
              begin
                ...
                RetornoForm.LbNomeOperacao.Caption := 'Processando Informações...';
                ...
                begin
                  ...
                  begin
                    ...
                    if not BaixaBolPar then begin
                    BancodeDados.Emissoes.SQL.Text := 'select * from boletos_emissao where banco_id = ' + IntToStr(BancodeDados.RetornoBANCO_ID.Value);
                    ...
                    if (Retorno.TipoRet = 2) then // Débito em conta
                      BancodeDados.Emissoes.SQL.Add(' and emissao_id = ' + Trim(BancodeDados.RetornoSEU_NUMERO.Value))
                      ...
                      BancodeDados.Emissoes.SQL.Add(' and nosso_numero = ' + QuotedStr(BancodeDados.RetornoNOSSO_NUMERO.Value));
                        ...
                        BancodeDados.V_Boletos.SQL.Text := 'select * from v_boletos where cod_banco = ' + IntToStr(BancodeDados.RetornoCODIGO_BANCO.Value)+ ' AND nosso_numero = ' + QuotedStr(BancodeDados.RetornoNOSSO_NUMERO.Value);
                        ...
                        BancodeDados.V_Boletos.SQL.Add(' AND EMISSAO_ID = ' + QuotedStr(BancodeDados.RetornoSEU_NUMERO.Value))
                          ...
                          BancodeDados.V_Boletos.SQL.Add(' AND DESCONTO ='+CurrToStr(BancodeDados.RetornoVALOR_DESCONTO.Value));
                    ...
                    begin
                      ...
                      if not (BancodeDados.Emissoes.State in [dsedit]) then BancodeDados.Emissoes.Edit;
                     ...
                     // if not (BancodeDados.EmissoesDATA_RECEBIMENTO.FieldKind in [fkCalculated, fkLookup, fkInternalCalc]) then
                      ...
                      BancodeDados.EmissoesDATA_RECEBIMENTO.Value:=BancodeDados.RetornoDATA_RECEBIMENTO.Value;
                     ...
                     // if not (BancodeDados.EmissoesCODIGO_BANCO.FieldKind in [fkCalculated, fkLookup, fkInternalCalc]) then
                      ...
                      BancodeDados.EmissoesCODIGO_BANCO.Value:=BancodeDados.RetornoCODIGO_BANCO.Value;

                      BancodeDados.Emissoes.Post;
                      //após gravar na tabela de emissoes, salvar

                      BancodeDados.Boletos.Close;
                      BancodeDados.Boletos.SQL.Text:= PrincipalForm.SQLrepositorio(0) + ' and b.boleto_id in ('+ Trim(BancodeDados.EmissoesID_BOLETOS.Value) +') and coalesce(b.duplic_bol_id, 0) = 0';
                      ...
                      begin
                        ...
                        try
                          BancodeDados.Boletos.Close;
                          BancodeDados.Boletos.SQL.Text := PrincipalForm.SQLrepositorio(0) + ' and b.emissao_id = ' + IntToStr(BancodeDados.EmissoesEMISSAO_ID.Value)+' and coalesce(b.duplic_bol_id, 0) = 0';
                          BancodeDados.Boletos.Open;
                        except
                        end;
                      ...
                      begin
                        ...
                        begin
                          vlBoletosCompara := vlBoletosCompara + (BancodeDados.BoletosVALOR_INTEGRAL.Value + BancodeDados.BoletosACRESCIMO.Value +
                        ...
                        begin
                          ...
                          while not BancodeDados.Boletos.Eof do
                          begin
                              vlAPagarTmp := vlAPagarTmp   + BancodeDados.BoletosVALOR_APAGAR.Value;
                            vlIntegralTmp := vlIntegralTmp + BancodeDados.BoletosVALOR_INTEGRAL.Value + BancodeDados.BoletosACRESCIMO.Value +
                              BancodeDados.BoletosACRESC_EXTRA.Value;
                            BancodeDados.Boletos.Next;
                          end;
                          ...
                          if BancodeDados.RetornoVALOR_PAGO.Value>0 then
                          vlrecebido := BancodeDados.RetornoVALOR_PAGO.Value else vlrecebido := BancodeDados.RetornoVALOR_DOCUMENTO.Value;   // eraldo 14/12/15  verificado para atende rum chamado de andre olimpo.
                          ...
                          begin
                           ...
                           // BancodeDados.Opcoes_Pagar.open;
                            ...
                            if (BancodeDados.BoletosSITUACAO_BOLETO.Value = 'RECEBIDO') then begin

                              if (BancodeDados.BoletosRETORNO_ID.Value > 0) {and (BancodeDados.Opcoes_PagarTIPO.Value in [3, 4])} then begin
                                if (BancodeDados.BoletosRETORNO_ID.Value <> BancodeDados.RetornoRETORNO_ID.Value) then
                                begin
                                  BancodeDados.QryRetorno.Close;
                                  BancodeDados.QryRetorno.SQL.Text := 'select * from retorno where retorno_id = ' + IntToStr(BancodeDados.BoletosRETORNO_ID.Value);
                                

                                  if VerifNossNum>0 then //eraldo inclui esta linda para filtrar os registros de retorno da mesma conta com unidades diferentes  08/01/16
                                  BancodeDados.QryRetorno.SQL.Add(' and (UPPER(nosso_numero) '+PrincipalForm.csContainingLIKE(IntToStr(VerifNossNum) ,0)+')');


                                  BancodeDados.QryRetorno.Open;
                                  if not BancodeDados.QryRetorno.IsEmpty then
                                  begin
                                    if (BancodeDados.QryRetornoNOSSO_NUMERO.Value <> BancodeDados.RetornoNOSSO_NUMERO.Value) then
                                    begin
                                      ...
                                      RetornoForm.TitQuitado2RESP_ID.Value              := BancodeDados.BoletosCLI_ID.Value;
                                      RetornoForm.TitQuitado2RESP_NOME.AsString         := BancodeDados.BoletosCLIENTE.Value;
                                      RetornoForm.TitQuitado2BOLETO_ID.Value            := BancodeDados.BoletosBOLETO_ID.Value;
                                      RetornoForm.TitQuitado2PARCELA.AsString           := BancodeDados.BoletosPARCELA.Value;
                                      //RetornoForm.TitQuitado2MATRICULA_ID.Value         := BancodeDados.BoletosMATRICULA_ID.Value;
                                      RetornoForm.TitQuitado2ALUNO.AsString             := BancodeDados.BoletosCLIENTE.Value;
                                      RetornoForm.TitQuitado2EMISSAO.Value              := BancodeDados.BoletosEMISSAO.Value;
                                      RetornoForm.TitQuitado2VENCIMENTO.Value           := BancodeDados.BoletosVENCIMENTO.Value;
                                      RetornoForm.TitQuitado2VALOR_INTEGRAL.Value       := BancodeDados.BoletosVALOR_INTEGRAL.Value;
                                      RetornoForm.TitQuitado2VALOR_APAGAR.Value         := BancodeDados.BoletosVALOR_APAGAR.Value;
                                      RetornoForm.TitQuitado2VALOR_PAGO.Value           := BancodeDados.BoletosVALOR_PAGO.Value;
                                      RetornoForm.TitQuitado2SITUACAO_BOLETO.AsString   := BancodeDados.BoletosSITUACAO_BOLETO.Value;
                                      RetornoForm.TitQuitado2RET1_RETORNO_ID.Value      := BancodeDados.QryRetornoRETORNO_ID.Value;
                                      RetornoForm.TitQuitado2RET1_NUMERO_ARQUIVO.Value  := BancodeDados.QryRetornoNUMERO_ARQUIVO.Value;
                                      RetornoForm.TitQuitado2RET1_DATA_ARQUIVO.Value    := BancodeDados.QryRetornoDATA_ARQUIVO.Value;
                                      RetornoForm.TitQuitado2RET1_NOSSO_NUMERO.AsString := BancodeDados.QryRetornoNOSSO_NUMERO.Value;
                                      RetornoForm.TitQuitado2RET1_DATA_CREDITO.Value    := BancodeDados.QryRetornoDATA_CREDITO.Value;
                                      RetornoForm.TitQuitado2RET1_DATA_VENCIMENTO.Value := BancodeDados.QryRetornoDATA_VENCIMENTO.Value;
                                      RetornoForm.TitQuitado2RET1_DATA_RECEBIMENTO.Value:= BancodeDados.QryRetornoDATA_RECEBIMENTO.Value;
                                      RetornoForm.TitQuitado2RET1_VALOR_DOCUMENTO.Value := BancodeDados.QryRetornoVALOR_DOCUMENTO.Value;
                                      RetornoForm.TitQuitado2RET1_VALOR_MORA.Value      := BancodeDados.QryRetornoVALOR_MORA.Value;
                                      RetornoForm.TitQuitado2RET1_VALOR_ABATIMENTO.Value:= BancodeDados.QryRetornoVALOR_DESCONTO.Value;
                                      RetornoForm.TitQuitado2RET1_VALOR_PAGO.Value      := BancodeDados.QryRetornoVALOR_PAGO.Value;
                                      RetornoForm.TitQuitado2RET1_NOME_ARQ.AsString     := BancodeDados.QryRetornoNOME_ARQ.Value;
                                      RetornoForm.TitQuitado2RET2_RETORNO_ID.Value      := BancodeDados.RetornoRETORNO_ID.Value;
                                      RetornoForm.TitQuitado2RET2_NUMERO_ARQUIVO.Value  := BancodeDados.RetornoNUMERO_ARQUIVO.Value;
                                      RetornoForm.TitQuitado2RET2_DATA_ARQUIVO.Value    := BancodeDados.RetornoDATA_ARQUIVO.Value;
                                      RetornoForm.TitQuitado2RET2_NOSSO_NUMERO.AsString := BancodeDados.RetornoNOSSO_NUMERO.Value;
                                      RetornoForm.TitQuitado2RET2_DATA_CREDITO.Value    := BancodeDados.RetornoDATA_CREDITO.Value;
                                      RetornoForm.TitQuitado2RET2_DATA_VENCIMENTO.Value := BancodeDados.RetornoDATA_VENCIMENTO.Value;
                                      RetornoForm.TitQuitado2RET2_DATA_RECEBIMENTO.Value:= BancodeDados.RetornoDATA_RECEBIMENTO.Value;
                                      RetornoForm.TitQuitado2RET2_VALOR_DOCUMENTO.Value := BancodeDados.RetornoVALOR_DOCUMENTO.Value;
                                      RetornoForm.TitQuitado2RET2_VALOR_MORA.Value      := BancodeDados.RetornoVALOR_MORA.Value;
                                      RetornoForm.TitQuitado2RET2_VALOR_ABATIMENTO.Value:= BancodeDados.RetornoVALOR_ABATIMENTO.Value;
                                      RetornoForm.TitQuitado2RET2_VALOR_PAGO.Value      := BancodeDados.RetornoVALOR_PAGO.Value;
                                      RetornoForm.TitQuitado2RET2_NOME_ARQ.AsString     := BancodeDados.RetornoNOME_ARQ.Value;
                                      RetornoForm.TitQuitado2.Post;

                                      RetornoForm.BoletosDup.Close;
                                      RetornoForm.BoletosDup.SQL.Text := 'select b.*, (select nome from clientes where cli_id=b.cli_id) AS cliente from boletos b where b.duplic_bol_id = ' + IntToStr(BancodeDados.BoletosBOLETO_ID.Value) + ' and coalesce(b.duplic_ret_id, 0) = 0';
                                      RetornoForm.BoletosDup.Open;

                                      if RetornoForm.BoletosDup.IsEmpty then
                                      begin
                                        RetornoForm.BoletosDup.Append;
                                        for j := 0 to RetornoForm.BoletosDup.FieldCount - 1 do
                                        begin
                                          nomeCampo := RetornoForm.BoletosDup.Fields[j].FieldName;
                                          if (nomeCampo <> 'DUPLIC_BOL_ID') and (nomeCampo <> 'DUPLIC_RET_ID') then
                                            RetornoForm.BoletosDup.Fields[j].Value := BancodeDados.Boletos.FieldByName(nomeCampo).Value;
                                        end;
                                        if RemotoDriveID in [2] then
                                        RetornoForm.BoletosDupBOLETO_ID.Value  := BancodeDados.MySqlPegaID('BOLETOS')
                                        else
                                        RetornoForm.BoletosDupBOLETO_ID.Value := BancodeDados.PegaNovo('GEN_BOLCOD');

                                        RetornoForm.BoletosDupDUPLIC_BOL_ID.Value := BancodeDados.BoletosBOLETO_ID.Value;
                                        //RetornoForm.BoletosDupUSR_CAD.Value := CodUsuario;
                                        //RetornoForm.BoletosDupLOJA_ID.Value := 0;
                                        RetornoForm.BoletosDupEMISSAO.Value := PrincipalForm.DataServidor;
                                        RetornoForm.BoletosDupDESCRICAO.Value := Copy('(TÍTULO DUPLICADO) ' + RetornoForm.BoletosDupDESCRICAO.Value, 1, 60);
                                    ...
                                    begin
                                      ...
                                      BancodeDados.qryPesqRet.Params[0].Value := BancodeDados.RetornoBANCO_ID.Value;
                                      BancodeDados.qryPesqRet.Params[1].Value := BancodeDados.RetornoNOSSO_NUMERO.Value;
                                      BancodeDados.qryPesqRet.Params[2].Value := BancodeDados.RetornoNUMERO_ARQUIVO.Value;
                                      BancodeDados.qryPesqRet.Params[3].Value := Retorno.TipoRet;
                                      BancodeDados.qryPesqRet.Open;
                                      if not BancodeDados.qryPesqRet.IsEmpty then
                                      begin

                                        if Trim(nossoNumDup) = '' then
                                          nossoNumDup := QuotedStr(BancodeDados.RetornoNOSSO_NUMERO.Value)
                                        else
                                          nossoNumDup := nossoNumDup + ', ' + QuotedStr(BancodeDados.RetornoNOSSO_NUMERO.Value);

                                        RetornoForm.BoletosDup.Close;
                                        RetornoForm.BoletosDup.SQL.Text := 'select b.*, (select nome from clientes where cli_id=b.cli_id) AS cliente from boletos b where duplic_bol_id = ' + IntToStr(BancodeDados.BoletosBOLETO_ID.Value) + ' and b.duplic_ret_id = ' + IntToStr(BancodeDados.BoletosRETORNO_ID.Value);
                                        RetornoForm.BoletosDup.Open;
                                        if RetornoForm.BoletosDup.IsEmpty then
                                        begin
                                          RetornoForm.BoletosDup.Append;
                                          ...
                                          RetornoForm.BoletosDupBOLETO_ID.Value := BancodeDados.PegaNovo('GEN_BOLCOD');
                                          RetornoForm.BoletosDupDUPLIC_BOL_ID.Value := BancodeDados.BoletosBOLETO_ID.Value;
                                          RetornoForm.BoletosDupDUPLIC_RET_ID.Value := BancodeDados.BoletosRETORNO_ID.Value;
                                          //RetornoForm.BoletosDupUSR_CAD.Value := CodUsuario;
                                          //RetornoForm.BoletosDupLOJA_ID.Value := 0;
                                          RetornoForm.BoletosDupEMISSAO.Value := PrincipalForm.DataServidor;
                                          RetornoForm.BoletosDupDESCRICAO.Value := Copy('(BOLETO DUPLICADO) ' + RetornoForm.BoletosDupDESCRICAO.Value, 1, 60);
                                          // RetornoForm.BoletosDupSITUACAO_BOLETO.Value := 'RECEBIDO';
                                          // RetornoForm.BoletosDupEMISSAO.Value := Date;
                              ...
                              end else begin //eraldo incluiu está funcão para tratamento de tiutlos pagos na escola e posteriormente pagos no banco

                                  RetornoForm.qryPegarRetorn.Open;
                                  RetornoForm.qryPegarRetorn.SQL.Text:='select r.retorno_id from retorno r, boletos_emissao e where r.nosso_numero=e.nosso_numero and (e.id_boletos '+ PrincipalForm.csContainingLIKE( IntToStr(BancodeDados.BoletosBOLETO_ID.Value) , 1) +') and e.banco_id ='+IntToStr(BancoID);

                                  RetornoForm.qryPegarRetorn.Open;

                                  BancodeDados.QryRetorno.Close;
                                  BancodeDados.QryRetorno.SQL.Text := 'select * from retorno where retorno_id = ' + IntToStr(RetornoForm.qryPegarRetornRETORNO_ID.Value);
                                  ...
                                  begin
                                      ...
                                      RetornoForm.TitQuitado2RET1_VALOR_PAGO.Value      := BancodeDados.QryRetornoVALOR_PAGO.Value;
                                      RetornoForm.TitQuitado2RET1_NOME_ARQ.AsString     := BancodeDados.QryRetornoNOME_ARQ.Value;
                                      RetornoForm.TitQuitado2RET2_RETORNO_ID.Value      := BancodeDados.QryRetornoRETORNO_ID.Value;
                                      RetornoForm.TitQuitado2RET2_NUMERO_ARQUIVO.Value  := BancodeDados.RetornoNUMERO_ARQUIVO.Value;
                                      RetornoForm.TitQuitado2RET2_DATA_ARQUIVO.Value    := BancodeDados.RetornoDATA_ARQUIVO.Value;
                                      RetornoForm.TitQuitado2RET2_NOSSO_NUMERO.AsString := BancodeDados.RetornoNOSSO_NUMERO.Value;
                                      RetornoForm.TitQuitado2RET2_DATA_CREDITO.Value    := BancodeDados.RetornoDATA_CREDITO.Value;
                                      ...
                                      begin
                                       ...
                                       if not (BancodeDados.BoletosRETORNO_ID.Value>0) then begin
                                          if not (BancodeDados.Boletos.state in [dsEdit]) then BancodeDados.Boletos.Edit;
                                          BancodeDados.BoletosRETORNO_ID.Value := (BancodeDados.QryRetornoRETORNO_ID.Value);
                                    ...
                                    begin
                                      ...
                                      begin
                                        ...
                                        RetornoForm.BoletosDup.Close;
                                        RetornoForm.BoletosDup.SQL.Text := 'select b.*, (select nome from clientes where cli_id=b.cli_id) AS cliente from boletos b where b.duplic_bol_id = ' +IntToStr(BancodeDados.BoletosBOLETO_ID.Value) + ' and b.duplic_ret_id = ' + IntToStr(BancodeDados.QryRetornoRETORNO_ID.Value);
                                        RetornoForm.BoletosDup.Open;
                                        if RetornoForm.BoletosDup.IsEmpty then
                                        begin
                                          RetornoForm.BoletosDup.Append;
                                          ...
                                          RetornoForm.BoletosDupBOLETO_ID.Value     := BancodeDados.PegaNovo('GEN_BOLCOD');
                                          RetornoForm.BoletosDupDUPLIC_BOL_ID.Value := BancodeDados.BoletosBOLETO_ID.Value;
                                          RetornoForm.BoletosDupDUPLIC_RET_ID.Value := BancodeDados.QryRetornoRETORNO_ID.Value;
                                          //RetornoForm.BoletosDupUSR_CAD.Value       := CodUsuario;
                                         // RetornoForm.BoletosDupLOJA_ID.Value       := 0;
                                          RetornoForm.BoletosDupEMISSAO.Value       := PrincipalForm.DataServidor;
                                          RetornoForm.BoletosDupDESCRICAO.Value     := Copy('(BOLETO DUPLICADO) ' + RetornoForm.BoletosDupDESCRICAO.Value, 1, 60);
                            ...
                            begin
                                  try
                                    if not(Atrazado = true) then
                                      PercBol := (qryB.FieldByName('VALOR_APAGAR').AsCurrency * 100) / vlTotal
                                    else
                                      PercBol :=
                                        ((qryB.FieldByName('VALOR_INTEGRAL').AsCurrency + qryB.FieldByName('ACRESCIMO').AsCurrency + qryB.FieldByName('ACRESCIMO_EXTRA')
                                  ...
                                  begin
                                    ...
                                    TipoContaID := qryB.FieldByName('TIPO_CONTAS_ID').AsInteger;
                                  end
                                  else
                                  begin
                                    ContaID := 0;
                                  ...
                                  vlProporcional := StrToCurr(FormatFloat('0.00', (vlrecebido * PercBol) / 100));
                                  qryB.Edit;

                                  if not(qryB.FieldByName('TIPO_CONTAS_ID').AsInteger > 0) then
                                    qryB.FieldByName('TIPO_CONTAS_ID').AsInteger := TipoContaID;

                                  //qryB.FieldByName('USUARIO').Value         := Usuario;
                                  qryB.FieldByName('SITUACAO_BOLETO').Value := 'RECEBIDO';
                                  qryB.FieldByName('OP_PAG_ID').Value       := 4;
                                  qryB.FieldByName('VALOR_PAGO').Value      := vlProporcional;

                                  qryB.FieldByName('MULTA').Value := StrToCurr(FormatFloat('0.00', (vlMulta * PercBol) / 100));
                                  qryB.FieldByName('JUROS').Value := StrToCurr(FormatFloat('0.00', (vlJuros * PercBol) / 100));

                                   qryB.FieldByName('RECEBIMENTO').Value := BancodeDados.RetornoDATA_RECEBIMENTO.Value;
                                  //qryB.FieldByName('RECEBIMENTO').Value := BancodeDados.RetornoDATA_CREDITO.Value;
                                  qryB.FieldByName('DT_CREDITO').Value := BancodeDados.RetornoDATA_CREDITO.Value;

                                  if (BancodeDados.RetornoDATA_RECEBIMENTO.Value <> BancodeDados.RetornoDATA_CREDITO.Value) then
                                  begin
                                    if Trim(qryB.FieldByName('OBS').AsString) = '' then
                                      qryB.FieldByName('OBS').AsString := 'Recebido em ' + FormatDateTime('dd/mm/yyyy', BancodeDados.RetornoDATA_RECEBIMENTO.Value) +
                                        ' - Creditado em ' + FormatDateTime('dd/mm/yyyy', BancodeDados.RetornoDATA_CREDITO.Value) + ' (' +
                                        BancodeDados.RetornoNOME_ARQ.Value + ')'
                                    else
                                      qryB.FieldByName('OBS').AsString := qryB.FieldByName('OBS').AsString + #13 + 'Recebido em ' +
                                        FormatDateTime('dd/mm/yyyy', BancodeDados.RetornoDATA_RECEBIMENTO.Value) + ' - Creditado em ' +
                                        FormatDateTime('dd/mm/yyyy', BancodeDados.RetornoDATA_CREDITO.Value) + ' (' + BancodeDados.RetornoNOME_ARQ.Value + ')';
                                  end
                                  else
                                  begin
                                    if Trim(qryB.FieldByName('OBS').AsString) = '' then
                                      ...
                                      qryB.FieldByName('OBS').AsString := qryB.FieldByName('OBS').AsString + #13 + 'Recebido em ' +
                                        FormatDateTime('dd/mm/yyyy', BancodeDados.RetornoDATA_RECEBIMENTO.Value) + ' (' + BancodeDados.RetornoNOME_ARQ.Value + ')';
                                  end;

                                  //qryB.FieldByName('HORA_RECEB').Value := PrincipalForm.HoraServidor;
                                  qryB.FieldByName('RETORNO_ID').Value := BancodeDados.RetornoRETORNO_ID.Value;

                                  vlTotalLvCaixa := vlTotalLvCaixa + qryB.FieldByName('VALOR_PAGO').AsCurrency;
                                  inc(NTitulosLvCaixa);

                              // Lancamento no livro caixa
                              if (RetornoForm.rgOpcoes.ItemIndex = 0) and (RetornoForm.chLancCX.Checked) then
                              begin

                                BancodeDados.Livro_Caixa.Close;
                                BancodeDados.Livro_Caixa.SQL.Text :=PrincipalForm.SQLrepositorio(2) + ' and l.boleto_id = ' + IntToStr(qryB.FieldByName('BOLETO_ID').AsInteger) + ' and l.retorno_id = ' + IntToStr(BancodeDados.RetornoRETORNO_ID.Value) +  ' and coalesce(cancelado, 0) = 0';

                                BancodeDados.Livro_Caixa.Open;


                                if BancodeDados.Livro_Caixa.IsEmpty then BancodeDados.Livro_Caixa.Append else BancodeDados.Livro_Caixa.Edit;

                                BancodeDados.Livro_CaixaDESCRICAO.Value         :=
                                  Copy('Ret. Par. ' + FormatFloat('000000', qryB.FieldByName('BOLETO_ID').AsInteger) + '(' + qryB.FieldByName('PARCELA')
                                  .AsString + ') - ' + FormatFloat('000000', qryB.FieldByName('CLI_ID').AsInteger) + '/' + qryB.FieldByName('CLIENTE')
                                  .AsString, 1, 60) + Copy(' - ' + qryB.FieldByName('DESCRICAO').AsString, 1, 20);

                                BancodeDados.Livro_CaixaDATA.Value              := BancodeDados.RetornoDATA_CREDITO.Value;
                                BancodeDados.Livro_CaixaDOCUMENTO.Value         := IntToStr(qryB.FieldByName('BOLETO_ID').Value);
                                BancodeDados.Livro_CaixaENTRADA.Value           := qryB.FieldByName('VALOR_PAGO').AsCurrency;
                                BancodeDados.Livro_CaixaBOLETO_ID.Value         := qryB.FieldByName('BOLETO_ID').Value;
                                BancodeDados.Livro_CaixaOP_PAG_ID.Value         := 4;
                                BancodeDados.Livro_CaixaDESCRICAO_PAGO.Value    :='Boleto';
                                BancodeDados.Livro_CaixaCONTA_ID.Value          := ContaID;
                                BancodeDados.Livro_CaixaTIPO_CONTAS_ID.Value    := TipoContaID;
                                BancodeDados.Livro_CaixaRETORNO_ID.Value        := BancodeDados.RetornoRETORNO_ID.Value;
                                BancodeDados.Livro_CaixaNUMERO_ARQUIVO.Value    := Retorno.NArquivo;
                                 BancodeDados.Livro_CaixaOBSERVACAO.AsString    := qryB.FieldByName('DESCRICAO').Value+': Parc.(' + qryB.FieldByName('PARCELA').AsString + ') Cliente: ' + FormatFloat('000000', qryB.FieldByName('CLI_ID').AsInteger) + ' / ' + Copy(qryB.FieldByName('CLIENTE').AsString, 1, 25);

                                qryB.FieldByName('CAIXA_ID').Value              := BancodeDados.Livro_CaixaCAIXA_ID.Value;

                                BancodeDados.Livro_Caixa.Post;
                              end;

                              if Trim(idsTitulos) = '' then
                                idsTitulos := qryB.FieldByName('BOLETO_ID').AsString
                              else
                                idsTitulos := idsTitulos + ', ' + qryB.FieldByName('BOLETO_ID').AsString;
                              ...
                              qryB.Post;
                                ...
                                begin
                                  Bancodedados.Livro_Caixa.Close;
                                  Bancodedados.Livro_Caixa.SQL.Text := PrincipalForm.SQLrepositorio(2) + ' and l.boleto_id = ' + IntToStr(qryB.FieldByName('BOLETO_ID').AsInteger) + ' and l.retorno_id = ' + IntToStr(BancodeDados.RetornoRETORNO_ID.Value) +
                                  ' and (l.descricao ' + PrincipalForm.csContainingLIKE( ('Taxa') ,0)+')';
                                  BancodeDados.Livro_Caixa.Open;
                                  ...
                                  Bancodedados.Livro_CaixaDESCRICAO.Value :=
                                    Trim(Copy('Taxa Boleto: ' + qryB.FieldByName('PARCELA').AsString + ' - ' + FormatFloat('000000', qryB.FieldByName('BOLETO_ID').AsInteger)
                                    + '/' + '/' + qryB.FieldByName('CLIENTE').AsString, 1, 60));

                                  Bancodedados.Livro_CaixaDATA.Value           := Bancodedados.RetornoDATA_CREDITO.Value;
                                  Bancodedados.Livro_CaixaDOCUMENTO.Value      := IntToStr(qryB.FieldByName('BOLETO_ID').Value);
                                  Bancodedados.Livro_CaixaSAIDA.Value          := StrToCurr(FormatFloat('0.00', ((vlTxRecebido * PercBol) / 100)));
                                  Bancodedados.Livro_CaixaBOLETO_ID.Value      := qryB.FieldByName('BOLETO_ID').AsInteger;
                                  Bancodedados.Livro_CaixaDESCRICAO_PAGO.Value := 'Boleto';
                                  BancodeDados.Livro_CaixaOP_PAG_ID.Value      := 4;
                                  Bancodedados.Livro_CaixaCONTA_ID.Value       := ContaID;
                                  BancodeDados.Livro_CaixaTIPO_CONTAS_ID.Value := TipoContaID;
                        ...
                        begin // vlBoletosCompara <> vlBoletosCompara2
                          BancodeDados.Retorno.Edit;
                          BancodeDados.RetornoBAIXOU.Value := 2; // Valor do boleto não bate com os valores dos títulos
                          BancodeDados.Retorno.Post;
                        end;
                      end
                      else
                      begin // Boletos.IsEmpty
                        BancodeDados.Retorno.Edit;
                        BancodeDados.RetornoBAIXOU.Value := 405; // Titulo a receber não encontrado
                        BancodeDados.Retorno.Post;
                      end;

                      BancodeDados.Clientes.Close;
                      BancodeDados.Clientes.SQL.Text := PrincipalForm.SQLrepositorio(1) +' and c.cli_id = ' + BancodeDados.EmissoesCLI_ID.AsString;
                      BancodeDados.Clientes.Open;
                      if BancodeDados.ClientesFAT_NOTA.Value = 1 then
                      begin
                        if not assigned(Boletos1Form) then Boletos1Form := TBoletos1Form.create(Application);
                    ...
                    begin
                      BancodeDados.Retorno.Edit;
                      BancodeDados.RetornoBAIXOU.Value := 404;  // não ha emissoes para esse titulo
                      BancodeDados.Retorno.Post;
                    end;
                    // end;//Baixou
                ...
                if (RetornoForm.rgOpcoes.ItemIndex = 1) and (RetornoForm.chLancCX.Checked) then
                begin
                  BancodeDados.Adiciona.SQL.Text := 'delete from livro_caixa where numero_arquivo=' + IntToStr(Retorno.NArquivo);
                  BancodeDados.Adiciona.ExecSQL;
                  ...
                  begin
                    BancodeDados.Livro_Caixa.Close;
                    BancodeDados.Livro_Caixa.SQL.Text :=  PrincipalForm.SQLrepositorio(2)  + ' and numero_arquivo =' + IntToStr(Retorno.NArquivo);
                    BancodeDados.Livro_Caixa.Open;

                    BancodeDados.Livro_Caixa.Append;
                    BancodeDados.Livro_CaixaDESCRICAO.Value       := Copy(IntToStr(NTitulosLvCaixa) + ' títulos. Ag.' + Agencia + ' CC.' + Conta + '. N.Ret ' +
                      IntToStr(Retorno.NArquivo) + ' - ' + FormatDateTime('dd/mm/yyyy', BancodeDados.RetornoDATA_ARQUIVO.Value), 1, 60);
                    BancodeDados.Livro_CaixaDATA.Value            := BancodeDados.RetornoDATA_CREDITO.Value;
...
//                ('2' + DateToStr(Dt_Arquivo));
                ...
                begin
                  if not DirectoryExists(aDiretorio + 'processado\') then CreateDir(aDiretorio + 'processado\');
                  if AlterandoNomeRet=1 then
                  MoveFile(PChar(Retorno.Arquivo), PChar(aDiretorio + 'processado\' + FormatFloat('000', BancoID) + '_' + FormatDateTime('yyyymmdd', Date) + '_' + aNomeArq)) else
                  if AlterandoNomeRet=2 then
                  MoveFile(PChar(Retorno.Arquivo), PChar(aDiretorio + 'processado\' + FormatFloat('000', BancoID) + '_' + FormatDateTime('yyyymmdd', Retorno.DataArquivo) + '_' + Format('%0.4d',[aNArquivo])));
                  MoveFile(PChar(Retorno.Arquivo), PChar(aDiretorio + 'processado\' + FormatFloat('000', BancoID) + '_' + FormatDateTime('yyyymmdd', Retorno.DataArquivo) + '_' + Format('%0.4d',[aNArquivo])));
                end;
                ...
                try
                  ...
                  if RetornoForm.chConfirma.Checked then
                  begin
                    Mensagem('Quitação bancária efetuada com sucesso.'#13#13+'Na próxima janela realize a impressão do(s) relatório(s) ou salve-os em PDF.', mtInformation, [mbOk], mrOk, 0);
                    ...
                    try
                      BancodeDados.Retorno.Close;
                      BancodeDados.Retorno.SQL.Text := 'select * from retorno where banco_id = ' + IntToStr(BancoID) + ' and numero_arquivo = ' +IntToStr(Retorno.NArquivo)+
                                                       ' and (tipo = ' + IntToStr(Retorno.TipoRet)+' )'; //eraldo  18/08/15
                      ...
                      //if BancodeDados.Bancos.Locate('BANCO_ID', BancodeDados.RetornoBANCO_ID.Value, []) then
                        if not BancodeDados.Bancos.Active then begin
                          BancodeDados.Bancos.close;
                          BancodeDados.Bancos.SQL.Text:='select * from bancos and banco_id = '+IntToStr(BancodeDados.RetornoBANCO_ID.Value);
                          BancodeDados.Bancos.Open;
                        end;
                      ...
                      begin
                        ...
                        begin
                          BancodeDados.Boletos.close;
                          BancodeDados.Boletos.SQL.Text := PrincipalForm.SQLrepositorio(0) + ' and b.boleto_id in (' + idsTitulos +') order by c.nome, b.parcela, b.valor_pago';
                          BancodeDados.Boletos.Open;
                          ...
                          begin
                            ...
                            CsBaixaBancoForm.lbPeriodo.Caption := 'Data do Arquivo: ' + FormatDateTime('dd/mm/yyyy', Dt_Arquivo) + ' - Cedente: ' +
                              BancodeDados.RetornoNOME_CEDENTE.Value;
                            CsBaixaBancoForm.LbArquivo.Caption := 'Banco: ' + Nome_Banco + ' - Retorno N.º ' + IntToStr(COD_NArquivo);
                            AtivandoEve := 1;
                          ...
                          BancodeDados.Cont_Prt_Boleto.SQL.Text := 'select * from cont_prt_boleto where prot_sit=0 and boleto_id in (' +(idsTitulos)+ ')';
                          BancodeDados.Cont_Prt_Boleto.Open;  {0-Protestado; 1-Retirado; 2- Cancelado}
                          idsTitProtest:='';

                            if not BancodeDados.Cont_Prt_Boleto.IsEmpty then begin
                               BancodeDados.Cont_Prt_Boleto.First;
                               while not BancodeDados.Cont_Prt_Boleto.eof do begin
                                 if Trim(idsTitProtest)<>'' then
                                 idsTitProtest:=idsTitProtest+','+IntToStr(BancodeDados.Cont_Prt_BoletoBOLETO_ID.Value) else idsTitProtest:=IntToStr(BancodeDados.Cont_Prt_BoletoBOLETO_ID.Value);
                                 BancodeDados.Cont_Prt_Boleto.next;
                               end; //while
                            end; //Cont_Prt_Boleto.IsEmpty

                          if Trim(idsTitProtest)<>'' then begin

                            BancodeDados.Boletos.Close;
                            BancodeDados.Boletos.SQL.Text := PrincipalForm.SQLrepositorio(0) + ' and b.boleto_id in (' + idsTitProtest + ') order by c.nome, b.parcela, b.valor_pago';
                            BancodeDados.Boletos.Open;
                            ...
                            begin
                              ...
                              CsBaixaBancoForm.LbArquivo.Caption  := 'Banco: ' + Nome_Banco + ' - Retorno N.º ' + IntToStr(COD_NArquivo);
                              AtivandoEve := 1;
                              if Mensagem('Foram baixados títulos protestados neste retorno.' + #13#13 + 'Deseja visualizar agora?', mtConfirmation, [mbYes, mbNo],
                                mrYes, 0) = idYes then
                                begin
                                CsBaixaBancoForm.DBGrid1.Columns[0].Visible:=true;
                          ...
                          BancodeDados.Boletos.SQL.Text :=
                                                          ...
                                                          'where b.boleto_id in (' + idsTitulos + ') and b.situacao_boleto = ' +QuotedStr('RECEBIDO') +
                          ...
                          if not BancodeDados.Boletos.IsEmpty then
                          begin
                            if Mensagem('Foram baixados títulos com valor a menos. ' + #13 + 'Deseja visualizar agora?', mtConfirmation, [mbYes, mbNo], mrYes, 0)
                              = idYes then
                            begin
                              vlInt := 0;
                              vlRec := 0;
                              ...
                              while not BancodeDados.Boletos.Eof do
                              begin
                                vlInt := vlInt + BancodeDados.BoletosVALOR_INTEGRAL.Value;
                                if (BancodeDados.BoletosSITUACAO_BOLETO.Value = 'RECEBIDO') then
                                  vlRec := vlRec + BancodeDados.BoletosVALOR_PAGO.Value;
                                if (BancodeDados.BoletosSITUACAO_BOLETO.Value = 'A RECEBER') then
                                  vlARec := vlARec + BancodeDados.BoletosvlAPagarCalc.Value;
                                BancodeDados.Boletos.Next;
                              end;

                              if not Assigned(relSituacaoTitulosForm) then
                                relSituacaoTitulosForm := TrelSituacaoTitulosForm.Create(Application);
                              try
                                relSituacaoTitulosForm.lbTitulo2.Caption := 'Títulos recebidos com valor a menos';
                                relSituacaoTitulosForm.lbDesc2.Caption := 'Banco: ' + Nome_Banco;
                                relSituacaoTitulosForm.lbPeriodo2.Caption := 'Retorno N.º ' + IntToStr(COD_NArquivo);

                                relSituacaoTitulosForm.RLGroup2.DataFields := 'TURMA_ID';

                                relSituacaoTitulosForm.lbTotalInt_.Caption := FormatFloat(',0.00', vlInt);
                                relSituacaoTitulosForm.lbTotalRec_.Caption := FormatFloat(',0.00', vlRec);
                                relSituacaoTitulosForm.lbTotalARec_.Caption := FormatFloat(',0.00', vlARec);
                                BancodeDados.PrepararRel(relSituacaoTitulosForm.RLReport2);
                              finally
                        ...
                        BaixaNEfetuadaForm.BaixaN.SQL.Text := 'select * from retorno where banco_id = ' + IntToStr(BancoID) + ' and numero_arquivo = ' +
                          IntToStr(COD_NArquivo) + ' and (tipo = ' + IntToStr(Retorno.TipoRet) + ') and (baixou<>1 or baixou is null)';

                        if VerifNossNum>0 then //eraldo inclui esta linda para filtrar os registros de retorno da mesma conta com unidades diferentes  08/01/16
                        BaixaNEfetuadaForm.BaixaN.SQL.Add(' and (UPPER(nosso_numero) '+PrincipalForm.csContainingLIKE(IntToStr(VerifNossNum) ,0)+')');

                        BaixaNEfetuadaForm.BaixaN.SQL.Add(' order by numero_arquivo');
      ...
      if Trim(nossoNumDup) <> '' then
      begin
        if Mensagem('Foram encontrados boletos (com mesmo nosso número) já quitados anteriormente, deseja visualizá-los?', mtConfirmation, [mbYes, mbNo], mrYes,
          0) = idYes then
        begin
           try
          if not Assigned(BaixaNEfetuadaForm) then
          ...
          try
            ...
            BaixaNEfetuadaForm.BaixaN.Close;
            BaixaNEfetuadaForm.BaixaN.SQL.Text := 'select * from retorno where banco_id = ' + IntToStr(BancoID) + ' and nosso_numero in (' + nossoNumDup + ') and coalesce(cod_deb, 0) = 0';

            if VerifNossNum>0 then //eraldo inclui esta linda para filtrar os registros de retorno da mesma conta com unidades diferentes  08/01/16
            BaixaNEfetuadaForm.BaixaN.SQL.Add(' and (UPPER(nosso_numero) '+PrincipalForm.csContainingLIKE(IntToStr(VerifNossNum) ,0)+')');

            BaixaNEfetuadaForm.BaixaN.SQL.Add(' order by nosso_numero, numero_arquivo');
            BaixaNEfetuadaForm.BaixaN.Open;
            if BaixaNEfetuadaForm.BaixaN.IsEmpty then
              Mensagem('Informações não disponíveis.', mtInformation, [mbOk], mrOk, 0)
            else
            begin
              try
                ...
                RelArquivoRetornoForm.lbTitulo1.Caption     := 'Relação de boletos pagos mais de uma vez';
                RelArquivoRetornoForm.lbPeriodo2.Caption    := 'Arquivo Ret.: ' + QuotedStr(BaixaNEfetuadaForm.BaixaNNOME_ARQ.Value) + ' -  N.º ' +
                  IntToStr(BaixaNEfetuadaForm.BaixaNNUMERO_ARQUIVO.Value);
                RelArquivoRetornoForm.lbSituacao2.Caption   := 'Data do Arquivo: ' + FormatDateTime('dd/mm/yyyy', BaixaNEfetuadaForm.BaixaNDATA_ARQUIVO.Value);
                RelArquivoRetornoForm.lbDescricao2.Caption  := 'Relação de boletos pagos mais de uma vez';
      ...
      begin
        if Mensagem('Foram encontrados títulos quitados anteriormente através de retornos/baixas diferentes.'#13+'Deseja visualizá-los?', mtConfirmation, [mbYes, mbNo],
          mrYes, 0) = idYes then
        begin
          try
            try
...
begin
  if not BancodeDados.Bancos.Locate('BANCO_NOME', RetornoForm.EditBanco.Text, [loCaseInsensitive]) then
    Mensagem('O banco não foi localizado.', mtInformation, [mbOk], mrOk, 0) else
  begin
      ...
      try
        ...
        if IndicaRetornoForm.ShowModal = mrOk then
          ...
          begin
           ...
           while not IndicaRetornoForm.QryRetorno.Eof do begin

            //if (listaDesfazerRet.IndexOf(IndicaRetornoForm.QryRetornoNUMERO_ARQUIVO.AsString) >= 0) then begin
             if indicaRetornoForm.QryRetornoATIVO.AsBoolean then begin


              BancodeDados.qryBanco.Close;
              BancodeDados.qryBanco.Params[0].Value:=BancodeDados.BancosBANCO_ID.Value;
              BancodeDados.qryBanco.Open;

              BancodeDados.Retorno.Close;
              BancodeDados.Retorno.SQL.Text := 'select * from retorno where numero_arquivo = ' + IntToStr(IndicaRetornoForm.QryRetornoNUMERO_ARQUIVO.Value)+' and CODIGO_BANCO = ' + IntToStr(IndicaRetornoForm.QryRetornoCODIGO_BANCO.Value);

              if not (BancodeDados.qryBancoBAIX_BOL_PARAM.value>0) then
              BancodeDados.Retorno.SQL.Add(' and banco_id = '+IntToStr(BancodeDados.BancosBANCO_ID.Value));
              ...
              BancodeDados.Retorno.SQL.Add(' and (UPPER(nosso_numero) '+PrincipalForm.csContainingLIKE(IntToStr(VerifNossNum) ,0)+')');
                ...
                begin
                  BancodeDados.Retorno.Last;
                  RetornoForm.Gauge1.MaxValue := BancodeDados.Retorno.RecNo;
                  RetornoForm.LbNomeOperacao.Caption := 'Revertendo Processamento de Retorno';
                  BancodeDados.Retorno.First;
                  while not BancodeDados.Retorno.Eof do
                  begin
                    RetornoForm.Gauge1.Progress := BancodeDados.Retorno.RecNo;
                    ...
                    if not (BancodeDados.qryBancoBAIX_BOL_PARAM.value>0) then begin
                      ...
                      BancodeDados.Emissoes.SQL.Add(' and nosso_numero=' + QuotedStr(BancodeDados.RetornoNOSSO_NUMERO.AsString)) else
                    ...
                    begin
                      BancodeDados.Boletos.Close;
                      BancodeDados.Boletos.SQL.Text := PrincipalForm.SQLrepositorio(0) + ' and b.emissao_id =' + IntToStr(BancodeDados.EmissoesEMISSAO_ID.Value)+' and op_pag_id = 4';
                      BancodeDados.Boletos.Open;                                                                                                   //nao estornar se ja recebido na escola,
                      if not BancodeDados.Boletos.IsEmpty then
                      begin
                        BancodeDados.Boletos.First;
                        while not BancodeDados.Boletos.Eof do
                  ...
                  BancodeDados.Adiciona.SQL.Text := 'delete from retorno where numero_arquivo=' + IntToStr(BancodeDados.RetornoNUMERO_ARQUIVO.Value)+' and CODIGO_BANCO = ' + IntToStr(BancodeDados.RetornoCODIGO_BANCO.Value);
                  //(BancodeDados.Adiciona.SQL.Text);
                  BancodeDados.Adiciona.ExecSQL;

                  BancodeDados.Adiciona.SQL.Text := 'delete from retorno_ocorrencia where numero_arquivo=' + IntToStr(BancodeDados.RetornoNUMERO_ARQUIVO.Value)+' and CODIGO_BANCO = ' + IntToStr(BancodeDados.RetornoCODIGO_BANCO.Value);
                  //(BancodeDados.Adiciona.SQL.Text);
                  BancodeDados.Adiciona.ExecSQL;

                  BancodeDados.Adiciona.SQL.Text := 'delete from livro_caixa where numero_arquivo=' + IntToStr(BancodeDados.RetornoNUMERO_ARQUIVO.Value);
                  //(BancodeDados.Adiciona.SQL.Text);
                  BancodeDados.Adiciona.ExecSQL;
                end; // empty
            ...
            // NumeroArq
          Mensagem('Nenhum registro válido foi selecionado!', mtInformation, [mbOk], mrOk, 0);
          end;
      finally
        Screen.Cursor := crDefault;
...
begin
  ...
  try
    ...
    begin
      ...
      if cbAnexarRel.Checked then //guardar no registro a ultima opção usada pelo usuario  da opção (Na Consulta de Recebimento Via Banco >> Anexar os relatórios de “Recebimento Via Banco” e “Arquivo Retorno”)
  ...
  begin
    Mensagem('O nome do banco não foi informado.', mtInformation, [mbOk], mrOk, 0);
    EditBanco.SetFocus;
  end
  else
  begin
...
//    suporte := false;
    ...
    begin
      ...
      begin
        Mensagem('Informe a conta de movimentação para o livro caixa.', mtInformation, [mbOk], mrOk, 0);
        EditConta.SetFocus;
      end else
        Processar;
    end else if RBDesfazer.Checked then
      Desfazer;
  end;
...
begin
RetornoTmpOBSERVACAO.Value :=(RetornoTmpOBS_1.AsString)+' '+(RetornoTmpOBS_2.AsString);
end;
...
begin
  ...
  begin
   ...
   if Mensagem('Deseja baixar sem lançar no livro caixa?',mtConfirmation,[mbyes,mbNo],mryes,0)=idNo then
    begin
      rgOpcoes.Enabled  := false;
...
begin
  ...
  begin
    ...
    visualizaRetornoForm.lblQtdeFalhas.caption := format('%0.4d',[RetornoForm.RetornoTmp.RecordCount]);
  end;
