...
begin
  ...
  Boletos1Form.lbAbertos.Caption := FormatFloat('000000', qtdAbertos) + ' registros a pagar.';
  Boletos1Form.lbPagos.Caption := FormatFloat('000000', qtdPagos) + ' registros pagos.';
  Boletos1Form.lbOutros.Caption := FormatFloat('000000', qtdOutros) + ' diversos.';
  Boletos1Form.lbRegistros.Caption := FormatFloat('000000', qtdTotal) + ' registros.';
...
begin
  ...
  BancodeDados.Boletos.SQL.Text := PrincipalForm.SQLrepositorio(0) + ' and b.cli_id =' + IntToStr(NovoCliente_ID);

  if (cbExibir.ItemIndex = 0) then
    BancodeDados.Boletos.SQL.Add(' and b.vencimento >= ' + QuotedStr(FormatDateTime('yyyy/mm/dd', Date - 366)));
...
procedure TBoletos1Form.DBGrid1DrawColumnCell(Sender: TObject; const Rect: TRect; DataCol: Integer; Column: TColumn; State: TGridDrawState);
begin
  ...
  begin
    ...
    if (BancodeDados.BoletosRECEBIMENTO.Value > StrToDate('01/01/1900')) and (BancodeDados.BoletosSITUACAO_BOLETO.Value = 'RECEBIDO') then
...
procedure TBoletos1Form.DBGrid1KeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  if (Key = 32) then
...
var
  ...
  begin
    ...
    try
      ...
      begin
        Result := IntToStr(strtoint(Copy(parcela,1,Pos('/',parcela)-1))+1)+'/'+Copy(parcela,Pos('/',parcela)+1,2);
      end;
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              try
                BancodeDados.Boletos.FieldByName(NomeCampo).Value := qry.FieldByName(NomeCampo).Value;
              except
              end;
            end;
          end;
          ...
          BancodeDados.BoletosVENCIMENTO.Value := PegaData(qry.FieldByName('VENCIMENTO').Value, 2);
          BancodeDados.BoletosPARCELA.Value := Retorna_Parcela(qry.FieldByName('PARCELA').AsString);
...
procedure TBoletos1Form.EditNomeKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
...
begin
  ...
  begin
    if Mensagem('Tem certeza que deseja atualizar o valor e o vencimento do(s) título(s) selecionado(s)?', mtConfirmation, [mbYes, mbNo], mrNo, 0) = idYes then
    begin

      BancodeDados.Boletos.Close;
      BancodeDados.Boletos.SQL.Text := PrincipalForm.SQLrepositorio(0) + ' and b.boleto_id in (' + IDsBoletos + ')';
      BancodeDados.Boletos.Open;
      ...
      begin
        ...
        begin
          ...
          BancodeDados.BoletosVENCIMENTO.Value:= BancodeDados.BoletosVENCIMENTO.Value+1;
          BancodeDados.Boletos.Post;
          BancodeDados.Boletos.Next;
        end;
      end;
        ...
        begin
          if (BancodeDados.BoletosSITUACAO_BOLETO.Value = 'A RECEBER') then
            ...
            begin
              ...
              BancodeDados.BoletosOBS.AsString          := 'O título foi cancelado pelo usuário: ' + supervisorLogin + ' na seguinte data - hora: ' + DateToStr(Date) +
                ' - ' + TimeToStr(Time);
  ...
  begin
    ...
    begin
      ...
      try
        ...
        while not BancodeDados.Boletos.Eof do
        begin
          if ((BancodeDados.BoletosSITUACAO_BOLETO.Value = 'RECEBIDO') Or (BancodeDados.BoletosSITUACAO_BOLETO.Value = 'CANCELADO')) then
            if (listaBoletos.IndexOf(BancodeDados.BoletosBOLETO_ID.AsString) >= 0) then
            begin
              if not BancodeDados.FDConnection1.InTransaction then
                BancodeDados.FDConnection1.StartTransaction;
              ...
              if not(BancodeDados.Boletos.State in [dsedit, dsinsert]) then
                BancodeDados.Boletos.Edit;
              BancodeDados.BoletosSITUACAO_BOLETO.Value := 'A RECEBER';
              BancodeDados.BoletosOBS.AsString := 'O título foi estornado pelo usuário: ' + supervisorLogin + ' na seguinte data - hora: ' + DateToStr(Date) +
                ' - ' + TimeToStr(Time);
...
if not BancodeDados.Boletos.IsEmpty then begin
  ...
  while not BancodeDados.Boletos.Eof do
  begin
  if BancodeDados.BoletosATIVO.AsBoolean then begin
    if BancodeDados.BoletosEMISSAO_ID.Value>0 then
    if Trim(IDsEmissao) = '' then IDsEmissao := IntToStr(BancodeDados.BoletosEMISSAO_ID.Value) else IDsEmissao := IDsEmissao +','+IntToStr(BancodeDados.BoletosEMISSAO_ID.Value);
  end;
    BancodeDados.Boletos.Next;
  end;
  ...
  begin
    ...
    BancodeDados.V_Boletos.SQL.Text := 'select * from v_boletos where emissao_id in (' + (IDsEmissao)+') order by VENCIMENTO';
  ...
  begin
    IDsBoletos := BancodeDados.PegarAtivo(listaBoletos);
    BancodeDados.V_Boletos.Close;
    BancodeDados.V_Boletos.SQL.Text := 'select * from v_boletos where (id_boletos ' + PrincipalForm.csContainingLIKE(IDsBoletos,1)+') order by emissao_id desc';
  ...
  begin
    ...
    begin
      ...
      try
        ...
        CsBoletosEmitidosForm.Caption := 'Consulta de Boletos Emitidos - Painel de consulta desabilitado.';
...
var
  ...
  begin
    ...
    begin
      if (BancodeDados.BoletosSITUACAO_BOLETO.Value = 'A RECEBER') then
      begin
        if Mensagem('Atenção! Este título ainda está aberto.' + #13#13 + 'Tem certeza que deseja prosseguir?', mtWarning, [mbYes, mbNo], mrNo, 0) = idNo then
          Result := False;
        break;
      end;
      BancodeDados.Boletos.Next;
...
begin
  ...
  begin
    ...
    begin
      if cont = 1 then
        if Mensagem('Deseja faturar este(s) título(s) agora?', mtConfirmation, [mbYes, mbNo], mrNo, 0) = idYes then
          PodeIr := true
        else
          PodeIr := false;
      ...
      begin
        try
          ...
          BancodeDados.Boletos.Close;
          BancodeDados.Boletos.SQL.Text := PrincipalForm.SQLrepositorio(0) + ' and b.boleto_id in (' + IDsBoletos + ') and coalesce(b.nfs_id, 0) = 0 order by b.vencimento';
          BancodeDados.Boletos.Open;
          ...
          begin
            ...
            begin
              ...
              BancodeDados.Nfs.SQL.Add(' where n.sit = 0 and c.cnpj = ' + QuotedStr(BancodeDados.ClientesCNPJ.Value) + ' order by n.dt_rps');
//              (BancodeDados.Nfs.SQL.Text);
              ...
              begin
                ...
                if Mensagem('Existe uma nota fiscal em aberto para este cliente no dia ' + FormatDateTime('dd/mm/yyyy', BancodeDados.NfsDT_RPS.Value) + '.' + #13
                  + 'Deseja utilizar esta mesma nota?', mtConfirmation, [mbYes, mbNo], mrYes, 0) = idNo then
                  insereNovo := true;
              end;
              ...
              if Trim(BancodeDados.NfsIDS_BOLETOS.Value) = '' then
                BancodeDados.NfsIDS_BOLETOS.Value := QuotedStr(BancodeDados.BoletosBOLETO_ID.AsString)
              else
                BancodeDados.NfsIDS_BOLETOS.Value := QuotedStr(BancodeDados.BoletosBOLETO_ID.AsString) + ', ' + Trim(BancodeDados.NfsIDS_BOLETOS.Value);
              ...
              begin
                ...
                BancodeDados.NfsServDESCRICAO.Value := BancodeDados.BoletosDESCRICAO.Value + ' - ' + FormatDateTime('mmm/yyyy', BancodeDados.BoletosVENCIMENTO.Value);
                BancodeDados.NfsServQUANT.Value     := 1;
                BancodeDados.NfsServVL_UNIT.Value   := BancodeDados.BoletosVALOR_APAGAR.Value;

                if (BancodeDados.BoletosVALOR_PAGO.Value < BancodeDados.BoletosVALOR_APAGAR.Value) and (BancodeDados.BoletosVALOR_PAGO.Value > 0) then
                  BancodeDados.NfsServVL_UNIT.Value := BancodeDados.BoletosVALOR_PAGO.Value;

                BancodeDados.NfsServ.Post;

                BancodeDados.Adiciona.SQL.Text := 'update boletos set nfs_id = ' + IntToStr(BancodeDados.NfsNFS_ID.Value) + ' where boleto_id = ' + IntToStr(BancodeDados.BoletosBOLETO_ID.Value);
              ...
              try
                ...
                BancodeDados.Nfs.SQL.Text := 'select n.*, c.nome, c.cnpj, c.logradouro, c.bairro, c.cidade, c.estado, c.email_contato1, c.email_contato2, m.nome as cidade_nome, m.uf as uf_nome';
                BancodeDados.Nfs.SQL.Add(' from nfs n left join clientes c on (c.cli_id = n.cli_id) left join municipios m on (m.codmun=c.cidade)');
                BancodeDados.Nfs.SQL.Add(' where  n.NFS_ID = '+IntToStr(idNota));
                BancodeDados.Nfs.Open;
            ...
            Mensagem('Título não disponível ou já faturado.', mtInformation, [mbok], mrOk, 0);
          BancodeDados.Boletos.Close;
      ...
      Mensagem('Cliente não configurado para faturar nota!', mtInformation, [mbOk], mrOk,0);
  end; // IDsBoletos = ''
end;
...
if not BancodeDados.Boletos.IsEmpty then begin
  ...
  if not (Trim(IDsEmissao) ='') or not (Trim(IDsBoletos) = '') then begin
...
begin
  if MensagemChk('Confirma impressão de boleto(s)?', mtConfirmation, [mbYes, mbNo], mrYes, 0, false, 'Papel cortado (1 boleto na vertical)?', papelCortado) = idYes
  then
  begin
...
begin
  if MensagemChk('Confirma impressão de boleto(s)?', mtConfirmation, [mbYes, mbNo], mrYes, 0, true, 'Papel cortado (1 boleto na vertical)?', papelCortado) = idYes
  then
  begin
...
begin
  ...
  Try
    ...
    begin
      Close;
      SQL.Clear;
      SQL.Text:= PrincipalForm.SQLrepositorio(0) + ' and b.cli_id=' + IntToStr(BancodeDados.ClientesCLI_ID.Value);
...
procedure TBoletos1Form.EditConsultaKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
