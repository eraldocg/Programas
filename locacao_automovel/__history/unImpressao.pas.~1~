...
x: integer;
begin
//letras:='abcdefghijlmnopqrstuvxzykw·ÈÌÛ˙„ıABCDEFGHIJLMNOPQRSTUVXZ¡…Õ”⁄√’ YWKÁ«0123456789 ';
if (comEspaco = true) then
letras:='0123456789 ' else
letras:='0123456789';

 for x:=1 to Length(em) do
     for i:=1 to Length(letras) do
        if em[x]=letras[i] then temp:=temp+em[x];
...
function AjustaAcento(Nome: String):String;
const
ComAcentuacao = '«È‚‡ÁÍ…Ù·Ì˙™∫';
SemAcentuacao : array[1..13] of char=(#128, #130, #131, #133, #135, #136, #144, #147, #160, #161, #163, #166, #167);
var
i : Integer;
begin

for i:=1 to Length(ComAcentuacao) do begin
...
function SemAcento(Nome: String):String;
const
ComAcentuacao = '‡·‚„‰ËÈÍÎÏÌÓÔÚÛÙıˆ˘˙˚¸Á¿¡¬√ƒ»… ÀÃÕŒœ“”‘’÷Ÿ⁄€‹«~^¥`®Ä™∫';
...
if Length(Texto)<Digitos then begin
...
function TSedImpDireta.IniciaImpressao(Metodo : TSedTipoImp; Porta : String = '') : Boolean;
var
verInfo : TOSVersionInfo;
begin
Result:=False;
if not fImprimindo then begin
  if UpperCase(Porta)='FISCAL' then Metodo:=tiFiscal;
  
  fTipoImpressao:=Metodo;
  if (fTipoImpressao = tiCanvas) then begin
    if Self.ExecDialogoImp then begin
      fLinhaAtual:=0;
      fContador:=0;
      fWinNT:=False;
      ...
      if GetVersionEx(verInfo) then begin
       if verInfo.dwPlatformId=VER_PLATFORM_WIN32_NT then fWinNT:=True;
      end;
  ...
  if (fTipoImpressao = tiMatricial) then begin
    fPorta:=Porta;

    if Trim(fPorta)='' then Raise Exception.Create('Informe a porta de impress„o!') else begin
      try
        AssignFile(fTextFile,fPorta);
        Rewrite(fTextFile);
        fImprimindo:=True;
        if Pos(':\',fPorta)>0 then fAbreArq:=True else fAbreArq:=False;
        ...
        if (fImprimindo = true) then begin
          CloseFile(fTextFile);
          fImprimindo:=False;
        end;
        //I/O error 103
  ...
  Raise Exception.Create('MÈtodo de Impress„o n„o definido.');
end;
end;
...
end else Result:=True;//Tipo de Impressao
end;

procedure TSedImpDireta.Imprime(vlTexto: String);
begin
if fImprimindo then begin
  if (fTamLinha > 0) then begin
    vlTexto:=Copy(vlTexto, 1, fTamLinha - Length(fEspacoInicial));
  end;

  if (fMaiusculo = true) then vlTexto:=AnsiUpperCase(vlTexto);

  if (fTipoImpressao = tiCanvas) then begin
    if fWinNT then begin
      Printer.Canvas.TextOut(0, fLinhaAtual, fEspacoInicial + SemAcento(vlTexto));
      Printer.NewPage;
    end else begin
      Inc(fContador);
      fLinhaAtual := fLinhaAtual + fAltura;
      if fContador >= 62 then begin
        Printer.EndDoc;
        fLinhaAtual:=0;
        fContador:=0;
        Printer.BeginDoc;
  ...
  if (fTipoImpressao = tiMatricial) then begin
    try
      ...
      Writeln(fTextFile, fEspacoInicial + SemAcento(vlTexto)) else
      Writeln(fTextFile, '');
...
if fImprimindo then
  if (fTipoImpressao = tiMatricial) then begin
    if Pos(':\',fPorta)>0 then
      ShellExecute(Application.Handle,'open',PChar(fPorta),nil,nil,SW_SHOWMAXIMIZED);
  end;
except end;
end;
...
if fImprimindo then begin
  if (fTipoImpressao = tiMatricial) then begin
    if not (Pos(':\',fPorta)>0) then begin
...
begin
if fImprimindo then
if (fMicroSalto = true) then begin
  if (fTipoImpressao = tiMatricial) then begin
    if not (Pos(':\',fPorta)>0) then begin
...
if fImprimindo then begin
  if (fTipoImpressao = tiMatricial) then begin
    if not (Pos(':\',fPorta)>0) then Write(fTextFile,#27+#64);
  end;
end;
end;
...
if (Value > 0) then begin
  if (Value > 255) then Raise Exception.Create('O tamanho m·ximo do espaÁo inicial È 255.') else begin
    for i:=1 to Value do fEspacoInicial:=fEspacoInicial + ' ';
  end;
end;
end;
...
begin
  while (fLinhaAtual < Value) do begin
    Imprime(' ');
  end;
end;
