  ...
  if BancodeDados.Pagar.State in [dsInsert, dsEdit] then
    try
      ...
      try
        ...
        begin
          BancodeDados.PagarFORNE_ID.Value := BancodeDados.FornecedoresFORNE_ID.Value;
          if (BancodeDados.FornecedoresBANCO_ID.Value > 0) then
            BancodeDados.PagarBANCO_ID.Value := BancodeDados.FornecedoresBANCO_ID.Value
          else
...
begin
  if ((BancodeDados.PagarCONTA_ID.Value = 0) or (BancodeDados.PagarCONTA_ID.IsNull)) then
    if Mensagem('Tem certeza que deseja sair?', mtConfirmation, [mbyes, mbNo], mryes, 0) = idNo then
      CanClose := False;

  if (not(BancodeDados.PagarCONTA_ID.Value = 0) or not(BancodeDados.PagarCONTA_ID.IsNull)) then
    if ((BancodeDados.PagarCONTA_ID.IsNull) or (BancodeDados.PagarCONTA_ID.Value = 0)) then
    begin
      CanClose := False;
      Mensagem('Importante:'#13 + 'Informar a conta!', mtInformation, [mbok], mrok, 0);
    end
    else
...
procedure TCadPagarForm.DBNavigator1Click(Sender: TObject; Button: TNavigateBtn);
begin
