...
begin
  PrincipalForm.CalcularValorBaixaIndividual(Baixa2Form.Dt_Pagar.DateTime, ApenasSomar);
...
begin
  try
    ...
    if not(Dt_Pagar.DateTime > 0) then
      ...
      Mensagem('Informe a conta de movimentação para o livro caixa.', mtWarning, [mbok], mrok, 0)
    else if (EditVlPago.Value <= 0) then
      Mensagem('O valor pago não pode ser menor ou igual a zero.', mtWarning, [mbok], mrok, 0)
    else
    begin
      ...
      if Mensagem('Confirmar o Pagamento?', mtConfirmation, [mbyes, mbno], mrno, 0) = idyes then
      begin
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                Mensagem('O valor proporcional da diferença está maior do que o valor a pagar da parcela ' + BancodeDados.RecBolPARCELA.AsString, mtWarning,
                  [mbok], mrok, 0);
                break;
              end;
            end;
        ...
        begin
              ...
              begin
                ...
                begin
                  ...
                  begin
                    ...
                    BancodeDados.RecBolVALOR_PAGO.Value := BancodeDados.RecBolVALOR_PAGO.Value - (vlDifParcelado + vlDifAjuste);
                    BancodeDados.RecBolOBS.AsString := FormatDateTime('dd/mm/yyyy', date) + ': Diferença no pagamento de ' +
                      FormatFloat('R$ ,0.00', (vlDifParcelado + vlDifAjuste));
                  ...
                  begin
                    ...
                    BancodeDados.RecBolVALOR_PAGO.Value := BancodeDados.RecBolVALOR_PAGO.Value - vlDifParcelado;
                    BancodeDados.RecBolOBS.AsString := FormatDateTime('dd/mm/yyyy', date) + ': Diferença no pagamento de ' +
                      FormatFloat('R$ ,0.00', vlDifParcelado);
          ...
          begin
            ...
            begin
              ...
              BancodeDados.RecBol.Edit;
              BancodeDados.RecBolOBS.AsString := FormatDateTime('dd/mm/yyyy', date) + ': Recebimento a maior de ' + FormatFloat('R$ ,0.00', (vlDif * -1));
            ...
            begin
              ...
              begin
                ...
                begin
                  BancodeDados.RecBol.Edit;
                  if (StrToCurr(FormatFloat('0.00', BancodeDados.RecBolVALOR_PAGO.Value))) >
                    (StrToCurr(FormatFloat('0.00', BancodeDados.RecBolVALOR_APAGAR.Value))) then
                    BancodeDados.RecBolOBS.AsString := FormatDateTime('dd/mm/yyyy', date) + ': Recebimento a maior de ' +
                      FormatFloat('R$ ,0.00', (BancodeDados.RecBolVALOR_PAGO.Value - BancodeDados.RecBolVALOR_APAGAR.Value))
                  else
                    BancodeDados.RecBolOBS.AsString := FormatDateTime('dd/mm/yyyy', date) + ': Diferença no pagamento de ' +
                      FormatFloat('R$ ,0.00', (BancodeDados.RecBolVALOR_APAGAR.Value - BancodeDados.RecBolVALOR_PAGO.Value));
                  // BancodeDados.RecBolVALOR_PAGO.Value:=BancodeDados.RecBolVALOR_PAGO.Value + (vlDif *-1);
                  BancodeDados.RecBol.Post;
                  // break;
                end;
          ...
          begin
            ...
            if BancodeDados.Boletos.Locate('BOLETO_ID', BancodeDados.RecBolBOLETO_ID.Value, []) then
            begin
              ...
              begin
                ...
                BancodeDados.BoletosVALOR_PAGO.Value := BancodeDados.BoletosVALOR_INTEGRAL.Value;
              ...
              else if (BancodeDados.BoletosSITUACAO_BOLETO.Value = 'A RECEBER') then
              begin

                BancodeDados.Boletos.Edit;
                BancodeDados.BoletosMULTA.Value := BancodeDados.RecBolMULTA.Value;
                BancodeDados.BoletosJUROS.Value := BancodeDados.RecBolJUROS.Value;

                // BancodeDados.BoletosVALOR_PAGO.Value:=BancodeDados.RecBolVALOR_APAGAR.Value;
                BancodeDados.BoletosVALOR_PAGO.Value := BancodeDados.RecBolVALOR_PAGO.Value;
                {
                ...
                begin
                  if Trim(BancodeDados.BoletosOBS.AsString) = '' then
                    BancodeDados.BoletosOBS.AsString := BancodeDados.RecBolOBS.AsString
                  else
                    BancodeDados.BoletosOBS.AsString := BancodeDados.RecBolOBS.AsString + #13 + #13 + BancodeDados.BoletosOBS.AsString;
                end;
                ...
                BancodeDados.BoletosDT_CREDITO.Value := Dt_Pagar.DateTime;   //eraldo 22/03
                BancodeDados.BoletosDESCRICAO_PAGO.Value := CBPagoem.Text;

                if chLancCX.Checked then
                begin
                  BancodeDados.Livro_Caixa.Close;
                  BancodeDados.Livro_Caixa.SQL.Text:= PrincipalForm.SQLrepositorio(2) + ' and l.boleto_id=' + IntToStr(BancodeDados.BoletosBOLETO_ID.Value);
                  ...
                  BancodeDados.Livro_CaixaDESCRICAO.Value :=
                    Copy('Par. ' + BancodeDados.BoletosPARCELA.Value + ' - ' + FormatFloat('000000', BancodeDados.BoletosCLI_ID.Value) + '/' +
                  ...
                  BancodeDados.Livro_CaixaDOCUMENTO.Value := IntToStr(BancodeDados.BoletosBOLETO_ID.Value);
                  BancodeDados.Livro_CaixaENTRADA.Value := BancodeDados.BoletosVALOR_PAGO.Value;
                  BancodeDados.Livro_CaixaBOLETO_ID.Value := BancodeDados.BoletosBOLETO_ID.Value;
                  BancodeDados.Livro_CaixaDESCRICAO_PAGO.Value := BancodeDados.BoletosDESCRICAO_PAGO.Value;
                  ...
                  BancodeDados.BoletosCAIXA_ID.Value := BancodeDados.Livro_CaixaCAIXA_ID.Value;
                end;
          ...
          { vlDif:=vlTotalAPagar - vlTotalPagoTmp;
            ...
            if (vlDif<0) and (vlDif>-0.03) then vlDif:=0; }
          if (vlDif > 0) then
            MsgTmp := 'Deseja gerar um título com o valor restante de  ' + FormatFloat('R$ ,0.00', vlDif) + '?'
          else if (vlDif < 0) then
            MsgTmp := 'Deseja gerar um crédito com o valor do troco de  ' + FormatFloat('R$ ,0.00', vlDif * -1) + '?';

          if (vlDif <> 0) then
          begin
            try
              if Mensagem(MsgTmp, mtConfirmation, [mbyes, mbno], mryes, 0) = idyes then
              begin
                ...
                if vlDif > 0 then
                begin
                  BancodeDados.BoletosDESCRICAO.Value := 'Saldo restante parc. ' + Parcela + DescricaoTmp;
                  ...
                  MsgTmp := 'Foi gerado um novo título com o saldo restante de R$ ' + FormatFloat(',0.00', vlDif);
                end
                else
                begin
                  BancodeDados.BoletosDESCRICAO.Value := 'Créd. ref. ao restante parc. ' + Parcela + DescricaoTmp;
                  BancodeDados.BoletosPARCELA.Value := Parcela;
                  ...
                  MsgTmp := 'Foi gerado um crédito com o saldo restante de R$ ' + FormatFloat(',0.00', vlDif * -1);
                end;
...
begin
  try
    try
      ...
      RecBolForm.lbDtReceb.Caption := 'Data de recebimento: ' + FormatDateTime('dd/mm/yyyy', Dt_Pagar.DateTime);
      RecBolForm.lbVlTotal.Caption := FormatFloat('R$ ,0.00', vlTotalAPagar + vlTotalCredito);
      RecBolForm.lbTotalAPagar.Caption := FormatFloat('R$ ,0.00', vlTotalAPagar);
      RecBolForm.lbTotalPago.Caption := FormatFloat('R$ ,0.00', vlTotalPago);
      RecBolForm.lbTotalCreditos.Caption := FormatFloat('R$ ,0.00', vlTotalCredito);
