...
begin
  ...
  try
    ...
    if Length(EditSenha.Text) > 0 then
      SenhaBCTemp := BancodeDados.Criptografar(EditSenha.Text)
    else
      SenhaBCTemp := EmptyStr;
...
begin
  ...
  try
    ...
    begin
      ...
      EditBancodeDados.Text := registro.LerTexto('banco',
        'D:\sedsoftdb\clientes.fdb');
      EditPorta.Text := registro.LerTexto('porta', '3050');
      EditUsuario.Text := registro.LerTexto('usuario', 'sysdba');
      EditSenha.Text := BancodeDados.Criptografar
        (registro.LerTexto('pswd', ''));
