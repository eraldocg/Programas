...
begin
  try
    ...
    RelComparativoForm.lbPeriodo.Caption := 'Período de ' +
      FormatDateTime('dd/mm/yyyy', DtIni.DateTime) + ' a ' +
    ...
    begin
      ...
      if Trim(txtContas) = '' then
        txtContas := RelComparativoForm.ContasCaixaCONTA_ID.AsString + ' / ' +
          RelComparativoForm.ContasCaixaCONTA.Value
      else
        txtContas := txtContas + #13 + RelComparativoForm.ContasCaixaCONTA_ID.
          AsString + ' / ' + RelComparativoForm.ContasCaixaCONTA.Value;

      if Trim(txtContas2) = '' then
        txtContas2 := FormatFloat(',0.00',
          RelComparativoForm.ContasCaixaSALDO.Value)
      else
        txtContas2 := txtContas2 + #13 + FormatFloat(',0.00',
          RelComparativoForm.ContasCaixaSALDO.Value);
