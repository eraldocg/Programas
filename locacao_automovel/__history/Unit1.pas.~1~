...
var MyProcess: TNfseConsulta; //TNfseConsulta;

begin
if (mythreadRunning1 = False) then
  begin
    mythreadRunning1 := True;
    MyProcess := TNfseConsulta.Create(True); { Create suspended-second process does not run yet. }
...
var YouProcess: TThread; //TNfseConsulta;

begin
  if (youthreadRunning = FALSE) then
  begin
    youthreadRunning:= TRUE;
    YouProcess := TThread.Create(True); { create suspended – secondprocess does not run yet }
