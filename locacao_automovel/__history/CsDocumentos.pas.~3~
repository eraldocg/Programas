...
begin
  ...
  begin
    ...
    if Mensagem('Deseja duplicar o registro atual?', mtConfirmation,
      [mbyes, mbno], mrok, 0) = idyes then
      try
        ...
        BancodeDados.QrySql.SQL.Text :=
          ...
          IntToStr(BancodeDados.DocumentosDOC_ID.Value);
        ...
        begin
          ...
          begin
            ...
            if nmCampo <> 'DOC_ID' then
              try
                BancodeDados.Documentos.FieldByName(nmCampo).Value :=
                  BancodeDados.QrySql.FieldByName(nmCampo).Value;
              except
              end;

          end;
          BancodeDados.DocumentosDOC_NOME.Value := 'Cópia ' +
            BancodeDados.QrySql.FieldByName('DOC_NOME').Value;
...
begin
  ...
  begin
    ...
    try
      ...
      BancodeDados.QrySql.Close;
      BancodeDados.QrySql.SQL.Text :=
        'select distinct doc_id  from veiculos where doc_id = ' +
        IntToStr(BancodeDados.DocumentosDOC_ID.Value);
      BancodeDados.QrySql.Open;
      ...
      begin
        ...
        Mensagem('Registro com vínculo no cadastro de veículos!'#13 +
...
procedure TCsDocumentosForm.DBGrid1DrawColumnCell(Sender: TObject;
