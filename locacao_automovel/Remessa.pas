      ...
      begin
        ...
        begin
          ...
          begin
            ...
            RemessaForm.Boletos.SQL.Text := 'select * from v_boletos_remessa where banco_id = ' + IntToStr(BancodeDados.BancosBANCO_ID.Value) + ' and situacao_boleto = ' + QuotedStr('A RECEBER') +
            ...
            if (RemessaForm.cbData.ItemIndex > 0) then
              RemessaForm.Boletos.SQL.Add(' and (' + campo + ' between ' + QuotedStr(FormatDateTime('yyyy/mm/dd',RemessaForm.dtIni.DateTime)) + ' and ' + QuotedStr(FormatDateTime('yyyy/mm/dd',RemessaForm.dtFim.DateTime))+ ')');
            RemessaForm.Boletos.SQL.Add(' order by vencimento, cli_nome');

            if suporte then
            Mensagem(RemessaForm.Boletos.SQL.Text, mtInformation, [mbok], mrok, 0);
...
procedure TRemessaForm.VisualizarListadeTtulosparaRemessa1Click(
  Sender: TObject);
begin
if not Boletos.IsEmpty then begin
  if not Assigned(Visualizar_Lista_RemessaForm) then Visualizar_Lista_RemessaForm:=tVisualizar_Lista_RemessaForm.Create(Application);
  try
...
begin
  ...
  begin
    ...
    begin
      ...
      if (CodBanco = 1) then begin //eraldo incluiu o teste ao lado e estas novas lindas abaixo
...
begin
  ...
  begin
    ...
    begin
      ...
      CampoReg[43] := EscreveCampo(02, 'N', IntToStr(BancodeDados.BancosDIAS_BAIXAR_BANCO.Value))else
      CampoReg[43] := EscreveCampo(02, 'N', '00'); //posição 226 a 227 | número de dias para baixar baixar/devolucao
    ...
    begin
      ...
      if (CodBanco in [1]) then begin
        ...
        CampoReg[28] := EscreveCampo(08, 'D', DateToStr(BoletosVENCIMENTO.Value + 1)) else CampoReg[28] := EscreveCampo(08, 'N', '0'); //ATENÇÃO, caso a informação seja inválida ou não informada, o sistema assumirá data igual à Data de Vencimento + 1
      end else
      ...
      CampoReg[39] := EscreveCampo(03, 'N', IntToStr(BoletosDIAS_BAIXAR_BANCO.Value))else
      CampoReg[39] := EscreveCampo(03, 'N', '29'); //posição 226 a 227 | número de dias para baixar baixar/devolucao
...
begin
  ...
  begin
    ...
    begin
      ...
      qryPesq.Close;
      qryPesq.SQL.Text := 'select nome, cli_id, cnpj, logradouro, bairro, (select nome from municipios where codmun=coalesce(clientes.cidade,0)) as cidade, (select uf from municipios where codmun=coalesce(clientes.cidade,0))  as uf, cep, numero '+ ' from clientes where cli_id = ' + IntToStr(BoletosCLI_ID.Value);
      qryPesq.Open;

      if trim(BoletosCLI_CNPJ_CPF.Value)<>'' then begin
    ...
    begin
      ...
      CampoReg[11] := EscreveCampo(40, 'X', qryPesq.FieldByName('LOGRADOURO').AsString + ' ' + qryPesq.FieldByName('NUMERO').AsString);
    ...
    begin
      ...
      begin
       ...
       // end;
        ...
        CampoReg[13] := EscreveCampo(15, 'N', ''); //Valor / Percentual a ser Concedido
...
begin
  ...
  begin
    ...
    begin
      CampoReg[01] := EscreveCampo(01, 'N', '0'); //001 a 001 > Preencher com número "0".
      CampoReg[02] := EscreveCampo(01, 'N', '1'); //002 a 002 > Preencher com número "1".
      CampoReg[03] := EscreveCampo(07, 'X', 'REMESSA'); //003 a 009 > Preencher com o literal "REMESSA".
      CampoReg[04] := EscreveCampo(02, 'N', '01'); //010 a 011 > Preencher com número "01".
      CampoReg[05] := EscreveCampo(15, 'X', 'COBRANCA');//012 a 026 > Preencher com o literal "COBRANCA".
      CampoReg[06] := EscreveCampo(04, 'N', BancodeDados.BancosAGENCIA.Value + BancodeDados.BancosDIG_AGENCIA.Value); //027 a 030 > Informar o cód. da Agência onde o cliente opera
      CampoReg[07] := EscreveCampo(02, 'N', '00'); //31 a 32 > Preencher espaços
      CampoReg[08] := EscreveCampo(08, 'N', RetornaNumeros(BancodeDados.BancosCONTA_CORRENTE.AsString) +
                                            RetornaNumeros(BancodeDados.BancosDIG_CONTA_CORRENTE.AsString)); //033 a 040> Informar a cta. do cliente cadastrada em cobrança como cedente.
      CampoReg[09] := EscreveCampo(06, 'X', ''); //041 a 046 >> Brancos.
      CampoReg[10] := EscreveCampo(30, 'X', BancodeDados.ConfigFANTASIA.Value); //47 a 76 >> PPreencher com o nome do cliente.
      CampoReg[11] := EscreveCampo(03, 'N', IntToStr(CodBanco));//077 a 079 > Preencher com o número "004".
      CampoReg[12] := EscreveCampo(15, 'X', 'B.DO NORDESTE'); //080 a 094  > Preencher com o literal "B.DO NORDESTE".
      CampoReg[13] := EscreveCampo(06, 'X', RetornaNumeros(FormatDateTime('dd/mm/yy',Date)) ); //95 a 100 > Informar a data da gravação do arquivo (formato "ddmmaa").
      CampoReg[14] := EscreveCampo(03, 'N', '000'); //101 a 103 >> código do usuário no Sistema de Cobrança do BANCO DO NORDESTE. Este código será fornecido pelo Banco.
      CampoReg[15] := EscreveCampo(291,'X', '');//104 a 394 > Preencher espaços
      CampoReg[16] := EscreveCampo(06, 'N', FormatFloat('000000',NSeqReg));//395 400 > Alinhado à direita e zeros à esquerda;
    end
    else
    ...
    begin
      ...
      CampoReg[06] := EscreveCampo(04, 'N', BancodeDados.BancosAGENCIA.AsString + BancodeDados.BancosDIG_AGENCIA.Value); //027 a 030 > Código do beneficiário
      CampoReg[07] := EscreveCampo(06, 'N', BancodeDados.BancosCODIGO_CEDENTE.AsString);//RetornaNumeros(BancodeDados.BancosCONTA_CORRENTE.AsString) + RetornaNumeros(BancodeDados.BancosDIG_CONTA_CORRENTE.AsString));
      CampoReg[08] := EscreveCampo(10, 'X', ''); //37 a 46 > Preencher espaços
      CampoReg[09] := EscreveCampo(30, 'X', BancodeDados.ConfigFANTASIA.Value); //47 a 76 >> Preencher com o Nome da Empresa
    ...
    begin
      ...
      CampoReg[07] := EscreveCampo(30, 'X', BancodeDados.ConfigFANTASIA.Value);
      CampoReg[08] := EscreveCampo(03, 'N', IntToStr(CodBanco));
      CampoReg[09] := EscreveCampo(15, 'X', 'BANCO ITAU SA');//BancodeDados.BancosBANCO_NOME.Value);
      CampoReg[10] := EscreveCampo(06, 'D2', DateToStr(Date));
...
begin
  ...
  begin
    inc(NSeqReg);
      ...
      qryPesq.Close;
      qryPesq.SQL.Text := 'select nome, cli_id, cnpj, cnpj, logradouro, bairro, (select nome from municipios where codmun=coalesce(clientes.cidade,0)) as cidade, (select uf from municipios where codmun=coalesce(clientes.cidade,0))  as uf, cep, numero '+ ' from clientes where cli_id= ' + IntToStr(BoletosCLI_ID.Value);
      qryPesq.Open;
    ...
    if (CodBanco = 004) then  begin //BANCO DO NORDESTE

       case BancodeDados.BancosCARTEIRA.Value of //Calculando a carteira para remessa
            //47  : CarteiraTmp :='1';
            //45  : CarteiraTmp :='2';
            //46  : CarteiraTmp :='3';
            50  : CarteiraTmp :='4'; //Cobrança Simples - boleto emitido pelo cliente.
            45  : CarteiraTmp :='5'; //Cobrança Vinculada - boleto emitido pelo cliente. 
            46  : CarteiraTmp :='6'; //Cobrança Caucionada - bloquete emitido pelo cliente. usar
            04  : CarteiraTmp :='7';
            48  : CarteiraTmp :='8';
            97  : CarteiraTmp :='9';
            51  : CarteiraTmp :='0';
      ...
      CampoReg[01] := EscreveCampo(01, 'N', '1'); //Posição: 1 a 1 >> Preencher com o número “1”.
      CampoReg[02] := EscreveCampo(16, 'X', '');  //Posição: 2 a 17 >> Brancos
      CampoReg[03] := EscreveCampo(04, 'N', FormatFloat('0000', BancodeDados.BancosAGENCIA.AsInteger) ); //Posição: 18 a 21 >> Preenchido com o código da Agência na qual o Cliente opera.
      CampoReg[04] := EscreveCampo(02, 'N', '00');//Posição: 22 a 23 >> Informar ‘00’
      CampoReg[05] := EscreveCampo(08, 'N', FormatFloat('000000', (BancodeDados.BancosCONTA_CORRENTE.AsInteger + BancodeDados.BancosDIG_CONTA_CORRENTE.AsInteger)) ); //Posição: 24 a 31 >> Preenchido com o número da Conta Corrente do Cliente cadastrado na cobrança como cedente + Preenchido com o Dígito da Conta do Cliente.

      if not BancodeDados.GradeFin.IsEmpty then //Posição: 32 a 33 >> Informar o percentual da multa a ser cobrada quando do atraso.
      begin
      if not (BancodeDados.GradeFinMULTA.Value>99) then
        CampoReg[06] := EscreveCampo(02, 'N', FormatFloat('00',BancodeDados.GradeFinMULTA.Value)) else  CampoReg[06] := EscreveCampo(02, 'N', FormatFloat(',0.00', 0));
      end
      else
      begin
        CampoReg[06] := EscreveCampo(02, 'N', FormatFloat(',0.00', 0));
      end;
      CampoReg[07] := EscreveCampo(04, 'X', '');//Posição: 034 a 037 >> Preencher com espaços
      CampoReg[08] := EscreveCampo(25, 'X', IntToStr(BoletosEMISSAO_ID.Value)); //Posição: 038 062  >>  número Controle.
      CampoReg[09] := EscreveCampo(08, 'X', EscreveCampo(09, 'X', RetornaNumerosComDV(BoletosNOSSO_NUMERO.Value))); //Posição: 63 a 70 >> Nosso Número + Dígito do Nosso Número.

      if trim(BancodeDados.BancosFOL_CONTRAT_REM.Value)<>'' then //Posição: 071 a 080 >> Nº do Contrato para cobrança caucionada e Vinculada, e com zeros para  cobrança simples.
      CampoReg[10] := EscreveCampo(10, 'N',BancodeDados.BancosFOL_CONTRAT_REM.Value) else CampoReg[10] := EscreveCampo(10, 'N', '0000000000');

      CampoReg[11] := EscreveCampo(06, 'N','000000'); //Posição: 81 a 86 >> Data do Segundo Desconto. Informar a Data do Segundo Desconto no formato “ddmmaa”, se houver. Caso contrário, preencher com zeros.
      CampoReg[12] := EscreveCampo(13, 'N','0000000000000');//Posição: 087 099 >>  Informar o Valor do Segundo Desconto, se houver. Caso  contrário, preencher com zeros.
      CampoReg[13] := EscreveCampo(08, 'X', ''); //Posição: 100 107 >> Brancos
      CampoReg[14] := EscreveCampo(01, 'N', CarteiraTmp);//Posição: 108 a 108
      CampoReg[15] := EscreveCampo(02, 'N', '01');//Posição: 109 110 >> 01 - Entrada Normal.
      ...
      CampoReg[17] := EscreveCampo(06, 'N', RetornaNumeros(FormatDateTime('dd/mm/yy',BoletosVENCIMENTO.Value)));// Posicao: 121 126 >> Informar no formato “ddmmaa”.
      CampoReg[18] := EscreveCampo(13, 'N', FormatFloat(',0.00', BoletosVALOR_DOC.Value));//Posição: 127 a 139 >> Valor do Título
      CampoReg[19] := EscreveCampo(03, 'N', IntToStr(CodBanco));//Posição: 140 a 142 >> Preencher ‘104’
      CampoReg[20] := EscreveCampo(04, 'N', '0000');//Posição: 143 146 >>  Preencher com zeros
      CampoReg[21] := EscreveCampo(01, 'X', '');//Posição: 147 147 >> Preencher com espaços
      CampoReg[22] := EscreveCampo(02, 'N', '04'); //Posição: 148 149 >> Informar a espécie do título: 04 - Carnê.
      CampoReg[23] := EscreveCampo(01, 'X', 'A');//Posição: 150 a 150 >> Preencher com "A" se aceito, ou "B" se não aceito
      CampoReg[24] := EscreveCampo(06, 'D2',  (FormatDateTime('dd/mm/yy',BoletosEMISSAO.Value)));///Posição: 151 156  >> Data de Emissão. Informar no formato "ddmmaa".
      CampoReg[25] := EscreveCampo(04, 'N', '05'); //Posição: 157 160 >> Código de Instrução: 05 - Acatar instruções contidas no título
      ...
      if BoletosDIAS_TOLERANCIA.Value>0 then //Posição: 174 a 179 >>> Data de Desconto. Informar no formato "ddmmaa". Se não houver, preencher com zeros.
      CampoReg[27] := EscreveCampo(06, 'N', RetornaNumeros(FormatDateTime('dd/mm/yy',BoletosVENCIMENTO.Value + BoletosDIAS_TOLERANCIA.Value))) else CampoReg[27] := EscreveCampo(06, 'N', '000000');
      CampoReg[28] := EscreveCampo(13, 'N', FormatFloat('00000000000000', BoletosDESCONTO.Value));  //Posição: 180 192 >> Valor do Desconto. Se não houver, preencher com zeros.
      CampoReg[29] := EscreveCampo(13, 'X', ''); //Posição: 193 205 >> Valor do IOC : mposto a ser recolhido pelo Banco quando se tratar de operações de seguro.
      CampoReg[30] := EscreveCampo(13, 'N', '0000000000000'); //Posição: 206 218 >> Especie  >> Valor do Abatimento   . Se não houver, preencher com zeros.

      if Length(CPFSacado) = 11 then   //Posicao: 219 a 220 >> Código de Inscrição do Sacado. Preencher c/ "01" para CPF, ou "02" para CGC.
        CampoReg[31] := EscreveCampo(02, 'N', '01')
      else
        CampoReg[31] := EscreveCampo(02, 'N', '02');

      CampoReg[32] := EscreveCampo(14, 'N', CPFSacado);//POsição: 221 234 >> Número do CGC/ CPF do Sacado. Informar CGC ou CPF do Sacado.
      CampoReg[33] := EscreveCampo(40, 'X', NomeSacado);//POsição: 235 274 >> Nome do Pagador

      CampoReg[34] := EscreveCampo(40, 'X', qryPesq.FieldByName('LOGRADOURO').AsString + ' ' + qryPesq.FieldByName('NUMERO').AsString); //Posição: 275 314  Endereço do Pagador
      ...
      CampoReg[39] := EscreveCampo(40, 'X', ''); //Posição: 368 a 389 >> Nome do Sacador/Avalista

      if (BancodeDados.BancosDIAS_PROTESTO.Value > 0) then  //Posição: 392 a 393 >> Prazo de Protesto. Informar o número de dias em atraso para envio do título  para protesto.
      begin
        CampoReg[40] := EscreveCampo(02, 'N', IntToStr(BancodeDados.BancosDIAS_PROTESTO.Value));
      end
      else
      begin
        CampoReg[40] := EscreveCampo(02, 'N', '99');  //Para que o título não seja protestado, preencher com "99".
      end;

      CampoReg[44] := EscreveCampo(01, 'N', '0');//Posição: 394 394 >> Código da Moeda. Informar o cód. da moeda do título  > 0 - R$.
      CampoReg[45] := EscreveCampo(06, 'N', IntToStr(NSeqReg)); //Posição:  395 a 400 >> Sequencial do Registro. Adicionar 1 ao sequencial do registro anterior.
    ...
    begin
      ...
      CampoReg[35] := EscreveCampo(38, 'X', qryPesq.FieldByName('LOGRADOURO').AsString + ' ' + qryPesq.FieldByName('NUMERO').AsString);

      CampoReg[36] := EscreveCampo(02, 'N', '0');
      CampoReg[37] := EscreveCampo(12, 'X', ObsEndSacado); //qryEndBAIRRO.Value);
    ...
    begin //CAIXA ECONÔMICA FEDERAL
      ...
      CampoReg[05] := EscreveCampo(06, 'N', BancodeDados.BancosCODIGO_CEDENTE.AsString); //FormatFloat('000000', (BancodeDados.BancosCONTA_CORRENTE.AsInteger + BancodeDados.BancosDIG_CONTA_CORRENTE.AsInteger)) ); //Posição: 22 a 27 >>Preencher com o Código que identifica a Empresa na CAIXA, fornecido pela agência de vinculação
      ...
      if  not Length(BoletosNOSSO_NUMERO.Value)>17 then begin
        CampoReg[10] := EscreveCampo(02, 'N', BancodeDados.BancosCARTEIRA.AsString); //Posição: 57 a 58 >> Identificação do Título na CAIXA
        CampoReg[11] := EscreveCampo(15, 'N', BoletosNOSSO_NUMERO.Value);  //59 a 73
      end else begin
        CampoReg[11] := EscreveCampo(17, 'N', (BoletosNOSSO_NUMERO.Value));  //57 a 73
      end;
      ...
      if (BoletosCOD_MOV_REM.Value>0) then //Posição: 109 a 110 >> Identificação Tipo Ocorrência do arquivo remessa: 01 Entrada de Título
      CampoReg[15] := EscreveCampo(02, 'N', FormatFloat('00',BoletosCOD_MOV_REM.Value)) else CampoReg[15] := EscreveCampo(02, 'N', '01'); // Ocorrencia = Remessa
      ...
      begin
        CampoReg[24] := EscreveCampo(02, 'X', '02'); // INSTRUCAO 1 >> ‘02’ = Devolver (Não Protestar)
      end;
      ...
      CampoReg[36] := EscreveCampo(08, 'N', qryPesq.FieldByName('CEP').AsString);//Posição: 327 a 334 >> CEP do Pagador
      CampoReg[37] := EscreveCampo(15, 'X', qryPesq.FieldByName('CIDADE').AsString);//Posição: 335 a 349 >> Cidade do Pagador
      CampoReg[38] := EscreveCampo(02, 'X', qryPesq.FieldByName('UF').AsString);//Posição: 350 351  >> Unidade da Federação do Pagado
      ...
      begin
        ...
        CampoReg[43] := EscreveCampo(02, 'N', IntToStr(BancodeDados.BancosDIAS_BAIXAR_BANCO.Value));
      end;
    ...
    if (CodBanco = 748) then
     begin //Sicredi e UNICRED
      ...
      CampoReg[42] := EscreveCampo(40, 'X', qryPesq.FieldByName('LOGRADOURO').AsString + ' ' + qryPesq.FieldByName('NUMERO').AsString + ',' + qryPesq.FieldByName('BAIRRO').AsString+ '-' +
    ...
    begin
      ...
      CampoReg[01] := EscreveCampo(01, 'N', '1'); // posição: 001 a 001 >> Identificação do  Registro: 1
      CampoReg[02] := EscreveCampo(05, 'N', '0'); // POsição: 002 a 006 >> Código da Agência do Pagador Exclusivo para Débito em Conta / opcional
      CampoReg[03] := EscreveCampo(01, 'X', '');  // POsição: 007 a 007 >> Dígito da Agência de Débito (opcional)
      CampoReg[04] := EscreveCampo(05, 'N', '0'); // POsição: 008 a 012 >> Razão da Conta Corrente (opcional)
      CampoReg[05] := EscreveCampo(07, 'N', '0'); // POsição: 013 a 019 >> Conta Corrente  (opcional)
      CampoReg[06] := EscreveCampo(01, 'X', '');  // POsição: 020 a 020 >> digito da conta  (opcional)
      CampoReg[07] := EscreveCampo(17, 'N', '0' + EscreveCampo(03, 'N', BancodeDados.BancosCARTEIRA.AsString) + EscreveCampo(05, 'N',
        ...
        BancodeDados.BancosDIG_CONTA_CORRENTE.Value)); // Posição: 021 a 037 >>   Identificação da Empresa Beneficiária no Banco

      CampoReg[08] := EscreveCampo(25, 'X', IntToStr(BoletosEMISSAO_ID.Value)); //Posição: 038 a 062  >> Nº Controle do Participante
      CampoReg[09] := EscreveCampo(03, 'N', '0'); // Posição: 063 a 065 >>  Código do Banco a ser debitado na Câmara de  Compensação
      ...
      if (bancodeDados.GradeFinMULTA.value>0) then begin
        ...
        CampoReg[10] := EscreveCampo(05, 'N', '2'+ EscreveCampo(4, 'N', FormatFloat(',0.00', bancodeDados.GradeFinMULTA.value)));//Posição: 066 a 066 >> Campo de Multa

                                                    //Posição: 067 a 070 >> Percentual de multa

      end else
      CampoReg[10] := EscreveCampo(05, 'N', '0'); //Posição: 066 a 066 >> Campo de Multa
                                                  //Posição: 067 a 070 >> Percentual de multa


      CampoReg[11] := EscreveCampo(12, 'X', Formatar(RetornaNumerosComDV(BoletosNOSSO_NUMERO.Value), 12, false, '0'));// Posição:
      ...
      CampoReg[20] := EscreveCampo(10, 'X', IntToStr(BoletosEMISSAO_ID.Value));  // Posição:

      CampoReg[21] := EscreveCampo(06, 'D2', DateToStr(BoletosVENCIMENTO.Value)); // Posição:
      CampoReg[22] := EscreveCampo(13, 'N', FormatFloat(',0.00', BoletosVALOR_DOC.Value)); // Posição:
      ...
      CampoReg[38] := EscreveCampo(40, 'X', qryPesq.FieldByName('LOGRADOURO').AsString + ' ' + qryPesq.FieldByName('NUMERO').AsString + ', ' + qryPesq.FieldByName('BAIRRO').AsString + '-' +
    ...
    begin
      CampoReg[01] := EscreveCampo(01, 'N', '1');  //IDENTIFICAÇÃO DO REGISTRO TRANSAÇÃO 001   001

      if Length(RetornaNumeros(BancodeDados.ConfigCNPJ.Value)) = 11 then
        CampoReg[02] := EscreveCampo(02, 'N', '1')
      else
        CampoReg[02] := EscreveCampo(02, 'N', '2'); //IDENTIFICAÇÃO DO LAYOUT PARA O REGISTRO  002   002
      ...
      CampoReg[33] := EscreveCampo(40, 'X', qryPesq.FieldByName('LOGRADOURO').AsString + ' ' + qryPesq.FieldByName('NUMERO').AsString);
    ...
    begin
      ...
      CampoReg[06] := EscreveCampo(35, 'X', qryPesq.FieldByName('LOGRADOURO').AsString + ' ' + qryPesq.FieldByName('NUMERO').AsString);
    ...
    begin
      ...
      CampoReg[29] := EscreveCampo(40, 'X', qryPesq.FieldByName('LOGRADOURO').AsString + ' ' + qryPesq.FieldByName('NUMERO').AsString);
    ...
    begin
      ...
      if not BancodeDados.GradeFin.IsEmpty then
      begin
        CampoReg[21] := EscreveCampo(13, 'N', FormatFloat(',0.00', 0)); // JUROS ///CampoReg[21] := EscreveCampo(13, 'N', FormatFloat(',0.00', (BoletosVALOR_DOC.Value * BancodeDados.Grade_FinanceiraJUROS.Value) / 100));
     ...
     //CampoReg[34] := EscreveCampo(30, 'X', '');  //eraldo cometou em 24/02/16 inluindo o nome do cedente no detalhe
      CampoReg[34] := EscreveCampo(30, 'X', copy(BancodeDados.ConfigFANTASIA.AsString,0,29));
    ...
    begin
      ...
      CampoReg[41] := EscreveCampo(40, 'X', qryPesq.FieldByName('LOGRADOURO').AsString + ' ' + qryPesq.FieldByName('NUMERO').AsString);
    ...
    begin
      ...
      CampoReg[34] := EscreveCampo(40, 'X', qryPesq.FieldByName('LOGRADOURO').AsString + ' ' + qryPesq.FieldByName('NUMERO').AsString);
    ...
    begin
      ...
      CampoReg[39] := EscreveCampo(30, 'X', BancodeDados.ConfigFANTASIA.Value); // Sacador
    ...
    begin
      ...
      CampoReg[46] := EscreveCampo(37, 'X', qryPesq.FieldByName('LOGRADOURO').AsString + ' ' + qryPesq.FieldByName('NUMERO').AsString);
...
begin
  ...
  begin
    ...
    begin
      ...
      if (CodBanco = 341) or (CodBanco = 756) or (CodBanco = 237) or (CodBanco = 004) then //Itaú, SICOOB, Bradesco, Nordeste
    ...
    begin
      try
        ...
        begin
          ...
          BancodeDados.Adiciona.SQL.Text := 'update boletos_emissao set remessa = ' + IntToStr(idRemessa) + ', remessa_id_ant = ' + IntToStr(idRemessa) + ' where emissao_id = ' + IntToStr(BoletosEMISSAO_ID.Value);
          BancodeDados.Adiciona.ExecSQL;

          BancodeDados.Adiciona.SQL.Text := 'update boletos set remessa_id = ' + IntToStr(idRemessa) +' where emissao_id = ' + IntToStr(BoletosEMISSAO_ID.Value);
          BancodeDados.Adiciona.ExecSQL;
...
BoletosTem_End.Value:='SIM' else BoletosTem_End.Value:='NÃO';
...
begin
  ...
  begin //1
    ...
    begin //2
      ...
      BancodeDados.QrySql.SQL.Text:='select * from remessa where banco_id= '+IntToStr(BancodeDados.BancosBANCO_ID.Value)+' and (data between ' + QuotedStr(FormatDateTime('yyyy/mm/dd', PrincipalForm.DataServidor)+' 00:00:00')+' and '+QuotedStr(FormatDateTime('yyyy/mm/dd', PrincipalForm.DataServidor)+' 23:59:59')+')';
        ...
        begin //3
          ...
          begin //       //BRADESCO     //Itaú              // HSBC               //BICBANCO            // Daycoval           //BRB
            if ((CodBanco <> 237) and (CodBanco <> 341) and (CodBanco <> 399) and (CodBanco <> 320) and (CodBanco <> 707) and (CodBanco <> 070) and
              (CodBanco <> 356) and (CodBanco <> 389) and (CodBanco <> 422) and (CodBanco <> 756) and (CodBanco <> 748) and (CodBanco <> 104) ) and (CodBanco <> 004)  then
          ...
          begin //4
            ...
            BancodeDados.Config.SQL.Text :='select c.*, (select m.nome from municipios m where m.codmun=c.cidade) as cidade_nome from config c where c.conf_id = ' + IntToStr(InstIdTmp);
            ...
            begin
              Mensagem('Não foi possível localizar da instituição.', mtInformation, [mbOk], mrOk, 0);
              Screen.Cursor := crDefault;
              HabilitarBotoes(Self, true);
            end else
            begin //5
              ...
              BancodeDados.Remessa.SQL.Text := 'select * from remessa where data = ' + QuotedStr(FormatDateTime('yyyy/mm/dd hh:nn:ss', DtHrArq));
