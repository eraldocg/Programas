...
procedure TCsLigacoesForm.EditNomeKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
...
begin
if BancodeDados.LigacoesLIGACAO_ID.Value>0 then begin
HabilitarBotoes(self,false);
  try


  //  if BancodeDados.QrySql.IsEmpty then begin
      if PrincipalForm.LiberaAcesso then begin
...
SoAtrasado:=false;
  ...
  Try
    ...
    begin
      Close;
      SQL.Text:='select * from ligacoes where (dt_ligar between '+QuotedStr(FormatDateTime('yyyy/mm/dd',Dt_Inicial.DateTime))+' and '+QuotedStr(FormatDateTime('yyyy/mm/dd',Dt_Final.DateTime)+' 23:59:59') +')';
...
begin
    ...
    begin
      ...
      try
        ...
        RelLigacoesForm.lbPeriodo4.Caption  :='Período: PENDÊNCIA' else
        RelLigacoesForm.lbPeriodo4.Caption  :='Perído: '+Dt_Inicial.DateTimeStr +' até '+Dt_Final.DateTimeStr;
...
begin
  ...
  try
    if not BancodeDados.Ligacoes.Active then BancodeDados.Ligacoes.Open;
    if not assigned(CadLigacoesForm) then
      CadLigacoesForm := TCadLigacoesForm.create(Application);
    try
      Screen.Cursor := crSQLWait;
...
begin
  ...
  begin
    ...
    begin
      if not (BancodeDados.Ligacoes.State in [dsEdit]) then  BancodeDados.Ligacoes.Edit;
      BancodeDados.MarcarAtivo(listaCli, BancodeDados.LigacoesLIGACAO_ID.Value, BancodeDados.LigacoesATIVO);
...
procedure TCsLigacoesForm.DBGrid1DrawColumnCell(Sender: TObject; const Rect: TRect; DataCol: Integer; Column: TColumn; State: TGridDrawState);
begin
...
procedure TCsLigacoesForm.DBGrid1KeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  if (Key = 32) then
  begin
...
if Assigned(CsClientesForm) then
BancodeDados.Ligacoes.SQL.Add(' and cli_id = '+IntToStr(BancodeDados.ClientesCLI_ID.Value));
